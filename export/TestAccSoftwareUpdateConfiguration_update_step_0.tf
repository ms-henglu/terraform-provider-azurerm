...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-220923011531865777"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-220923011531865777"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-220923011531865777"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-220923011531865777"
  ...
  schedule {
    ...
    start_time          = "2022-09-23T11:15:31Z"
    expiry_time         = "2022-09-25T03:15:31Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
