...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-240119024619139606"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF240119024619139606"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone240119024619139606.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-240119024619139606"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-240119024619139606"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol240119024619139606"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
