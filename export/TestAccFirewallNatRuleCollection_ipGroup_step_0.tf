...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-211013071906788421"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet211013071906788421"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip211013071906788421"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall211013071906788421"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall_nat_rule_collection" "test" {
  name                = "acctestnrc-211013071906788421"
