...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-lab-231218071951639357"
  location = "West Europe"
}


resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig231218071951639357"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_shared_image" "test" {
  name                = "acctestimg231218071951639357"
  ...
  identifier {
    publisher = "AccTesPublisher231218071951639357"
    offer     = "AccTesOffer231218071951639357"
    sku       = "AccTesSku231218071951639357"
  }
}
...
resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-231218071951639357"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-sn-231218071951639357"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-231218071951639357"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-linux-231218071951639357"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  size                = "Standard_F2"
...
resource "azurerm_lab_service_plan" "test" {
  name                = "acctest-lslp-231218071951639357"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_lab_service_lab" "test" {
  name                = "acctest-lab-231218071951639357"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  title               = "Test Title"
