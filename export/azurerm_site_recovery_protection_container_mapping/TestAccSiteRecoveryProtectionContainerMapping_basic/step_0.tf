...
resource "azurerm_resource_group" "test1" {
  name     = "acctestRG-recovery-221028165431279415-1"
  location = "West Europe"
}

resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-vault-221028165431279415"
...
resource "azurerm_site_recovery_fabric" "test1" {
  ...
  name                = "acctest-fabric1-221028165431279415"
  location            = azurerm_resource_group.test1.location
}
...
resource "azurerm_site_recovery_fabric" "test2" {
  resource_group_name = azurerm_resource_group.test1.name
  recovery_vault_name = azurerm_recovery_services_vault.test.name
  name                = "acctest-fabric2-221028165431279415"
  location            = "West US 2"
...
resource "azurerm_site_recovery_protection_container" "test1" {
  ...
  name                 = "acctest-protection-cont1-221028165431279415"
}
...
resource "azurerm_site_recovery_protection_container" "test2" {
  ...
  name                 = "acctest-protection-cont2-221028165431279415"
}

resource "azurerm_site_recovery_replication_policy" "test" {
  resource_group_name                                  = azurerm_resource_group.test1.name
  recovery_vault_name                                  = azurerm_recovery_services_vault.test.name
  name                                                 = "acctest-policy-221028165431279415"
...
resource "azurerm_site_recovery_protection_container_mapping" "test" {
  ...
  name                                      = "mapping-221028165431279415"
}
