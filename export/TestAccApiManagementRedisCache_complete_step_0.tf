...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-apim-220324175907138173"
...
resource "azurerm_redis_cache" "test" {
  name                = "acctestRedis-220324175907138173"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  capacity            = 1
  family              = "C"
...
resource "azurerm_redis_cache" "test2" {
  name                = "acctestRedis2-220324175907138173"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  capacity            = 1
  family              = "C"
...
resource "azurerm_api_management_redis_cache" "test" {
  name              = "acctest-Redis-Cache-220324175907138173"
