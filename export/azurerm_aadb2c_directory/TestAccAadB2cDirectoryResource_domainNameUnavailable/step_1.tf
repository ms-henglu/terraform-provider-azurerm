...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230512010118678296"
...
resource "azurerm_aadb2c_directory" "test" {
  ...
  display_name            = "acctest230512010118678296"
  domain_name             = "acctest230512010118678296.onmicrosoft.com"
...
resource "azurerm_resource_group" "duplicate" {
  name     = "acctestRG-duplicate-230512010118678296"
  location = "West US 2"
}
...
resource "azurerm_aadb2c_directory" "duplicate" {
  ...
  display_name            = "acctest-duplicate-230512010118678296"
