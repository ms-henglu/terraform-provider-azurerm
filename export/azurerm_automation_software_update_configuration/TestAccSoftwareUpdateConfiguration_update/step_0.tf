...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-230428045239862630"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-230428045239862630"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-230428045239862630"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-230428045239862630"
  ...
  schedule {
    ...
    start_time          = "2023-04-28T14:52:39Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
