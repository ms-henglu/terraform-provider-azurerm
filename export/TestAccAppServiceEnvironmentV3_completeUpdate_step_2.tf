...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ase-210830084633040201"
  location = "West Europe"
}

resource "azurerm_resource_group" "test2" {
  name     = "acctestRG2-ase-210830084633040201"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-210830084633040201"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-subnet-210830084633040201"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_app_service_environment_v3" "test" {
  name                         = "acctest-ase-210830084633040201"
