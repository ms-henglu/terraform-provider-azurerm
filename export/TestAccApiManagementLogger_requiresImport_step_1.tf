...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220610022158461126"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-220610022158461126"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-220610022158461126"
...
resource "azurerm_api_management" "test" {
  name                = "acctestAM-220610022158461126"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  publisher_name      = "pub1"
...
resource "azurerm_api_management_logger" "test" {
  name                = "acctestapimnglogger-220610022158461126"
