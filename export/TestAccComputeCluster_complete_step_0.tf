...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ml-210924004503364589"
...
resource "azurerm_application_insights" "test" {
  name                = "acctestai-210924004503364589"
...
resource "azurerm_machine_learning_workspace" "test" {
  name                    = "acctest-MLW2109240045033645"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet210924004503364589"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet210924004503364589"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "test-nsg-210924004503364589"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  security_rule {
    name                       = "test123"
