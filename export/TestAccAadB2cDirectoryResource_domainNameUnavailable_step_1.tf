...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220128082028411340"
...
resource "azurerm_aadb2c_directory" "test" {
  ...
  display_name            = "acctest220128082028411340"
  domain_name             = "acctest220128082028411340.onmicrosoft.com"
...
resource "azurerm_resource_group" "duplicate" {
  name     = "acctestRG-duplicate-220128082028411340"
  location = "West US 2"
}
...
resource "azurerm_aadb2c_directory" "duplicate" {
  ...
  display_name            = "acctest-duplicate-220128082028411340"
