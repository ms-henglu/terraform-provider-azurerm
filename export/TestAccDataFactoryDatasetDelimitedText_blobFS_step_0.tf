...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-210917031554154381"
...
resource "azurerm_storage_data_lake_gen2_filesystem" "test" {
  name               = "acctest-datalake-210917031554154381"
  storage_account_id = azurerm_storage_account.test.id
}

resource "azurerm_data_factory" "test" {
  name                = "acctestdf210917031554154381"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_linked_service_data_lake_storage_gen2" "test" {
  name                 = "acctestDataLakeStorage210917031554154381"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_dataset_delimited_text" "test" {
  name                = "acctestds210917031554154381"
  resource_group_name = azurerm_resource_group.test.name
  data_factory_name   = azurerm_data_factory.test.name
