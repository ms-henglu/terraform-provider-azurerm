...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-datashare-211001020720834350"
  location = "West Europe"
}

resource "azurerm_data_share_account" "test" {
  name                = "acctest-DSA-211001020720834350"
...
resource "azurerm_data_share" "test" {
  name       = "acctest_DS_211001020720834350"
  account_id = azurerm_data_share_account.test.id
  kind       = "CopyBased"
}

resource "azurerm_data_lake_store" "test" {
  name                = "acctestdlsslhm0"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_data_share_dataset_data_lake_gen1" "test" {
  name               = "acctest-DSDL1-211001020720834350"
