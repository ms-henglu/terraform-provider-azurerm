...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mongouserdef-231013043220273598"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca-231013043220273598"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-231013043220273598"
...
resource "azurerm_cosmosdb_mongo_role_definition" "test" {
  ...
  role_name                = "acctestmongoroledef231013043220273598"
}

resource "azurerm_cosmosdb_mongo_role_definition" "test2" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  role_name                = "acctestmongoroledef2231013043220273598"
}

resource "azurerm_cosmosdb_mongo_user_definition" "test" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  username                 = "myUserName-231013043220273598"
  password                 = "myPassword2-231013043220273598"
