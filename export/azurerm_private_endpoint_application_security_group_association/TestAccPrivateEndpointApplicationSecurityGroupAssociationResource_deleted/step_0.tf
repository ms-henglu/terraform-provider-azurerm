...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-PEASGAsso-230120054937306960"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvnet-230120054937306960"
...
resource "azurerm_subnet" "service" {
  name                 = "acctestsnetservice-230120054937306960"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "endpoint" {
  name                 = "acctestsnetendpoint-230120054937306960"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip-230120054937306960"
  sku                 = "Standard"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_lb" "test" {
  name                = "acctestlb-230120054937306960"
  sku                 = "Standard"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_private_link_service" "test" {
  name                           = "acctestPLS-230120054937306960"
  location                       = azurerm_resource_group.test.location
  resource_group_name            = azurerm_resource_group.test.name
  ...
  nat_ip_configuration {
    name      = "primaryIpConfiguration-230120054937306960"
    primary   = true
...
resource "azurerm_private_endpoint" "test" {
  name                = "acctest-privatelink-230120054937306960"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_application_security_group" "test" {
  name                = "acctest-230120054937306960"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
