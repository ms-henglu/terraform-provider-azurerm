...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-backup-221216014047854640"
...
resource "azurerm_storage_share" "test" {
  name                 = "acctest-ss-221216014047854640"
...
resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-VAULT-221216014047854640"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
  sku                 = "Standard"
...
resource "azurerm_backup_policy_file_share" "test1" {
  name                = "acctest-PFS-221216014047854640"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_backup_policy_file_share" "test2" {
  name                = "acctest-221216014047854640-Secondary"
  resource_group_name = azurerm_resource_group.test.name
  recovery_vault_name = azurerm_recovery_services_vault.test.name
