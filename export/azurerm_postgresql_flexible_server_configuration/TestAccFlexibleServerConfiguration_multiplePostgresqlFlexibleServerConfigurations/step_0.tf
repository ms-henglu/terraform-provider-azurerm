...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-postgresql-231016034526735198"
  location = "West Europe"
}


resource "azurerm_virtual_network" "test" {
  name                = "acctest-vn-231016034526735198"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-sn-231016034526735198"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_private_dns_zone" "test" {
  name                = "acc231016034526735198.postgres.database.azure.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_private_dns_zone_virtual_network_link" "test" {
  name                  = "acctestVnetZone231016034526735198.com"
...
resource "azurerm_postgresql_flexible_server" "test" {
  name                   = "acctest-fs-231016034526735198"
  resource_group_name    = azurerm_resource_group.test.name
  location               = azurerm_resource_group.test.location
