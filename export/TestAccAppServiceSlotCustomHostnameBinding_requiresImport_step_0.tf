...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220527034834667672"
  location = "West Europe"
}

resource "azurerm_app_service_plan" "test" {
  name                = "acctestASP-220527034834667672"
...
resource "azurerm_app_service" "test" {
  name                = "accestAS-220527034834667672"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_app_service_slot_custom_hostname_binding" "test" {
  ...
  hostname            = "staging.accestAS-220527034834667672.ARM_TEST_DNS_ZONE"
