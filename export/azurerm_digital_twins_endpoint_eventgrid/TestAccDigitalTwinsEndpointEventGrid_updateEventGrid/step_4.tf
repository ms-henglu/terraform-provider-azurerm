...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-dtwin-230915023329592691"
  location = "West Europe"
}


resource "azurerm_digital_twins_instance" "test" {
  name                = "acctest-DT-230915023329592691"
...
resource "azurerm_eventgrid_topic" "test" {
  name                = "acctesteg-230915023329592691"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_eventgrid_topic" "test_alt" {
  name                = "acctesteg-alt-230915023329592691"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_digital_twins_endpoint_eventgrid" "test" {
  name                                 = "acctest-EG-230915023329592691"
