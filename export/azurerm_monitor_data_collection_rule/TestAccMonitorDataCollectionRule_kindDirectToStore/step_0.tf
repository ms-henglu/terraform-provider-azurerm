...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-DataCollectionRule-240311032620066432"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acceventn240311032620066432"
...
resource "azurerm_eventhub" "test" {
  name                = "accevent240311032620066432"
...
resource "azurerm_storage_container" "test" {
  name                  = "acccontainer240311032620066432"
  storage_account_name  = azurerm_storage_account.test.name
  container_access_type = "private"
}

resource "azurerm_storage_table" "test" {
  name                 = "acctable240311032620066432"
  storage_account_name = azurerm_storage_account.test.name
}

resource "azurerm_monitor_data_collection_rule" "test" {
  name                = "acctestmdcr-240311032620066432"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
