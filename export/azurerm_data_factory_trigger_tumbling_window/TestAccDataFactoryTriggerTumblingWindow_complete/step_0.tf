...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-221111020338615173"
  location = "West Europe"
}

resource "azurerm_data_factory" "test" {
  name                = "acctestdf221111020338615173"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_data_factory_pipeline" "test" {
  name            = "acctestdfp221111020338615173"
...
resource "azurerm_data_factory_trigger_tumbling_window" "dependency" {
  name            = "acctestdft2221111020338615173"
  data_factory_id = azurerm_data_factory.test.id
  frequency       = "Minute"
  interval        = 15
...
resource "azurerm_data_factory_trigger_tumbling_window" "test" {
  name            = "acctestdft221111020338615173"
  data_factory_id = azurerm_data_factory.test.id
  start_time      = "2022-09-21T00:00:00Z"
