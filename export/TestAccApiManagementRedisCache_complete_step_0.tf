...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-apim-220408050840583096"
  location = "West Europe"
}

resource "azurerm_api_management" "test" {
  name                = "acctestAM-220408050840583096"
...
resource "azurerm_redis_cache" "test" {
  name                = "acctestRedis-220408050840583096"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  capacity            = 1
  family              = "C"
...
resource "azurerm_redis_cache" "test2" {
  name                = "acctestRedis2-220408050840583096"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  capacity            = 1
  family              = "C"
...
resource "azurerm_api_management_redis_cache" "test" {
  name              = "acctest-Redis-Cache-220408050840583096"
