...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-230519074315128566"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-230519074315128566"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "accTestOriginGroup-230519074315128566"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "accTestOrigin-230519074315128566"
...
resource "azurerm_cdn_frontdoor_rule_set" "test" {
  name                     = "accTestRuleSet230519074315128566"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
}
...
resource "azurerm_cdn_frontdoor_rule" "test" {
  ...
  name                      = "accTestRule230519074315128566"
