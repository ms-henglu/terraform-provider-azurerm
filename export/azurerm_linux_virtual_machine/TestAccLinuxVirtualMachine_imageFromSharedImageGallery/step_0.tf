...
locals {
  ...
  vm_name           = "acctestsourcevm-230414020945045509"
  admin_username    = "testadmin230414020945045509"
  admin_password    = "Password1234!230414020945045509"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230414020945045509"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230414020945045509"
...
resource "azurerm_public_ip" "test" {
  name                = "acctpip-230414020945045509"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_network_interface" "public" {
  name                = "acctnicsource-230414020945045509"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-230414020945045509"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig230414020945045509"
  resource_group_name = "${azurerm_resource_group.test.name}"
  location            = "${azurerm_resource_group.test.location}"
}
...
resource "azurerm_linux_virtual_machine" "source" {
  name                            = "acctestsourceVM-230414020945045509"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_F2"
