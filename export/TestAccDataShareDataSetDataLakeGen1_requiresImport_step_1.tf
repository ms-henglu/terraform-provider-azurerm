...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-datashare-211203013728742331"
  location = "West Europe"
}

resource "azurerm_data_share_account" "test" {
  name                = "acctest-DSA-211203013728742331"
...
resource "azurerm_data_share" "test" {
  name       = "acctest_DS_211203013728742331"
  account_id = azurerm_data_share_account.test.id
  kind       = "CopyBased"
}

resource "azurerm_data_lake_store" "test" {
  name                = "acctestdlspi64x"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_data_share_dataset_data_lake_gen1" "test" {
  name               = "acctest-DSDL1-211203013728742331"
