...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-210825032133291492"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctestehn-210825032133291492"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteh-210825032133291492"
...
resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-210825032133291492"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_eventhub_namespace" "updated" {
  name                = "acctestehn2-210825032133291492"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  capacity            = 1
...
resource "azurerm_eventhub" "updated" {
  name                = "acctesteh2-210825032133291492"
...
resource "azurerm_stream_analytics_output_eventhub" "test" {
  name                      = "acctestinput-210825032133291492"
