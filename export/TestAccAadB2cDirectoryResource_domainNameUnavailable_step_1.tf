...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220726001506466157"
...
resource "azurerm_aadb2c_directory" "test" {
  ...
  display_name            = "acctest220726001506466157"
  domain_name             = "acctest220726001506466157.onmicrosoft.com"
...
resource "azurerm_resource_group" "duplicate" {
  name     = "acctestRG-duplicate-220726001506466157"
  location = "West US 2"
}
...
resource "azurerm_aadb2c_directory" "duplicate" {
  ...
  display_name            = "acctest-duplicate-220726001506466157"
