...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-watcher-230707010742396016"
  location = "West Europe"
}

resource "azurerm_network_watcher" "test" {
  name                = "acctestnw-230707010742396016"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-230707010742396016"
...
resource "azurerm_network_interface" "test" {
  name                = "acctni-230707010742396016"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "test" {
  name                  = "acctvm-230707010742396016"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  os_profile {
    computer_name  = "hostname230707010742396016"
...
resource "azurerm_virtual_machine_packet_capture" "test" {
  name               = "acctestpc-230707010742396016"
