...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240112225231573692"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctestacc8zhkp"
...
resource "azurerm_cosmosdb_sql_container" "update" {
  name                = "test-containerupdate8zhkp"
  resource_group_name = azurerm_cosmosdb_account.test.resource_group_name
  account_name        = azurerm_cosmosdb_account.test.name
...
resource "azurerm_service_plan" "update" {
  location            = azurerm_resource_group.test.location
  name                = "testserviceplanupdate8zhkp"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "P1v2"
  os_type             = "Linux"
}
...
resource "azurerm_app_service_connection" "test" {
  name               = "acctestserviceconnector240112225231573692"
