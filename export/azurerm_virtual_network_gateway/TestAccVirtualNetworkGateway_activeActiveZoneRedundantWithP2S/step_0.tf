...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221117231247926838"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-221117231247926838"
...
resource "azurerm_public_ip" "first" {
  name = "acctestpip1-221117231247926838"
...
resource "azurerm_public_ip" "second" {
  name = "acctestpip2-221117231247926838"
...
resource "azurerm_public_ip" "thirth" {
  name = "acctestpip3-221117231247926838"
...
resource "azurerm_virtual_network_gateway" "test" {
  ...
  name                = "acctestvng-221117231247926838"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
