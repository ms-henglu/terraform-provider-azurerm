...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-240119025436789358"
  location = "West Europe"
}

resource "azurerm_storage_account" "test" {
  name                     = "unlikely23exst2acctqishf"
...
resource "azurerm_mssql_virtual_machine_group" "test" {
  ...
  wsfc_domain_profile {
    ...
    cluster_bootstrap_account_name = "bootstrapaccqishf"
    cluster_operator_account_name  = "opaccqishf"
    sql_service_account_name       = "sqlsrvaccqishf"
