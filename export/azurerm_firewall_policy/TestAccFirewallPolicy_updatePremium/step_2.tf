...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-networkfw-240112224516969106"
  location = "westeurope"
}

resource "azurerm_key_vault" "test" {
  name                            = "tlskv240112224516969106"
...
resource "azurerm_user_assigned_identity" "test" {
  name                = "acctestUAI-240112224516969106"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
data "azurerm_storage_account_sas" "test" {
  ...
  start  = "2024-01-12T22:45:16Z"
  expiry = "2024-01-13T22:45:16Z"
...
resource "azurerm_firewall_policy" "test" {
  name                     = "acctest-networkfw-Policy-240112224516969106"
  resource_group_name      = azurerm_resource_group.test.name
  location                 = azurerm_resource_group.test.location
  sku                      = "Premium"
