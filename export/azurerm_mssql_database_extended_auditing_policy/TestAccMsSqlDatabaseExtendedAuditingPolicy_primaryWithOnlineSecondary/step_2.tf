...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-221111020851864220"
  location = "West Europe"
}

resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-221111020851864220"
...
resource "azurerm_mssql_database" "test" {
  name      = "acctest-db-221111020851864220"
  server_id = azurerm_mssql_server.test.id
}

resource "azurerm_storage_account" "test" {
  name                     = "unlikely23exst2accttui71"
  resource_group_name      = azurerm_resource_group.test.name
  location                 = azurerm_resource_group.test.location
...
resource "azurerm_resource_group" "second" {
  name     = "acctestRG-mssql2-221111020851864220"
  location = "West US 2"
}

resource "azurerm_mssql_server" "second" {
  name                         = "acctest-sqlserver2-221111020851864220"
...
resource "azurerm_mssql_database" "secondary" {
  name                        = "acctest-dbs-221111020851864220"
