...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-221021030847884758"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-221021030847884758"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-221021030847884758"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-221021030847884758"
  ...
  schedule {
    ...
    start_time          = "2022-10-21T13:08:47Z"
    expiry_time         = "2022-10-23T05:08:47Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
