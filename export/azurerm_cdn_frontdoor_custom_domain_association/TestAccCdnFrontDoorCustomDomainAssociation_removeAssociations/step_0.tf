...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-230106031206501543"
...
resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "acctest-profile-230106031206501543"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "acctest-origin-group-230106031206501543"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "acctest-origin-230106031206501543"
  cdn_frontdoor_origin_group_id = azurerm_cdn_frontdoor_origin_group.test.id
  enabled                       = true

  certificate_name_check_enabled = false
  host_name                      = join(".", ["3s6f00qo9v", azurerm_dns_zone.test.name])
  priority                       = 1
  weight                         = 1
}

resource "azurerm_cdn_frontdoor_endpoint" "test" {
  name                     = "acctest-endpoint-230106031206501543"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
  enabled                  = true
}

resource "azurerm_cdn_frontdoor_route" "contoso" {
  name                          = "acctest-contoso-230106031206501543"
...
resource "azurerm_cdn_frontdoor_route" "fabrikam" {
  name                          = "acctest-fabrikam-230106031206501543"
  cdn_frontdoor_endpoint_id     = azurerm_cdn_frontdoor_endpoint.test.id
  cdn_frontdoor_origin_group_id = azurerm_cdn_frontdoor_origin_group.test.id
  cdn_frontdoor_origin_ids      = [azurerm_cdn_frontdoor_origin.test.id]
  enabled                       = true
  ...
  https_redirect_enabled = true
  forwarding_protocol    = "HttpsOnly"
  patterns_to_match      = ["/sub-h9iyzbosfv"]
  supported_protocols    = ["Http", "Https"]
