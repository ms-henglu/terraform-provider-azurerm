...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-monitor-211001224301503065"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestWorkspace-211001224301503065"
...
resource "azurerm_monitor_action_group" "test" {
  name                = "acctestActionGroup-211001224301503065"
  resource_group_name = "${azurerm_resource_group.test.name}"
  short_name          = "acctestag"
}

resource "azurerm_monitor_scheduled_query_rules_log" "test" {
  name                = "acctestsqr-211001224301503065"
  resource_group_name = "${azurerm_resource_group.test.name}"
  location            = "${azurerm_resource_group.test.location}"
...
resource "azurerm_monitor_metric_alert" "test" {
  name                = "acctestmal-211001224301503065"
  resource_group_name = "${azurerm_resource_group.test.name}"
