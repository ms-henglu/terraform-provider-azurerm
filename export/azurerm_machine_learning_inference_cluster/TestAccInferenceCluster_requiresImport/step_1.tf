...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ml-230512004303698698"
...
resource "azurerm_application_insights" "test" {
  name                = "acctestai-230512004303698698"
...
resource "azurerm_machine_learning_workspace" "test" {
  name                    = "acctest-MLW2305120043036986"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet230512004303698698"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet230512004303698698"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks230512004303698698"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = join("", ["acctestaks", azurerm_resource_group.test.location])
  node_resource_group = "acctestRGAKS-230512004303698698"
