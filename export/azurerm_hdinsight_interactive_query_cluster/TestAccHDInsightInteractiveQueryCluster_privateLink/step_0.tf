...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230203063444067373"
  location = "West Europe"
}

resource "azurerm_storage_account" "gen2test" {
  name                     = "accgen2testafm7t"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet230203063444067373"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet230203063444067373"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip230203063444067373"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_nat_gateway" "test" {
  name                    = "acctestnat230203063444067373"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
  sku_name                = "Standard"
...
resource "azurerm_hdinsight_interactive_query_cluster" "test" {
  ...
  name                = "acctesthdi-230203063444067373"
  resource_group_name = "${azurerm_resource_group.test.name}"
  location            = "${azurerm_resource_group.test.location}"
  cluster_version     = "4.0"
  tier                = "Standard"
...
resource "azurerm_network_security_group" "test" {
  name                = "acctestnsg-230203063444067373"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
