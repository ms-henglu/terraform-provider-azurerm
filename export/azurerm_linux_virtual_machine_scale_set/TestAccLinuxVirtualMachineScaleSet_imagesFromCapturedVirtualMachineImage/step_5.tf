...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221222034403142169"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-221222034403142169"
...
resource "azurerm_public_ip" "source" {
  name                = "source-221222034403142169"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "source" {
  name                = "sourcenic-221222034403142169"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-221222034403142169"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard_F2"
  instances           = 1
