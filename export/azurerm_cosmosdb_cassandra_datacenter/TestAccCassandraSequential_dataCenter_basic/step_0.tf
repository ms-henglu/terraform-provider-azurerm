...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ca-230120051745105226"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-230120051745105226"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-230120051745105226"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_cosmosdb_cassandra_cluster" "test" {
  name                           = "acctca-mi-cluster-230120051745105226"
  resource_group_name            = azurerm_resource_group.test.name
  location                       = azurerm_resource_group.test.location
...
resource "azurerm_cosmosdb_cassandra_datacenter" "test" {
  name                           = "acctca-mi-dc-230120051745105226"
