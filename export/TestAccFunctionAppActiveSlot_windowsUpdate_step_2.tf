...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220923011516699024"
...
resource "azurerm_service_plan" "test" {
  name                = "acctestASP-WAS-220923011516699024"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  os_type             = "Windows"
  sku_name            = "EP1"
...
resource "azurerm_windows_function_app" "test" {
  name                = "acctestWA-220923011516699024"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_windows_function_app_slot" "test" {
  name                       = "acctest-WFAS-220923011516699024"
...
resource "azurerm_windows_function_app_slot" "update" {
  name                       = "acctestWAS2-220923011516699024"
  function_app_id            = azurerm_windows_function_app.test.id
  storage_account_name       = azurerm_storage_account.test.name
  storage_account_access_key = azurerm_storage_account.test.primary_access_key
