...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-backup-231020041729059642"
...
resource "azurerm_storage_share" "test" {
  name                 = "acctest-ss-231020041729059642"
...
resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-VAULT-231020041729059642"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
  sku                 = "Standard"
...
resource "azurerm_backup_policy_file_share" "test1" {
  name                = "acctest-PFS-231020041729059642"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_backup_policy_file_share" "test2" {
  name                = "acctest-231020041729059642-Secondary"
  resource_group_name = azurerm_resource_group.test.name
  recovery_vault_name = azurerm_recovery_services_vault.test.name
