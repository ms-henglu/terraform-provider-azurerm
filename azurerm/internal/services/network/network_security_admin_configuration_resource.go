		...
		Schema: map[string]*schema.Schema{
			...
			"security_type": {
				...
				ValidateFunc: validation.StringInSlice([]string{
					string(network.SecurityTypeAdminPolicy),
					string(network.SecurityTypeUserPolicy),
				}, false),
			},
		},
	}
...
func resourceNetworkSecurityAdminConfigurationCreateUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	id := parse.NewNetworkSecurityAdminConfigurationID(subscriptionId, resourceGroup, networkManagerName, name)
	...
	securityConfiguration := network.SecurityConfiguration{
		SecurityConfigurationPropertiesFormat: &network.SecurityConfigurationPropertiesFormat{
			DeleteExistingNSGs: utils.Bool(d.Get("delete_existing_nsgs").(bool)),
