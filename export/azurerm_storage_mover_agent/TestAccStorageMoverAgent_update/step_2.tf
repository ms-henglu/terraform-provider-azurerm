...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230728033157807456"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230728033157807456"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-230728033157807456"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip-230728033157807456"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                            = "acctestVM-230728033157807456"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_F2"
  ...
  provisioner "file" {
    content = templatefile("scripts/install_arc.sh.tftpl", {
      resource_group_name = azurerm_resource_group.test.name
      uuid                = "440c76f0-4e13-1ee9-5f2d-6a34ba01acb4"
      location            = azurerm_resource_group.test.location
...
resource "azurerm_storage_mover" "test" {
  name                = "acctest-ssm-230728033157807456"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  depends_on = [
...
resource "azurerm_storage_mover_agent" "test" {
  name                     = "acctest-sa-230728033157807456"
