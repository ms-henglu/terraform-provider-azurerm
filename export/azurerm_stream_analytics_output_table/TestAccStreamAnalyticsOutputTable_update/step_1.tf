...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221028172904145330"
  location = "West Europe"
}

resource "azurerm_storage_account" "test" {
  name                     = "acctestaccvka4y"
...
resource "azurerm_storage_table" "test" {
  name                 = "acctestst221028172904145330"
  storage_account_name = azurerm_storage_account.test.name
}

resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-221028172904145330"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_storage_table" "updated" {
  name                 = "accteststu221028172904145330"
  storage_account_name = azurerm_storage_account.test.name
}

resource "azurerm_stream_analytics_output_table" "test" {
  name                      = "acctestoutput-221028172904145330"
