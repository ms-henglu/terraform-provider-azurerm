...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-appservice-240105061743614038"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-VNET-240105061743614038"
...
resource "azurerm_app_service_plan" "test" {
  name                = "acctest-ASP-240105061743614038"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_app_service" "test" {
  name                = "acctest-AS-240105061743614038"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_app_service_slot" "test-staging" {
  name                = "acctest-AS-240105061743614038-staging"
