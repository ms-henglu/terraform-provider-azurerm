...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-210825031936333605"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet-210825031936333605"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet-210825031936333605"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
  address_prefix       = "10.1.0.0/24"

  delegation {
    name = "acctestdelegation-210825031936333605"
...
resource "azurerm_network_profile" "test" {
  name                = "acctestnetprofile-210825031936333605"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name

  container_network_interface {
    name = "acctesteth-210825031936333605"

    ip_configuration {
      name      = "acctestipconfig-210825031936333605"
