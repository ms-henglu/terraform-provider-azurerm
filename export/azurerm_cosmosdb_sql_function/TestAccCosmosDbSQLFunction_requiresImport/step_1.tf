...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cosmos-230825024317966959"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-cosmos-230825024317966959"
...
resource "azurerm_cosmosdb_sql_database" "test" {
  name                = "acctest-sql-database-230825024317966959"
  resource_group_name = azurerm_cosmosdb_account.test.resource_group_name
  account_name        = azurerm_cosmosdb_account.test.name
}

resource "azurerm_cosmosdb_sql_container" "test" {
  name                = "acctest-sql-container-230825024317966959"
  resource_group_name = azurerm_cosmosdb_account.test.resource_group_name
  account_name        = azurerm_cosmosdb_account.test.name
...
resource "azurerm_cosmosdb_sql_function" "test" {
  name         = "acctest-dssdf-230825024317966959"
