...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240311031710546250"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet240311031710546250"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet240311031710546250"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "test-nic240311031710546250"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                            = "vm-test-proxy240311031710546250"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_B1s"
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks240311031710546250"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = "acctestaks240311031710546250"
  kubernetes_version  = "1.29.0"

  linux_profile {
    admin_username = "acctestuser240311031710546250"
    ssh_key {
