...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-security-230120052646285537"
  location = "West Europe"
}

resource "azurerm_iothub" "test" {
  name                = "acctestIoTHub-230120052646285537"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctest-law-230120052646285537"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
}
...
resource "azurerm_log_analytics_workspace" "test2" {
  name                = "acctest-law2-230120052646285537"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
}
...
resource "azurerm_iot_security_solution" "test" {
  name                = "acctest-Iot-Security-Solution-230120052646285537"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
