...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-traffic-230227180109658656"
  location = "West Europe"
}

resource "azurerm_traffic_manager_profile" "parent" {
  name                   = "acctest-TMP-230227180109658656"
  resource_group_name    = azurerm_resource_group.test.name
  traffic_routing_method = "Weighted"

  dns_config {
    relative_name = "acctest-tmp-230227180109658656"
    ttl           = 30
  }
...
resource "azurerm_traffic_manager_profile" "child" {
  name                   = "acctesttmpchild230227180109658656"
  resource_group_name    = azurerm_resource_group.test.name
  traffic_routing_method = "Priority"

  dns_config {
    relative_name = "acctesttmpchild230227180109658656"
    ttl           = 30
  }
...
resource "azurerm_traffic_manager_nested_endpoint" "test" {
  name                    = "acctestend-parent230227180109658656"
