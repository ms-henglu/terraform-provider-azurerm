...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-231013043849444225"
...
resource "azurerm_automation_account" "test" {
  name                = "acctestAA-231013043849444225"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_logic_app_workflow" "test" {
  name                = "acctestLA-231013043849444225"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
}
...
resource "azurerm_app_service_plan" "test" {
  name                = "acctestSP-231013043849444225"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_function_app" "test" {
  name                       = "acctestFA-231013043849444225"
  location                   = "${azurerm_resource_group.test.location}"
  resource_group_name        = "${azurerm_resource_group.test.name}"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acceptanceTestEventHubNamespace-231013043849444225"
  location            = "West Europe"
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  capacity            = 1
