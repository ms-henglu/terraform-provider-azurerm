...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211001054144768573"
  location = "West Europe"
}

resource "azurerm_logic_app_workflow" "test" {
  name                = "acctestlogicapp-211001054144768573"
  location            = "West Europe"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_logic_app_workflow" "test2" {
  name                = "acctestlogicapp2-211001054144768573"
  location            = "West Europe"
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_security_center_automation" "test" {
  name                = "acctestautomation-211001054144768573"
  location            = "West Europe"
  resource_group_name = azurerm_resource_group.test.name
