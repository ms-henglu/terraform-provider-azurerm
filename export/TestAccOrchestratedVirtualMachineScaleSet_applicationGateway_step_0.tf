...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-OVMSS-211119050629406068"
  location = "West Europe"
}


resource "azurerm_public_ip" "test" {
  name                = "acctpip-211119050629406068"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctvn-211119050629406068"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-211119050629406068"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_nat_gateway" "test" {
  name                    = "acctng-211119050629406068"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
  sku_name                = "Standard"
...
resource "azurerm_orchestrated_virtual_machine_scale_set" "test" {
  name                = "acctestOVMSS-211119050629406068"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    linux_configuration {
      computer_name_prefix = "testvm-211119050629406068"
...
resource "azurerm_subnet" "gwtest" {
  name                 = "gw-subnet-211119050629406068"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_public_ip" "gwtest" {
  name                = "acctest-pubip-211119050629406068"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_application_gateway" "test" {
  name                = "acctestgw-211119050629406068"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
