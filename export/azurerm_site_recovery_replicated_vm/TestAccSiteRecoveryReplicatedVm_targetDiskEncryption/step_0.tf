...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-recovery-230428050409644427-1"
  location = "West Europe"
}
resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-recovery-230428050409644427-2"
  location = "West US 2"
}

resource "azurerm_key_vault" "test1" {
  name                        = "acckv-230428050409644427"
...
resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-vault-230428050409644427"
...
resource "azurerm_site_recovery_fabric" "test1" {
  ...
  name                = "acctest-fabric1-230428050409644427"
  location            = azurerm_resource_group.test.location
}
resource "azurerm_site_recovery_protection_container" "test1" {
  ...
  name                 = "acctest-protection-cont1-230428050409644427"
}
resource "azurerm_site_recovery_protection_container" "test2" {
  resource_group_name  = azurerm_resource_group.test2.name
  recovery_vault_name  = azurerm_recovery_services_vault.test.name
  recovery_fabric_name = azurerm_site_recovery_fabric.test1.name
  name                 = "acctest-protection-cont2-t-230428050409644427"
}
resource "azurerm_site_recovery_replication_policy" "test" {
  resource_group_name                                  = azurerm_resource_group.test2.name
  recovery_vault_name                                  = azurerm_recovery_services_vault.test.name
  name                                                 = "acctest-policy-230428050409644427"
...
resource "azurerm_site_recovery_protection_container_mapping" "test" {
  ...
  name                                      = "mapping-230428050409644427"
}
resource "azurerm_virtual_network" "test1" {
  name                = "net-230428050409644427"
  resource_group_name = azurerm_resource_group.test.name
  address_space       = ["192.168.1.0/24"]
  location            = azurerm_site_recovery_fabric.test1.location
}
resource "azurerm_subnet" "test1" {
  name                 = "snet-230428050409644427"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test1.name
  address_prefixes     = ["192.168.1.0/24"]
}
resource "azurerm_network_interface" "test" {
  name                = "vm-230428050409644427"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ip_configuration {
    name                          = "vm-230428050409644427"
...
resource "azurerm_snapshot" "test" {
  name                = "snapshot-230428050409644427"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  create_option       = "Copy"
...
resource "azurerm_site_recovery_replicated_vm" "test" {
  ...
  network_interface {
    ...
    target_subnet_name          = "snet-230428050409644427"
  }
  depends_on = [
