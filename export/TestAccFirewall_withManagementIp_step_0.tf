...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-220128052544431152"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet220128052544431152"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip220128052544431152"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test_mgmt" {
  name                = "acctestmgmtpip220128052544431152"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  sku                 = "Standard"
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall220128052544431152"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
