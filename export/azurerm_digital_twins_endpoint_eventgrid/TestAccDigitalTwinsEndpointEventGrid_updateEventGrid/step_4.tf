...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-dtwin-230616074655795011"
  location = "West Europe"
}


resource "azurerm_digital_twins_instance" "test" {
  name                = "acctest-DT-230616074655795011"
...
resource "azurerm_eventgrid_topic" "test" {
  name                = "acctesteg-230616074655795011"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_eventgrid_topic" "test_alt" {
  name                = "acctesteg-alt-230616074655795011"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_digital_twins_endpoint_eventgrid" "test" {
  name                                 = "acctest-EG-230616074655795011"
