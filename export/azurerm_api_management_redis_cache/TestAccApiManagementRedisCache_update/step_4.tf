...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-apim-221111013031387766"
  location = "West Europe"
}

resource "azurerm_api_management" "test" {
  name                = "acctestAM-221111013031387766"
...
resource "azurerm_redis_cache" "test" {
  name                = "acctestRedis-221111013031387766"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  capacity            = 1
  family              = "C"
...
resource "azurerm_redis_cache" "test2" {
  name                = "acctestRedis2-221111013031387766"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  capacity            = 1
  family              = "C"
...
resource "azurerm_api_management_redis_cache" "test" {
  name              = "acctest-Redis-Cache-221111013031387766"
