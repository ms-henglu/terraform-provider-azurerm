...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-230609091200456730"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230609091200456730"
...
resource "azurerm_public_ip" "test" {
  name                = "acctpip-230609091200456730"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-230609091200456730"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine_extension" "test" {
  name                 = "acctestExt-230609091200456730"
...
resource "azurerm_resource_group" "host" {
  name     = "acctesthostRG-df-230609091200456730"
  location = "West Europe"
}

resource "azurerm_data_factory" "host" {
  name                = "acctestdfirshh230609091200456730"
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "host" {
  name            = "acctestirshh230609091200456730"
  data_factory_id = azurerm_data_factory.host.id
}

resource "azurerm_resource_group" "target" {
  name     = "acctesttargetRG-230609091200456730"
  location = "West Europe"
}
...
resource "azurerm_data_factory" "target" {
  name                = "acctestdfirsht230609091200456730"
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "target" {
  name            = "acctestirsht230609091200456730"
