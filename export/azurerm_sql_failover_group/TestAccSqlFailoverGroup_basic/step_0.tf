...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221124182353842332"
  location = "West Europe"
}

resource "azurerm_sql_server" "test_primary" {
  name                         = "acctestmssql221124182353842332-primary"
...
resource "azurerm_sql_server" "test_secondary" {
  name                         = "acctestmssql221124182353842332-secondary"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = "West US 2"
  version                      = "12.0"
  administrator_login          = "mradministrator"
...
resource "azurerm_sql_database" "test" {
  name                             = "acctestdb221124182353842332"
  resource_group_name              = azurerm_resource_group.test.name
...
resource "azurerm_sql_failover_group" "test" {
  name                = "acctestsfg221124182353842332"
  resource_group_name = azurerm_resource_group.test.name
  server_name         = azurerm_sql_server.test_primary.name
