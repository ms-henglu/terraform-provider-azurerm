...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-la-221019054524073096"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-221019054524073096"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctest-EHN-221019054524073096"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_eventhub" "test" {
  name                = "acctest-EH-221019054524073096"
...
resource "azurerm_log_analytics_data_export_rule" "test" {
  name                    = "acctest-DER-221019054524073096"
  resource_group_name     = azurerm_resource_group.test.name
