...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-Watcher-231218072256408846"
  location = "West Europe"
}

resource "azurerm_network_watcher" "test" {
  name                = "acctest-Watcher-231218072256408846"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-Vnet-231218072256408846"
...
resource "azurerm_network_interface" "src" {
  name                = "acctest-SrcNIC-231218072256408846"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "src" {
  name                  = "acctest-SrcVM-231218072256408846"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_os_disk {
    name              = "osdisk-src231218072256408846"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "hostname231218072256408846"
...
resource "azurerm_network_interface" "dest" {
  name                = "acctest-DestNic-231218072256408846"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "dest" {
  name                  = "acctest-DestVM-231218072256408846"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_os_disk {
    name              = "osdisk-dest231218072256408846"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
  }
...
resource "azurerm_network_connection_monitor" "test" {
  name               = "acctest-CM-231218072256408846"
