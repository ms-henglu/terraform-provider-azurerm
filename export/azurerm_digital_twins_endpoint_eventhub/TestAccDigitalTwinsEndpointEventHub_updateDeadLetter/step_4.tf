...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-dtwin-230512003909562383"
  location = "West Europe"
}


resource "azurerm_digital_twins_instance" "test" {
  name                = "acctest-DT-230512003909562383"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-230512003909562383"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-230512003909562383"
...
resource "azurerm_eventhub_authorization_rule" "test" {
  name                = "acctest-r230512003909562383"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_digital_twins_endpoint_eventhub" "test" {
  name                                 = "acctest-EH-230512003909562383"
