...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ml-220627132046285309"
...
resource "azurerm_application_insights" "test" {
  name                = "acctestai-220627132046285309"
...
resource "azurerm_machine_learning_workspace" "test" {
  name                    = "acctest-MLW2206271320462853"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet220627132046285309"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet220627132046285309"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks220627132046285309"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = join("", ["acctestaks", azurerm_resource_group.test.location])
  node_resource_group = "acctestRGAKS-220627132046285309"
