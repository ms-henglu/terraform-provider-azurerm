...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cosmos-240112224223178979"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test1" {
  name                = "acctest-ca-240112224223178979"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-240112224223178979"
...
resource "azurerm_cosmosdb_mongo_collection" "test" {
  name                = "acctest-mongodb-coll-240112224223178979"
...
resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca2-240112224223178979"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  offer_type          = "Standard"
  kind                = "MongoDB"
