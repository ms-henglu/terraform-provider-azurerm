...
resource "azurerm_resource_group" "test" {
  name     = "acctest-rg-231218072152342860"
  location = "West Europe"
}

resource "azurerm_monitor_action_group" "test" {
  name                = "acctestActionGroup-231218072152342860"
  resource_group_name = azurerm_resource_group.test.name
  short_name          = "acctestag"
}

resource "azurerm_monitor_workspace" "test" {
  name                = "acctest-amw-231218072152342860"
  resource_group_name = azurerm_resource_group.test.name
  location            = "West Europe"
}
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks231218072152342860"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = "acctestaks231218072152342860"
...
resource "azurerm_kubernetes_cluster" "test2" {
  name                = "acctestaks2231218072152342860"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = "acctestaks2231218072152342860"
...
resource "azurerm_monitor_action_group" "test2" {
  name                = "acctestActionGroup2-231218072152342860"
  resource_group_name = azurerm_resource_group.test.name
  short_name          = "acctestag2"
}

resource "azurerm_monitor_alert_prometheus_rule_group" "test" {
  name                = "acctest-amprg-231218072152342860"
  resource_group_name = azurerm_resource_group.test.name
  location            = "West Europe"
