...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-PEASGAsso-230428050247002757"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvnet-230428050247002757"
...
resource "azurerm_subnet" "service" {
  name                 = "acctestsnetservice-230428050247002757"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "endpoint" {
  name                 = "acctestsnetendpoint-230428050247002757"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip-230428050247002757"
  sku                 = "Standard"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_lb" "test" {
  name                = "acctestlb-230428050247002757"
  sku                 = "Standard"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_private_link_service" "test" {
  name                           = "acctestPLS-230428050247002757"
  location                       = azurerm_resource_group.test.location
  resource_group_name            = azurerm_resource_group.test.name
  ...
  nat_ip_configuration {
    name      = "primaryIpConfiguration-230428050247002757"
    primary   = true
...
resource "azurerm_private_endpoint" "test" {
  name                = "acctest-privatelink-230428050247002757"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_application_security_group" "test" {
  name                = "acctest-230428050247002757"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
