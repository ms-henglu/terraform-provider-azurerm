...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-aks-220429075237983309"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-220429075237983309"
...
resource "azurerm_nat_gateway" "test" {
  name                = "acctest-220429075237983309"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_public_ip" "test" {
  name                = "acctest-PIP-220429075237983309"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_kubernetes_cluster" "test" {
  ...
  name                = "acctestaks220429075237983309"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = "acctestaks220429075237983309"
