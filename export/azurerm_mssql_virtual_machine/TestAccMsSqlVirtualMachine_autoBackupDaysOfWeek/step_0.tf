...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-230818024434519397"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-VN-230818024434519397"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-SN-230818024434519397"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "vm" {
  name                = "acctest-PIP-230818024434519397"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "acctest-NSG-230818024434519397"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_network_interface" "test" {
  name                = "acctest-NIC-230818024434519397"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "test" {
  name                  = "acctest-VM-230818024434519397"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_os_disk {
    name              = "acctvm-230818024434519397OSDisk"
    caching           = "ReadOnly"
