...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240105061507721593"
  location = "West Europe"
}

resource "azurerm_logic_app_workflow" "test" {
  name                = "acctestlogicapp-240105061507721593"
  location            = "West Europe"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_logic_app_workflow" "test2" {
  name                = "acctestlogicapp2-240105061507721593"
  location            = "West Europe"
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_security_center_automation" "test" {
  name                = "acctestautomation-240105061507721593"
  location            = "West Europe"
  resource_group_name = azurerm_resource_group.test.name
