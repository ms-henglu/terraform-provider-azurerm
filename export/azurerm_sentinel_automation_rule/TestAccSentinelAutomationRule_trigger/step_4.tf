...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-sentinel-230825025244764143"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctest-workspace-230825025244764143"
...
resource "azurerm_api_connection" "test" {
  ...
  name                = "azuresentinel-230825025244764143"
  resource_group_name = azurerm_resource_group.test.name
  parameter_values = {
...
resource "azurerm_logic_app_workflow" "test" {
  name                = "acctestLogicApp-230825025244764143"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_sentinel_automation_rule" "test" {
  name                       = "23421fdf-4620-4fe2-8866-eb70c7a00f3c"
  log_analytics_workspace_id = azurerm_sentinel_log_analytics_workspace_onboarding.test.workspace_id
  display_name               = "acctest-SentinelAutoRule-230825025244764143-update"
  order                      = 1
  triggers_on                = "Alerts"
  action_playbook {
    logic_app_id = azurerm_logic_app_workflow.test.id
