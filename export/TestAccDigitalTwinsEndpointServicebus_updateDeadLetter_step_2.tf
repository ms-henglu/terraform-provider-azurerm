...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-dtwin-210906022229461200"
  location = "West Europe"
}


resource "azurerm_digital_twins_instance" "test" {
  name                = "acctest-DT-210906022229461200"
...
resource "azurerm_servicebus_namespace" "test" {
  name                = "acctestservicebusnamespace-210906022229461200"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_servicebus_topic" "test" {
  name                = "acctestservicebustopic-210906022229461200"
  namespace_name      = azurerm_servicebus_namespace.test.name
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_servicebus_topic_authorization_rule" "test" {
  name                = "acctest-rule-210906022229461200"
  namespace_name      = azurerm_servicebus_namespace.test.name
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_digital_twins_endpoint_servicebus" "test" {
  name                                   = "acctest-EndpointSB-210906022229461200"
