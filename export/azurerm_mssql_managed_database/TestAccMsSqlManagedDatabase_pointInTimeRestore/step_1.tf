...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG1-sql-240119025440334879"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet1-240119025440334879"
...
resource "azurerm_subnet" "test" {
  name                 = "subnet1-240119025440334879"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "mi-security-group1-240119025440334879"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_route_table" "test" {
  name                          = "routetable1-240119025440334879"
  location                      = azurerm_resource_group.test.location
  resource_group_name           = azurerm_resource_group.test.name
...
resource "azurerm_mssql_managed_instance" "test" {
  name                = "acctestsqlserver240119025440334879"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_mssql_managed_database" "test" {
  ...
  name                = "acctest-240119025440334879"
}


resource "azurerm_mssql_managed_database" "pitr" {
  managed_instance_id = azurerm_mssql_managed_instance.test.id
  name                = "acctest-240119025440334879-pitr"

  point_in_time_restore {
    restore_point_in_time = "2024-01-19T02:54:40Z"
