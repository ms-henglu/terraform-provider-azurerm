...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-210910021313254572"
  location = "West Europe"
}

resource "azurerm_data_factory" "test" {
  name                = "acctestdf210910021313254572"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_data_factory_pipeline" "test" {
  name                = "acctestdfp210910021313254572"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_trigger_tumbling_window" "dependency" {
  name            = "acctestdft2210910021313254572"
...
resource "azurerm_data_factory_trigger_tumbling_window" "test" {
  name            = "acctestdft210910021313254572"
  data_factory_id = azurerm_data_factory.test.id
  start_time      = "2022-09-21T00:00:00Z"
