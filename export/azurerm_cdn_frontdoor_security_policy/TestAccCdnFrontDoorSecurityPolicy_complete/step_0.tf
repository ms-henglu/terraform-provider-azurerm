...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-221111013200901634"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF221111013200901634"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone221111013200901634.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-221111013200901634"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-221111013200901634"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol221111013200901634"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
