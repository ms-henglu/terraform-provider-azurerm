...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-eg-231020041053043656"
  location = "West Europe"
}

resource "azurerm_storage_account" "test" {
  name                     = "acctestacc7e2n6"
...
resource "azurerm_storage_queue" "test" {
  name                 = "mysamplequeue-231020041053043656"
  storage_account_name = azurerm_storage_account.test.name
}

resource "azurerm_eventgrid_system_topic" "test" {
  name                   = "acctesteg-231020041053043656"
  location               = "Global"
  resource_group_name    = azurerm_resource_group.test.name
...
resource "azurerm_eventgrid_system_topic_event_subscription" "test1" {
  name                = "acctesteg-231020041053043656-1"
...
resource "azurerm_eventgrid_system_topic_event_subscription" "test2" {
  name                = "acctesteg-231020041053043656-2"
  system_topic        = azurerm_eventgrid_system_topic.test.name
  resource_group_name = azurerm_resource_group.test.name
