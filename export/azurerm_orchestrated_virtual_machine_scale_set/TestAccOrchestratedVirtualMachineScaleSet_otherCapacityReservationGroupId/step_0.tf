...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-OVMSS-221021030948770483"
  location = "West Europe"
}

resource "azurerm_capacity_reservation_group" "test" {
  name                = "acctest-ccrg-221021030948770483"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_capacity_reservation" "test" {
  name                          = "acctest-ccr-221021030948770483"
...
resource "azurerm_public_ip" "test" {
  name                = "acctpip-221021030948770483"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctvn-221021030948770483"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-221021030948770483"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_nat_gateway" "test" {
  name                    = "acctng-221021030948770483"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
  sku_name                = "Standard"
...
resource "azurerm_orchestrated_virtual_machine_scale_set" "test" {
  name                = "acctestOVMSS-221021030948770483"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
