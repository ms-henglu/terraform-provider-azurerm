...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cosmos-220422011722257055"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test1" {
  name                = "acctest-ca-220422011722257055"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-220422011722257055"
...
resource "azurerm_cosmosdb_mongo_collection" "test" {
  name                = "acctest-mongodb-coll-220422011722257055"
...
resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca2-220422011722257055"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  offer_type          = "Standard"
  kind                = "MongoDB"
