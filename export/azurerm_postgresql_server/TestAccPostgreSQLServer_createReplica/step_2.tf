...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-psql-221021031617911052"
  location = "West Europe"
}

resource "azurerm_postgresql_server" "test" {
  name                = "acctest-psql-server-221021031617911052"
...
resource "azurerm_resource_group" "replica" {
  name     = "acctestRG-psql-221021031617911052-replica"
  location = "West US 2"
}

resource "azurerm_postgresql_server" "replica" {
  name                = "acctest-psql-server-221021031617911052-replica"
  location            = "West US 2"
