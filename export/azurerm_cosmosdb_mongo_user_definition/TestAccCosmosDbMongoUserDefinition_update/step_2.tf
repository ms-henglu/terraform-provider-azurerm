...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mongouserdef-230728025410672142"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca-230728025410672142"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-230728025410672142"
...
resource "azurerm_cosmosdb_mongo_role_definition" "test" {
  ...
  role_name                = "acctestmongoroledef230728025410672142"
}

resource "azurerm_cosmosdb_mongo_role_definition" "test2" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  role_name                = "acctestmongoroledef2230728025410672142"
}

resource "azurerm_cosmosdb_mongo_user_definition" "test" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  username                 = "myUserName-230728025410672142"
  password                 = "myPassword2-230728025410672142"
