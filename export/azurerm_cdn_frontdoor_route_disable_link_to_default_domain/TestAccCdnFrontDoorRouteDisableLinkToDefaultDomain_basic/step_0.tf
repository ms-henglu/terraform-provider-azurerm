...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-221221204025506550"
  location = "West Europe"
}

resource "azurerm_dns_zone" "test" {
  name                = "acctest-dns-zone221221204025506550.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "acctest-profile-221221204025506550"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "accTest-origin-group-221221204025506550"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "acctest-origin-221221204025506550"
...
resource "azurerm_cdn_frontdoor_endpoint" "test" {
  name                     = "acctest-endpoint-221221204025506550"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
}

resource "azurerm_cdn_frontdoor_route" "test" {
  name                          = "acctest-route-221221204025506550"
...
resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "acctest-custom-domain-221221204025506550"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
  dns_zone_id              = azurerm_dns_zone.test.id
  host_name                = join(".", ["ste18q0k", azurerm_dns_zone.test.name])
