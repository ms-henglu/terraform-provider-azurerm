...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-vhub-211217075627691397"
  location = "West Europe"
}

resource "azurerm_virtual_wan" "test" {
  name                = "acctest-vwan-211217075627691397"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_virtual_hub" "test" {
  name                = "acctest-VHUB-211217075627691397"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_vpn_gateway" "test" {
  name                = "acctest-VPNG-211217075627691397"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_hub_security_partner_provider" "test" {
  name                   = "acctest-SPP-211217075627691397"
  resource_group_name    = azurerm_resource_group.test.name
  location               = azurerm_resource_group.test.location
