...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-wps-220715015024459227"
  location = "West Europe"
}

resource "azurerm_web_pubsub" "test" {
  name                = "acctest-webpubsub-220715015024459227"
...
resource "azurerm_user_assigned_identity" "test1" {
  name                = "acctest-uai1-220715015024459227"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
resource "azurerm_user_assigned_identity" "test2" {
  name                = "acctest-uai2-220715015024459227"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
resource "azurerm_web_pubsub_hub" "test" {
  name          = "acctestwpsh220715015024459227"
