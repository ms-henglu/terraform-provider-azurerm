...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-211203013707564412"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-211203013707564412"
...
resource "azurerm_public_ip" "test" {
  name                = "acctpip-211203013707564412"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-211203013707564412"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine_extension" "test" {
  name                 = "acctestExt-211203013707564412"
...
resource "azurerm_resource_group" "host" {
  name     = "acctesthostRG-df-211203013707564412"
  location = "West Europe"
}

resource "azurerm_data_factory" "host" {
  name                = "acctestdfirshh211203013707564412"
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "host" {
  name                = "acctestirshh211203013707564412"
  data_factory_name   = azurerm_data_factory.host.name
  resource_group_name = azurerm_resource_group.host.name
}

resource "azurerm_resource_group" "target" {
  name     = "acctesttargetRG-211203013707564412"
  location = "West Europe"
}
...
resource "azurerm_data_factory" "target" {
  name                = "acctestdfirsht211203013707564412"
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "target" {
  name                = "acctestirsht211203013707564412"
