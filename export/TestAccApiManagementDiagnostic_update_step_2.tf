...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211210024250492256"
  location = "West Europe"
}

resource "azurerm_application_insights" "test" {
  name                = "acctestappinsights-211210024250492256"
...
resource "azurerm_api_management" "test" {
  name                = "acctestAM-211210024250492256"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  publisher_name      = "pub1"
...
resource "azurerm_api_management_logger" "test" {
  name                = "acctestapimnglogger-211210024250492256"
...
resource "azurerm_application_insights" "test2" {
  name                = "acctestappinsightsUpdate-211210024250492256"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  application_type    = "web"
}
...
resource "azurerm_api_management_logger" "test2" {
  name                = "acctestapimngloggerUpdate-211210024250492256"
  api_management_name = azurerm_api_management.test.name
  resource_group_name = azurerm_resource_group.test.name
