...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-traffic-220513180921719500"
  location = "West Europe"
}

resource "azurerm_traffic_manager_profile" "test" {
  name                   = "acctest-TMP-220513180921719500"
  resource_group_name    = azurerm_resource_group.test.name
  traffic_routing_method = "Weighted"

  dns_config {
    relative_name = "acctest-tmp-220513180921719500"
    ttl           = 30
  }
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpublicip-220513180921719500"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  domain_name_label   = "acctestpublicip-220513180921719500"
}

resource "azurerm_traffic_manager_azure_endpoint" "test" {
  name               = "acctestend-azure220513180921719500"
