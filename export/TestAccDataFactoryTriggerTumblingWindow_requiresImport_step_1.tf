...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-211210024512760988"
  location = "West Europe"
}

resource "azurerm_data_factory" "test" {
  name                = "acctestdf211210024512760988"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_data_factory_pipeline" "test" {
  name                = "acctestdfp211210024512760988"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_trigger_tumbling_window" "dependency" {
  name            = "acctestdft2211210024512760988"
...
resource "azurerm_data_factory_trigger_tumbling_window" "test" {
  name            = "acctestdft211210024512760988"
  data_factory_id = azurerm_data_factory.test.id
  frequency       = "Minute"
  interval        = 15
  start_time      = "2022-09-21T00:00:00Z"
