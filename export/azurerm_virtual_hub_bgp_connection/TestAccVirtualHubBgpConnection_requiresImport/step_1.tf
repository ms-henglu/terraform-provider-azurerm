...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-VHub-230804030421802197"
  location = "West Europe"
}

resource "azurerm_virtual_hub" "test" {
  name                = "acctest-VHub-230804030421802197"
...
resource "azurerm_public_ip" "test" {
  name                = "acctest-PIP-230804030421802197"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctest-VNet-230804030421802197"
...
resource "azurerm_virtual_hub_ip" "test" {
  name                         = "acctest-VHub-IP-230804030421802197"
...
resource "azurerm_virtual_hub_bgp_connection" "test" {
  name           = "acctest-VHub-BgpConnection-230804030421802197"
  virtual_hub_id = azurerm_virtual_hub.test.id
  peer_asn       = 65514
