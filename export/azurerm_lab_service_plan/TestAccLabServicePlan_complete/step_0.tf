...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-lslp-230407023555352440"
  location = "West Europe"
}


resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig230407023555352440"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-230407023555352440"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-sn-230407023555352440"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_lab_service_plan" "test" {
  name                      = "acctest-lslp-230407023555352440"
  resource_group_name       = azurerm_resource_group.test.name
  location                  = azurerm_resource_group.test.location
