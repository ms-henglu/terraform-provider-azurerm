...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220811053940378219"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctestehn-220811053940378219"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteh-220811053940378219"
...
resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-220811053940378219"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_eventhub_namespace" "updated" {
  name                = "acctestehn2-220811053940378219"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  capacity            = 1
...
resource "azurerm_eventhub" "updated" {
  name                = "acctesteh2-220811053940378219"
...
resource "azurerm_stream_analytics_output_eventhub" "test" {
  name                      = "acctestinput-220811053940378219"
