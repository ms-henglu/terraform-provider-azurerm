...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-220128082443337730"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet220128082443337730"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip220128082443337730"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall220128082443337730"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall_nat_rule_collection" "test" {
  name                = "acctestnrc-220128082443337730"
