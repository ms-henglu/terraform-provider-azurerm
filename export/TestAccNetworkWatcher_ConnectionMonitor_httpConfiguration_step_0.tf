...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-Watcher-220128082747432209"
  location = "West Europe"
}

resource "azurerm_network_watcher" "test" {
  name                = "acctest-Watcher-220128082747432209"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-Vnet-220128082747432209"
...
resource "azurerm_network_interface" "src" {
  name                = "acctest-SrcNIC-220128082747432209"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "src" {
  name                  = "acctest-SrcVM-220128082747432209"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_os_disk {
    name              = "osdisk-src220128082747432209"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "hostname220128082747432209"
...
resource "azurerm_network_interface" "dest" {
  name                = "acctest-DestNic-220128082747432209"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "dest" {
  name                  = "acctest-DestVM-220128082747432209"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_os_disk {
    name              = "osdisk-dest220128082747432209"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
  }
...
resource "azurerm_network_connection_monitor" "test" {
  name               = "acctest-CM-220128082747432209"
