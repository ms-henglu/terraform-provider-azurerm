...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-iothub-230203063513650945"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-230203063513650945"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-230203063513650945"
...
resource "azurerm_eventhub_authorization_rule" "test" {
  name                = "acctest-230203063513650945"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_user_assigned_identity" "test" {
  name                = "acctestuai-230203063513650945"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
...
resource "azurerm_iothub" "test" {
  name                = "acctestIoTHub-230203063513650945"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
