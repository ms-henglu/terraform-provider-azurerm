...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240112224137875552"
  location = "westeurope"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-240112224137875552"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-240112224137875552"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard_F2s_v2"
  instances           = 1
  ...
  data_disk {
    ...
    disk_size_gb                   = 10
    lun                            = 10
    ultra_ssd_disk_iops_read_write = 3001
    ultra_ssd_disk_mbps_read_write = 126
  }
