...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240119025423978812"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-240119025423978812"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                            = "acctvmss-240119025423978812"
  location                        = azurerm_resource_group.test.location
  resource_group_name             = azurerm_resource_group.test.name
  upgrade_mode                    = "Manual"
  sku                             = "Standard_F2"
  ...
  computer_name_prefix            = "testvm-240119025423978812"
  ...
  network_interface {
    name    = "TestNetworkProfile-240119025423978812"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-240119025423978812"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-240119025423978812@example.com", "acctest2-240119025423978812@example.com"]
    }
  }
}
