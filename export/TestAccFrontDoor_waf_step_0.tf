...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-frontdoor-211126031226255907"
...
resource "azurerm_frontdoor_firewall_policy" "test" {
  name                = "acctestwafp211126031226255907"
  resource_group_name = azurerm_resource_group.test.name
  mode                = "Prevention"
}

resource "azurerm_frontdoor" "test" {
  name                                         = "acctest-FD-211126031226255907"
  resource_group_name                          = azurerm_resource_group.test.name
  ...
  frontend_endpoint {
    ...
    host_name                               = "acctest-FD-211126031226255907.azurefd.net"
