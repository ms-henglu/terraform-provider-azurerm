...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211001020443397947"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-211001020443397947"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-211001020443397947"
...
resource "azurerm_api_management" "test" {
  name                = "acctestAM-211001020443397947"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  publisher_name      = "pub1"
...
resource "azurerm_api_management_logger" "test" {
  name                = "acctestapimnglogger-211001020443397947"
