...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cosmos-220610092509584945"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test1" {
  name                = "acctest-ca-220610092509584945"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-220610092509584945"
...
resource "azurerm_cosmosdb_mongo_collection" "test" {
  name                = "acctest-mongodb-coll-220610092509584945"
...
resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca2-220610092509584945"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  offer_type          = "Standard"
  kind                = "MongoDB"
