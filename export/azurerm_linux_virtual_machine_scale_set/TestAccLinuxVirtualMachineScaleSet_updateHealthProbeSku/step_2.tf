...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230519074408666706"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230519074408666706"
...
resource "azurerm_public_ip" "test" {
  ...
  name                    = "acctestpip${count.index}-230519074408666706"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_lb" "test" {
  ...
  name                = "acctestlb${count.index}-230519074408666706"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = count.index == 0 ? "Basic" : "Standard"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-230519074408666706"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard_F2"
  instances           = 1
