...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-221111020028257695"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-221111020028257695"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-221111020028257695"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-221111020028257695"
  ...
  schedule {
    ...
    start_time          = "2022-11-11T12:00:28Z"
    expiry_time         = "2022-11-13T04:00:28Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
