...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-OVMSS-211203161152211647"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-211203161152211647"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsn-211203161152211647"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip-211203161152211647"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_lb" "test" {
  name                = "acctestlb-211203161152211647"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_lb_backend_address_pool" "test" {
  name            = "acctestbap-211203161152211647"
  loadbalancer_id = azurerm_lb.test.id
}

resource "azurerm_orchestrated_virtual_machine_scale_set" "test" {
  name                = "acctestOVMSS-211203161152211647"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
