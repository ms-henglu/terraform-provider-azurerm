...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ca-220506015728791760"
  location = "West US 2"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-220506015728791760"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-220506015728791760"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_cosmosdb_cassandra_cluster" "test" {
  name                           = "acctca-mi-cluster-220506015728791760"
  resource_group_name            = azurerm_resource_group.test.name
  location                       = azurerm_resource_group.test.location
...
resource "azurerm_cosmosdb_cassandra_datacenter" "test" {
  name                           = "acctca-mi-dc-220506015728791760"
