...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-StorageSync-220627135032105929"
  location = "West Europe"
}

resource "azurerm_storage_sync" "test" {
  name                = "acctest-StorageSync-220627135032105929"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_storage_sync_group" "test" {
  name            = "acctest-StorageSyncGroup-220627135032105929"
...
resource "azurerm_storage_share" "test" {
  name                 = "acctest-share-220627135032105929"
...
resource "azurerm_storage_sync_cloud_endpoint" "test" {
  name                  = "acctest-CEP-220627135032105929"
