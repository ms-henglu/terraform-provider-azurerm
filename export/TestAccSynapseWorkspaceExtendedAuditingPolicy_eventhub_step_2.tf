...
resource "azurerm_resource_group" "test" {
  name     = "acctestsw211001054249960533"
...
resource "azurerm_storage_data_lake_gen2_filesystem" "test" {
  name               = "acctest-211001054249960533"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctest-LAW-211001054249960533"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctest-EHN-211001054249960533"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_eventhub" "test" {
  name                = "acctest-EH-211001054249960533"
...
resource "azurerm_monitor_diagnostic_setting" "test" {
  name                           = "acctest-DS-211001054249960533"
