...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-230728025126448155"
  location = "West Europe"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-230728025126448155"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-230728025126448155"
  location            = "west europe"
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-230728025126448155"
  ...
  schedule {
    ...
    expiry_time        = "2023-07-28T04:51:00Z"
    is_enabled         = true
    interval           = 2
    frequency          = "Hour"
    time_zone          = "Etc/UTC"
