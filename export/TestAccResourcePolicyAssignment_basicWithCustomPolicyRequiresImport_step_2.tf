...
resource "azurerm_resource_group" "test" {
  name     = "acctest220407231308288523"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-220407231308288523"
...
resource "azurerm_policy_definition" "test" {
  name         = "acctestpol-220407231308288523"
  policy_type  = "Custom"
  mode         = "All"
  display_name = "acctestpol-220407231308288523"
...
resource "azurerm_resource_policy_assignment" "test" {
  name                 = "acctestpa-220407231308288523"
