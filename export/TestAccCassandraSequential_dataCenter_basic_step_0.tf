...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ca-220722051758967165"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-220722051758967165"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-220722051758967165"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_cosmosdb_cassandra_cluster" "test" {
  name                           = "acctca-mi-cluster-220722051758967165"
  resource_group_name            = azurerm_resource_group.test.name
  location                       = azurerm_resource_group.test.location
...
resource "azurerm_cosmosdb_cassandra_datacenter" "test" {
  name                           = "acctca-mi-dc-220722051758967165"
