...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-220225034436032248"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet220225034436032248"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip220225034436032248"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall220225034436032248"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall_nat_rule_collection" "test" {
  name                = "acctestnrc-220225034436032248"
