...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-monitor-210825045006674648"
  location = "West Europe"
}

resource "azurerm_application_insights" "test" {
  name                = "acctestAppInsights-210825045006674648"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestWorkspace-210825045006674648"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
  sku                 = "PerGB2018"
...
resource "azurerm_monitor_action_group" "test" {
  name                = "acctestActionGroup-210825045006674648"
  resource_group_name = "${azurerm_resource_group.test.name}"
  short_name          = "acctestag"
}

resource "azurerm_monitor_scheduled_query_rules_alert" "test" {
  name                = "acctestsqr-210825045006674648"
  resource_group_name = "${azurerm_resource_group.test.name}"
  location            = "${azurerm_resource_group.test.location}"
