...
locals {
  ...
  vm_name           = "acctestsourcevm-230512010404675313"
  admin_username    = "testadmin230512010404675313"
  admin_password    = "Password1234!230512010404675313"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230512010404675313"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230512010404675313"
...
resource "azurerm_public_ip" "test" {
  name                = "acctpip-230512010404675313"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_network_interface" "public" {
  name                = "acctnicsource-230512010404675313"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-230512010404675313"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig230512010404675313"
  resource_group_name = "${azurerm_resource_group.test.name}"
  location            = "${azurerm_resource_group.test.location}"
}
...
resource "azurerm_linux_virtual_machine" "source" {
  name                            = "acctestsourceVM-230512010404675313"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_F2"
