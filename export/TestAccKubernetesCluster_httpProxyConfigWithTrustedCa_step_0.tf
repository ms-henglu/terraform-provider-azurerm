...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220114014030994439"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet220114014030994439"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet220114014030994439"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "test-nic220114014030994439"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                            = "vm-test-proxy220114014030994439"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_B1s"
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks220114014030994439"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = "acctestaks220114014030994439"
  kubernetes_version  = "1.21.2"

  linux_profile {
    admin_username = "acctestuser220114014030994439"
    ssh_key {
