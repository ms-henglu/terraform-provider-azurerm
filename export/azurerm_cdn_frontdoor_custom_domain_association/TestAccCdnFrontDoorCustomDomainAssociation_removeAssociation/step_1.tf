...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-221216013159895825"
...
resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "acctest-profile-221216013159895825"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "acctest-origin-group-221216013159895825"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "acctest-origin-221216013159895825"
  cdn_frontdoor_origin_group_id = azurerm_cdn_frontdoor_origin_group.test.id
  enabled                       = true

  certificate_name_check_enabled = false
  host_name                      = join(".", ["eqo0ij9kzx", azurerm_dns_zone.test.name])
  priority                       = 1
  weight                         = 1
}

resource "azurerm_cdn_frontdoor_endpoint" "test" {
  name                     = "acctest-endpoint-221216013159895825"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
  enabled                  = true
}

resource "azurerm_cdn_frontdoor_route" "contoso" {
  name                          = "acctest-contoso-221216013159895825"
