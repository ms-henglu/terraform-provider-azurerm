...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-postgresql-220923012214946057"
  location = "West Europe"
}


resource "azurerm_virtual_network" "test" {
  name                = "acctest-vn-220923012214946057"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-sn-220923012214946057"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_private_dns_zone" "test" {
  name                = "acc220923012214946057.postgres.database.azure.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_private_dns_zone_virtual_network_link" "test" {
  name                  = "acctestVnetZone220923012214946057.com"
...
resource "azurerm_postgresql_flexible_server" "test" {
  name                   = "acctest-fs-220923012214946057"
  resource_group_name    = azurerm_resource_group.test.name
  location               = azurerm_resource_group.test.location
