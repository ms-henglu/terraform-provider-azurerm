...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-psql-210825041152474834"
  location = "West Europe"
}

resource "azurerm_postgresql_server" "test" {
  name                = "acctest-psql-server-210825041152474834"
...
resource "azurerm_resource_group" "replica1" {
  name     = "acctestRG-psql-210825041152474834-replica1"
  location = "West US 2"
}

resource "azurerm_postgresql_server" "replica1" {
  name                = "acctest-psql-server-210825041152474834-replica1"
  location            = "West US 2"
...
resource "azurerm_postgresql_server" "replica2" {
  name                = "acctest-psql-server-210825041152474834-replica2"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
