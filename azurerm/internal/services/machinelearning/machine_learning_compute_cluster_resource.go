	...
	return &pluginsdk.Resource{
		...
		Schema: map[string]*pluginsdk.Schema{
			...
			"local_auth_enabled": {
				...
				Optional: true,
				ForceNew: true,
				Computed: true, // `local_auth_enabled` sets to true `true` by default even if unspecified
			},
...
func resourceComputeClusterCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	log.Printf("[INFO] computeClusterAmlComputeProperties.RemoteLoginPortPublicAccess: %v", computeClusterAmlComputeProperties.RemoteLoginPortPublicAccess)
	log.Printf("[INFO] d.Get(\"remote_login_port_public_access_enabled\").(bool): %v", d.Get("remote_login_port_public_access_enabled").(bool))
	...
	j, _ := json.Marshal(computeClusterAmlComputeProperties)
	log.Printf("[INFO] computeClusterAmlComputeProperties: %v", string(j))
	...
	j, _ = json.Marshal(computeClusterParameters)
