...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-la-221104005600711841"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-221104005600711841"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctest-EHN-221104005600711841"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_eventhub" "test" {
  name                = "acctest-EH-221104005600711841"
...
resource "azurerm_log_analytics_data_export_rule" "test" {
  name                    = "acctest-DER-221104005600711841"
  resource_group_name     = azurerm_resource_group.test.name
