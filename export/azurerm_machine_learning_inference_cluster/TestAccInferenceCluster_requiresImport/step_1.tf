...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ml-240311032447398507"
...
resource "azurerm_application_insights" "test" {
  name                = "acctestai-240311032447398507"
...
resource "azurerm_machine_learning_workspace" "test" {
  name                    = "acctest-MLW2403110324473985"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet240311032447398507"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet240311032447398507"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks240311032447398507"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = join("", ["acctestaks", azurerm_resource_group.test.location])
  node_resource_group = "acctestRGAKS-240311032447398507"
