...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-erconnection-220610022923576183"
  location = "West Europe"
}

resource "azurerm_express_route_port" "test" {
  name                = "acctest-erp-220610022923576183"
...
resource "azurerm_express_route_circuit" "test" {
  name                  = "acctest-erc-220610022923576183"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
...
resource "azurerm_virtual_wan" "test" {
  name                = "acctest-vwan-220610022923576183"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_virtual_hub" "test" {
  name                = "acctest-vhub-220610022923576183"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_express_route_gateway" "test" {
  name                = "acctest-ergw-220610022923576183"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_virtual_hub_route_table" "test" {
  name           = "acctest-vhubrt-220610022923576183"
  virtual_hub_id = azurerm_virtual_hub.test.id
}


resource "azurerm_express_route_connection" "test" {
  name                             = "acctest-ExpressRouteConnection-220610022923576183"
