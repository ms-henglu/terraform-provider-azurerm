...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-databricks-231016033729527456"
  location = "West US 2"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-231016033729527456"
...
resource "azurerm_subnet" "public" {
  name                 = "acctest-sn-public-231016033729527456"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "private" {
  name                 = "acctest-sn-private-231016033729527456"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_subnet" "privatelink" {
  name                 = "acctest-snpl-231016033729527456"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_network_security_group" "nsg" {
  name                = "acctest-nsg-231016033729527456"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_databricks_workspace" "test" {
  ...
  name                        = "acctestDBW-231016033729527456"
  resource_group_name         = azurerm_resource_group.test.name
  location                    = azurerm_resource_group.test.location
  sku                         = "premium"
  managed_resource_group_name = "acctestRG-DBW-231016033729527456-managed"
...
resource "azurerm_private_endpoint" "databricks" {
  ...
  name                = "acctest-endpoint-231016033729527456"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  subnet_id           = azurerm_subnet.privatelink.id

  private_service_connection {
    name                           = "acctest-psc-231016033729527456"
