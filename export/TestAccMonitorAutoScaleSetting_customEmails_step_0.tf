...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-210924011231736026"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-210924011231736026"
...
resource "azurerm_virtual_machine_scale_set" "test" {
  name                = "acctvmss-210924011231736026"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    computer_name_prefix = "testvm-210924011231736026"
  ...
  network_profile {
    name    = "TestNetworkProfile-210924011231736026"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-210924011231736026"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-210924011231736026@example.com"]
    }
  }
}
