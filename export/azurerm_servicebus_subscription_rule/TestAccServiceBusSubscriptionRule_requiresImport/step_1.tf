...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230505051240494786"
  location = "West Europe"
}

resource "azurerm_servicebus_namespace" "test" {
  name                = "acctestservicebusnamespace-230505051240494786"
...
resource "azurerm_servicebus_topic" "test" {
  name         = "acctestservicebustopic-230505051240494786"
  namespace_id = azurerm_servicebus_namespace.test.id
}

resource "azurerm_servicebus_subscription" "test" {
  name               = "acctestservicebussubscription-230505051240494786"
...
resource "azurerm_servicebus_subscription_rule" "test" {
  name            = "acctestservicebusrule-230505051240494786"
