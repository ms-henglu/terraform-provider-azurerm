...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220722035002552116"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-220722035002552116"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-220722035002552116"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_capacity_reservation_group" "test" {
  name                = "acctest-ccrg-220722035002552116"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_capacity_reservation" "test" {
  name                          = "acctest-ccr-220722035002552116"
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-220722035002552116"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  size                = "Standard_F2"
