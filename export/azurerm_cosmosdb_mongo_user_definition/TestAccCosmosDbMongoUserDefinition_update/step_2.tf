...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mongouserdef-240112034129166243"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca-240112034129166243"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-240112034129166243"
...
resource "azurerm_cosmosdb_mongo_role_definition" "test" {
  ...
  role_name                = "acctestmongoroledef240112034129166243"
}

resource "azurerm_cosmosdb_mongo_role_definition" "test2" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  role_name                = "acctestmongoroledef2240112034129166243"
}

resource "azurerm_cosmosdb_mongo_user_definition" "test" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  username                 = "myUserName-240112034129166243"
  password                 = "myPassword2-240112034129166243"
