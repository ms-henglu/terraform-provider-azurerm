...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-221202035200779903"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-221202035200779903"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-221202035200779903"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-221202035200779903"
  ...
  schedule {
    ...
    start_time          = "2022-12-02T13:52:00Z"
    expiry_time         = "2022-12-04T05:52:00Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
