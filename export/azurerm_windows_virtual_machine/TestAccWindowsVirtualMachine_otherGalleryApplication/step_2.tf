...
locals {
  vm_name = "acctestvmtj1b1"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230728025312868405"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230728025312868405"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-230728025312868405"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig230728025312868405"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_gallery_application" "test" {
  name              = "acctest-app-230728025312868405"
...
resource "azurerm_gallery_application" "test2" {
  name              = "acctest-app2-230728025312868405"
  gallery_id        = azurerm_shared_image_gallery.test.id
  location          = azurerm_shared_image_gallery.test.location
  supported_os_type = "Windows"
}
