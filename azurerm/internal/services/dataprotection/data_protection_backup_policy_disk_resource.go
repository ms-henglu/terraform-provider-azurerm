...
func resourceDataProtectionBackupPolicyDiskCreate(d *schema.ResourceData, meta interface{}) error {
	...
	taggingCriteria := expandBackupPolicyDiskTaggingCriteriaArray(d.Get("retention_rules").([]interface{}))
	policyRules := make([]dataprotection.BasicBasePolicyRule, 0)
	policyRules = append(policyRules, expandBackupPolicyDiskAzureBackupRuleArray(d.Get("backup_rules").([]interface{}), taggingCriteria)...)
	policyRules = append(policyRules, expandBackupPolicyDiskDefaultAzureRetentionRule(d.Get("default_retention_duration")))
	policyRules = append(policyRules, expandBackupPolicyDiskAzureRetentionRuleArray(d.Get("retention_rules").([]interface{}))...)
...
func resourceDataProtectionBackupPolicyDiskRead(d *schema.ResourceData, meta interface{}) error {
	...
	if resp.Properties != nil {
		if props, ok := resp.Properties.AsBackupPolicy(); ok {
			if err := d.Set("backup_rules", flattenBackupPolicyDiskBackupRuleArray(props.PolicyRules)); err != nil {
			...
			if err := d.Set("retention_rules", flattenBackupPolicyDiskRetentionRuleArray(props.PolicyRules)); err != nil {
...
func expandBackupPolicyDiskTaggingCriteriaArray(input []interface{}) *[]dataprotection.TaggingCriteria {
	...
	for _, item := range input {
		...
		results = append(results, dataprotection.TaggingCriteria{
			Criteria:        expandBackupPolicyDiskCriteriaArray(v["tagging_criteria"].([]interface{})),
			IsDefault:       utils.Bool(false),
