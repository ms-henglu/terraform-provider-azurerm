...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221222035027019401"
  location = "West Europe"
}

resource "azurerm_mssql_server" "test_primary" {
  name                         = "acctestmssql221222035027019401-primary"
...
resource "azurerm_mssql_server" "test_secondary" {
  name                         = "acctestmssql221222035027019401-secondary"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = "West US 2"
  version                      = "12.0"
  administrator_login          = "mradministrator"
...
resource "azurerm_mssql_database" "test" {
  name        = "acctestdb221222035027019401"
...
resource "azurerm_mssql_failover_group" "test" {
  name      = "acctestsfg221222035027019401"
  server_id = azurerm_mssql_server.test_primary.id
