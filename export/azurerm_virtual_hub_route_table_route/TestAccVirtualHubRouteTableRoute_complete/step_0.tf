...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-VHUB-230526085610061341"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-VNET-230526085610061341"
...
resource "azurerm_network_security_group" "test" {
  name                = "acctest-NSG-230526085610061341"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_subnet" "test" {
  name                 = "acctest-SUBNET-230526085610061341"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_virtual_wan" "test" {
  name                = "acctest-VWAN-230526085610061341"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_virtual_hub" "test" {
  name                = "acctest-VHUB-230526085610061341"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_virtual_hub_connection" "test" {
  name                      = "acctest-VHUBCONN-230526085610061341"
  virtual_hub_id            = azurerm_virtual_hub.test.id
  remote_virtual_network_id = azurerm_virtual_network.test.id
}

resource "azurerm_virtual_hub_route_table" "test" {
  name           = "acctest-RouteTable-230526085610061341"
  virtual_hub_id = azurerm_virtual_hub.test.id
  labels         = ["Label1"]
}
...
resource "azurerm_virtual_hub_route_table_route" "test" {
  ...
  name = "acctest-Route-230526085610061341"
...
resource "azurerm_virtual_hub_route_table_route" "test_2" {
  ...
  name = "acctest-Route-230526085610061341-2"
...
resource "azurerm_virtual_hub_route_table_route" "test_3" {
  ...
  name = "acctest-Route-230526085610061341-3"
