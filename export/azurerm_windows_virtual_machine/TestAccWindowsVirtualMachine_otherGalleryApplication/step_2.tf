...
locals {
  vm_name = "acctestvmsfvr6"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-231013043133478563"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-231013043133478563"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-231013043133478563"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig231013043133478563"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_gallery_application" "test" {
  name              = "acctest-app-231013043133478563"
...
resource "azurerm_gallery_application" "test2" {
  name              = "acctest-app2-231013043133478563"
  gallery_id        = azurerm_shared_image_gallery.test.id
  location          = azurerm_shared_image_gallery.test.location
  supported_os_type = "Windows"
}
