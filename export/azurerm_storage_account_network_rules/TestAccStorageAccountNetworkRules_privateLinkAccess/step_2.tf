...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-storage-221019061131671517"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet221019061131671517"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet221019061131671517"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "blob_endpoint" {
  name                 = "acctestsnetblobendpoint-221019061131671517"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_subnet" "table_endpoint" {
  name                 = "acctestsnettableendpoint-221019061131671517"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_private_endpoint" "blob" {
  name                = "acctest-privatelink-blob-221019061131671517"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  subnet_id           = azurerm_subnet.blob_endpoint.id

  private_service_connection {
    name                           = "acctest-privatelink-mssc-221019061131671517"
...
resource "azurerm_private_endpoint" "table" {
  name                = "acctest-privatelink-table-221019061131671517"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
