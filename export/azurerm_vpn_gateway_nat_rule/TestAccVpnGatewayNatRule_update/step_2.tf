...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-vpnnatrule-230227175810598239"
  location = "West Europe"
}

resource "azurerm_virtual_wan" "test" {
  name                = "acctest-vwan-230227175810598239"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_virtual_hub" "test" {
  name                = "acctest-vhub-230227175810598239"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_vpn_gateway" "test" {
  name                = "acctest-vpngateway-230227175810598239"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_vpn_gateway_nat_rule" "test" {
  name                            = "acctest-vpnnatrule-230227175810598239"
  resource_group_name             = azurerm_resource_group.test.name
