...
description: |-
  Manages a Backup Instance Disk.
---
...
resource "azurerm_data_protection_backup_instance_disk" "example" {
  ...
  policy_id                    = azurerm_data_protection_backup_policy_disk.example.id
}
```
...
* `vault_name` - (Required) The name of the Backup Vault where the Backup Instance Disk should exist. Changing this forces a new Backup Instance Disk to be created.
