...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211119050445528572"
  location = "West Europe"
}

resource "azurerm_api_management" "test" {
  name                = "acctestAM-211119050445528572"
...
resource "azurerm_api_management_openid_connect_provider" "test" {
  name                = "acctest-211119050445528572"
  api_management_name = azurerm_api_management.test.name
  resource_group_name = azurerm_resource_group.test.name
  client_id           = "00001111-2222-3333-211119050445528572"
  client_secret       = "211119050445528572-cwdavsxbacsaxZX-211119050445528572"
