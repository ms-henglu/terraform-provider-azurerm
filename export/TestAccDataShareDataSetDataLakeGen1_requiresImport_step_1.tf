...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-datashare-211210034617764187"
  location = "West Europe"
}

resource "azurerm_data_share_account" "test" {
  name                = "acctest-DSA-211210034617764187"
...
resource "azurerm_data_share" "test" {
  name       = "acctest_DS_211210034617764187"
  account_id = azurerm_data_share_account.test.id
  kind       = "CopyBased"
}

resource "azurerm_data_lake_store" "test" {
  name                = "acctestdlsinucd"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_data_share_dataset_data_lake_gen1" "test" {
  name               = "acctest-DSDL1-211210034617764187"
