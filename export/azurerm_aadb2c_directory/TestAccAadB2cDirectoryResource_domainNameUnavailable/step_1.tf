...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230203062745281156"
...
resource "azurerm_aadb2c_directory" "test" {
  ...
  display_name            = "acctest230203062745281156"
  domain_name             = "acctest230203062745281156.onmicrosoft.com"
...
resource "azurerm_resource_group" "duplicate" {
  name     = "acctestRG-duplicate-230203062745281156"
  location = "West US 2"
}
...
resource "azurerm_aadb2c_directory" "duplicate" {
  ...
  display_name            = "acctest-duplicate-230203062745281156"
