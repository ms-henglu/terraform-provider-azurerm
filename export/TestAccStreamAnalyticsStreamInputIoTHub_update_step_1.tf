...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-210825030243512372"
  location = "West Europe"
}

resource "azurerm_iothub" "test" {
  name                = "acctestiothub-210825030243512372"
...
resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-210825030243512372"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_iothub" "updated" {
  name                = "acctestiot2-210825030243512372"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_stream_analytics_stream_input_iothub" "test" {
  name                         = "acctestinput-210825030243512372"
