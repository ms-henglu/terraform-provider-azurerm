...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-211112020642548403"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet211112020642548403"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip211112020642548403"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test_mgmt" {
  name                = "acctestmgmtpip211112020642548403"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  sku                 = "Standard"
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall211112020642548403"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
