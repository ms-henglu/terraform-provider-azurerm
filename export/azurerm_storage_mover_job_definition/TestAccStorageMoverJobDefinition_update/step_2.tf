...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230721020143084299"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230721020143084299"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-230721020143084299"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip-230721020143084299"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                            = "acctestVM-230721020143084299"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_F2"
  ...
  provisioner "file" {
    content = templatefile("scripts/install_arc.sh.tftpl", {
      resource_group_name = azurerm_resource_group.test.name
      uuid                = "e1c1c65b-bccd-3cb3-3014-995e5c7e8cad"
      location            = azurerm_resource_group.test.location
...
resource "azurerm_storage_mover" "test" {
  name                = "acctest-ssm-230721020143084299"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  depends_on = [
...
resource "azurerm_storage_mover_agent" "test" {
  name                     = "acctest-sa-230721020143084299"
...
resource "azurerm_storage_mover_source_endpoint" "test" {
  name             = "acctest-smse-230721020143084299"
  storage_mover_id = azurerm_storage_mover.test.id
  host             = "192.168.0.1"
}

resource "azurerm_storage_mover_project" "test" {
  name             = "acctest-sp-230721020143084299"
  storage_mover_id = azurerm_storage_mover.test.id
}


resource "azurerm_storage_mover_agent" "test2" {
  name                     = "acctest-sa2-230721020143084299"
  storage_mover_id         = azurerm_storage_mover.test.id
  arc_virtual_machine_id   = data.azurerm_hybrid_compute_machine.test.id
  arc_virtual_machine_uuid = data.azurerm_hybrid_compute_machine.test.vm_uuid
}
...
resource "azurerm_storage_mover_job_definition" "test" {
  name                     = "acctest-sjd-230721020143084299"
