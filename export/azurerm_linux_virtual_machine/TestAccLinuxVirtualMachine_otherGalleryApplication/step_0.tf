...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221021033910543184"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-221021033910543184"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-221021033910543184"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig221021033910543184"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_gallery_application" "test" {
  name              = "acctest-app-221021033910543184"
...
resource "azurerm_gallery_application" "test2" {
  name              = "acctest-app2-221021033910543184"
  gallery_id        = azurerm_shared_image_gallery.test.id
  location          = azurerm_shared_image_gallery.test.location
  supported_os_type = "Linux"
}
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-221021033910543184"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  size                = "Standard_F2"
