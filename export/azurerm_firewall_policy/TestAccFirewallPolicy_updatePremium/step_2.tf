...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-networkfw-240315123101619313"
  location = "westeurope"
}

resource "azurerm_key_vault" "test" {
  name                            = "tlskv240315123101619313"
...
resource "azurerm_user_assigned_identity" "test" {
  name                = "acctestUAI-240315123101619313"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
data "azurerm_storage_account_sas" "test" {
  ...
  start  = "2024-03-15T12:31:01Z"
  expiry = "2024-03-16T12:31:01Z"
...
resource "azurerm_firewall_policy" "test" {
  name                     = "acctest-networkfw-Policy-240315123101619313"
  resource_group_name      = azurerm_resource_group.test.name
  location                 = azurerm_resource_group.test.location
  sku                      = "Premium"
