...
resource "azurerm_resource_group" "test1" {
  name     = "acctestRG-api1-221221203917773468"
  location = "West Europe"
}

resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-api2-221221203917773468"
  location = "West US 2"
}

resource "azurerm_resource_group" "test3" {
  name     = "acctestRG-api3-221221203917773468"
  location = "East US 2"
}

resource "azurerm_api_management" "test" {
  name                      = "acctestAM-221221203917773468"
  publisher_name            = "pub1"
  ...
  hostname_configuration {
    proxy {
      host_name                    = "acctestAM-221221203917773468.azure-api.net"
