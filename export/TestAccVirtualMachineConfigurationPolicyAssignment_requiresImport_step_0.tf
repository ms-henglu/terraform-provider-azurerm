...
locals {
  vm_name = "acctestvmg9g9a"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211029020002421526"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-211029020002421526"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-211029020002421526"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine_configuration_policy_assignment" "test" {
  name               = "acctest-gca-211029020002421526"
