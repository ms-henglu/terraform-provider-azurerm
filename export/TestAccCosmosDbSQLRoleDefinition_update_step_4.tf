...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cosmos-220905045628582283"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-cosmos-220905045628582283"
...
resource "azurerm_cosmosdb_sql_role_definition" "test" {
  role_definition_id  = "d1c1e7fd-637e-482a-bd95-651dadf06b89"
  resource_group_name = azurerm_resource_group.test.name
  account_name        = azurerm_cosmosdb_account.test.name
  name                = "acctestsqlroleum9cy"
  type                = "BuiltInRole"
