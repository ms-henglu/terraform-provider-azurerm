...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ca-230505050137099630"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-230505050137099630"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-230505050137099630"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_cosmosdb_cassandra_cluster" "test" {
  name                           = "acctca-mi-cluster-230505050137099630"
  resource_group_name            = azurerm_resource_group.test.name
  location                       = azurerm_resource_group.test.location
...
resource "azurerm_cosmosdb_cassandra_datacenter" "test" {
  name                           = "acctca-mi-dc-230505050137099630"
