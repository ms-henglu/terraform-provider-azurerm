...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240119024407460633"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestVNET-240119024407460633"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestSNET-240119024407460633"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "acctest-NSG-240119024407460633"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-240119024407460633-2"
  location = "West US 2"
}

// subnet2 from the second location
resource "azurerm_virtual_network" "test2" {
  name                = "acctestVNET2-240119024407460633"
...
resource "azurerm_subnet" "test2" {
  name                 = "acctestSNET2-240119024407460633"
  resource_group_name  = azurerm_resource_group.test2.name
...
resource "azurerm_network_security_group" "test2" {
  name                = "acctest-NSG2-240119024407460633"
  location            = azurerm_resource_group.test2.location
  resource_group_name = azurerm_resource_group.test2.name
}
...
resource "azurerm_public_ip" "test1" {
  name                = "acctest-IP1-qovku"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctest-ip1-qovku"
}

resource "azurerm_public_ip" "test2" {
  name                = "acctest-IP2-qovku"
  resource_group_name = azurerm_resource_group.test2.name
  location            = azurerm_resource_group.test2.location
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctest-ip2-qovku"
}

resource "azurerm_api_management" "test" {
  name                = "acctestAM-240119024407460633"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  publisher_name      = "pub1"
