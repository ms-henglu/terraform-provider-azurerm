...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-CAEnv-230728025336216726"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestCAEnv-230728025336216726"
...
resource "azurerm_container_app_environment" "test" {
  name                       = "acctest-CAEnv230728025336216726"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
...
resource "azurerm_container_app_environment_dapr_component" "test" {
  name                         = "acctest-dapr-230728025336216726"
