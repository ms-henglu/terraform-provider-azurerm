...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-lslp-230602030646326965"
  location = "West Europe"
}


resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig230602030646326965"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-230602030646326965"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-sn-230602030646326965"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_lab_service_plan" "test" {
  name                      = "acctest-lslp-230602030646326965"
  resource_group_name       = azurerm_resource_group.test.name
  location                  = azurerm_resource_group.test.location
