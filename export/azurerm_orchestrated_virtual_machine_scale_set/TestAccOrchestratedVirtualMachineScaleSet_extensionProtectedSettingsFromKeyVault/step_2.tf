...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-OVMSS-230810143140364886"
  location = "West Europe"
}


resource "azurerm_public_ip" "test" {
  name                = "acctpip-230810143140364886"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctvn-230810143140364886"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-230810143140364886"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_nat_gateway" "test" {
  name                    = "acctng-230810143140364886"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
  sku_name                = "Standard"
...
resource "azurerm_key_vault" "test" {
  ...
  name                   = "acctestkv${count.index}qj0hm"
  location               = azurerm_resource_group.test.location
  resource_group_name    = azurerm_resource_group.test.name
...
resource "azurerm_orchestrated_virtual_machine_scale_set" "test" {
  name                = "acctestOVMSS-230810143140364886"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    linux_configuration {
      computer_name_prefix = "testvm-230810143140364886"
