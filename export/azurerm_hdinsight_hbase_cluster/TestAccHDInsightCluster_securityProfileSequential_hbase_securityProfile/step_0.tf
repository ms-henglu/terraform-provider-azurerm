...
resource "azurerm_resource_group" "test" {
  name     = "acctestRGhdi-230512010753897683"
...
resource "azurerm_storage_container" "test" {
  name                  = "acctestsc-230512010753897683"
  storage_account_name  = azurerm_storage_account.test.name
  container_access_type = "private"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestVnet-230512010753897683"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "test" {
  name                 = "acctestSubnet-230512010753897683"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "acctestNSG-230512010753897683"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azuread_group" "test" {
  display_name     = "AAD DC Administrators 111sp"
...
resource "azuread_user" "test" {
  user_principal_name = "acctestAADDSAdminUser-111sp@${data.azuread_domains.test.domains.0.domain_name}"
  display_name        = "acctestAADDSAdminUser-111sp"
...
resource "azurerm_hdinsight_hbase_cluster" "test" {
  name                = "acctesthdihbase-230512010753897683"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  cluster_version     = "4.0"
  tier                = "Premium"
