...
locals {
  vm_name = "acctestvmxnh2p"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220225034148165432"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-220225034148165432"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-220225034148165432"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_dedicated_host_group" "test" {
  name                        = "acctestDHG-220225034148165432"
  resource_group_name         = azurerm_resource_group.test.name
  location                    = azurerm_resource_group.test.location
...
resource "azurerm_dedicated_host" "test" {
  name                    = "acctestDH-220225034148165432"
...
resource "azurerm_dedicated_host_group" "second" {
  name                        = "acctestDHG2-220225034148165432"
  resource_group_name         = azurerm_resource_group.test.name
  location                    = azurerm_resource_group.test.location
  platform_fault_domain_count = 2
  automatic_placement_enabled = true
...
resource "azurerm_dedicated_host" "second" {
  name                    = "acctestDH2-220225034148165432"
