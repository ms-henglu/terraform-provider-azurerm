...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-postgresql-221104005756485588"
  location = "West Europe"
}


resource "azurerm_virtual_network" "test" {
  name                = "acctest-vn-221104005756485588"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-sn-221104005756485588"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_private_dns_zone" "test" {
  name                = "acc221104005756485588.postgres.database.azure.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_private_dns_zone_virtual_network_link" "test" {
  name                  = "acctestVnetZone221104005756485588.com"
...
resource "azurerm_postgresql_flexible_server" "test" {
  name                   = "acctest-fs-221104005756485588"
  resource_group_name    = azurerm_resource_group.test.name
  location               = azurerm_resource_group.test.location
