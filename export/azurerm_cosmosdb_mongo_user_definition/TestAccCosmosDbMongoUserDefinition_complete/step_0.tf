...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mongouserdef-240311031725353853"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca-240311031725353853"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-240311031725353853"
...
resource "azurerm_cosmosdb_mongo_role_definition" "test" {
  ...
  role_name                = "acctestmongoroledef240311031725353853"
}

resource "azurerm_cosmosdb_mongo_user_definition" "test" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  username                 = "myUserName-240311031725353853"
  password                 = "myPassword-240311031725353853"
