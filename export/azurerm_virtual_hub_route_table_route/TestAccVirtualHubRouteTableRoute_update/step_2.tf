...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-VHUB-221111020925371990"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-VNET-221111020925371990"
...
resource "azurerm_network_security_group" "test" {
  name                = "acctest-NSG-221111020925371990"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_subnet" "test" {
  name                 = "acctest-SUBNET-221111020925371990"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_virtual_wan" "test" {
  name                = "acctest-VWAN-221111020925371990"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_virtual_hub" "test" {
  name                = "acctest-VHUB-221111020925371990"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_virtual_hub_connection" "test" {
  name                      = "acctest-VHUBCONN-221111020925371990"
  virtual_hub_id            = azurerm_virtual_hub.test.id
  remote_virtual_network_id = azurerm_virtual_network.test.id
}

resource "azurerm_virtual_hub_route_table" "test" {
  name           = "acctest-RouteTable-221111020925371990"
  virtual_hub_id = azurerm_virtual_hub.test.id
  labels         = ["Label1"]
}
...
resource "azurerm_virtual_hub_route_table_route" "test" {
  ...
  name = "acctest-Route-221111020925371990"
...
resource "azurerm_virtual_hub_route_table_route" "test_2" {
  ...
  name = "acctest-Route-221111020925371990-2"
...
resource "azurerm_virtual_hub_route_table_route" "test_3" {
  ...
  name = "acctest-Route-221111020925371990-3"
