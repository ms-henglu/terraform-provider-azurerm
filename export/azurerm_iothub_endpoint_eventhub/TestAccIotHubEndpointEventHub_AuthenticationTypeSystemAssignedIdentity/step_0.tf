...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-iothub-221019060710008889"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-221019060710008889"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-221019060710008889"
...
resource "azurerm_eventhub_authorization_rule" "test" {
  name                = "acctest-221019060710008889"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_user_assigned_identity" "test" {
  name                = "acctestuai-221019060710008889"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
...
resource "azurerm_iothub" "test" {
  name                = "acctestIoTHub-221019060710008889"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
