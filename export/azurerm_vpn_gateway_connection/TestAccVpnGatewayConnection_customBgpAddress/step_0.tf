...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230613072348674366"
  location = "West Europe"
}

resource "azurerm_virtual_wan" "test" {
  name                = "acctestVWAN-230613072348674366"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_virtual_hub" "test" {
  name                = "acctestVHUB-230613072348674366"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_vpn_gateway" "test" {
  name                = "acctestVPNGW-230613072348674366"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_vpn_site" "test" {
  name                = "acctestVPNSite-230613072348674366"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  virtual_wan_id      = azurerm_virtual_wan.test.id
...
resource "azurerm_vpn_gateway_connection" "test" {
  name               = "acctestVPNGWConn-230613072348674366"
