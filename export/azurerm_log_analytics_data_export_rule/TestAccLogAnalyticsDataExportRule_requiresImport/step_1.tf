...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-la-221222034907021526"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-221222034907021526"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctest-EHN-221222034907021526"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_eventhub" "test" {
  name                = "acctest-EH-221222034907021526"
...
resource "azurerm_log_analytics_data_export_rule" "test" {
  name                    = "acctest-DER-221222034907021526"
  resource_group_name     = azurerm_resource_group.test.name
