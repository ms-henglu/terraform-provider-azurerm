...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-monitor-230609091652431676"
  location = "West Europe"
}

resource "azurerm_application_insights" "test" {
  name                = "acctestAppInsights-230609091652431676"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestWorkspace-230609091652431676"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_monitor_action_group" "test" {
  name                = "acctestActionGroup-230609091652431676"
  resource_group_name = azurerm_resource_group.test.name
  short_name          = "acctestag"
}

resource "azurerm_monitor_scheduled_query_rules_log" "test" {
  name                = "acctestsqr-230609091652431676"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_monitor_metric_alert" "test" {
  name                = "acctestmal-230609091652431676"
  resource_group_name = azurerm_resource_group.test.name
