...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-networkfw-211210024625263453"
  location = "West Europe"
}


resource "azurerm_log_analytics_workspace" "default" {
  name                = "acctestLAW-211210024625263453"
...
resource "azurerm_log_analytics_workspace" "regional" {
  name                = "acctestLAW-region-211210024625263453"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
  retention_in_days   = 30
...
resource "azurerm_firewall_policy" "test" {
  name                = "acctest-networkfw-Policy-211210024625263453"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  insights {
    enabled                            = true
