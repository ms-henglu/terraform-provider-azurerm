...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-datashare-210825031621063412"
  location = "West Europe"
}

resource "azurerm_data_share_account" "test" {
  name                = "acctest-DSA-210825031621063412"
...
resource "azurerm_data_share" "test" {
  name       = "acctest_DS_210825031621063412"
  account_id = azurerm_data_share_account.test.id
  kind       = "CopyBased"
}

resource "azurerm_data_lake_store" "test" {
  name                = "acctestdlsmlvgb"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_data_share_dataset_data_lake_gen1" "test" {
  name               = "acctest-DSDL1-210825031621063412"
