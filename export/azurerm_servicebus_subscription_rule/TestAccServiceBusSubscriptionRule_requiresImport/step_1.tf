...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230721020014595210"
  location = "West Europe"
}

resource "azurerm_servicebus_namespace" "test" {
  name                = "acctestservicebusnamespace-230721020014595210"
...
resource "azurerm_servicebus_topic" "test" {
  name         = "acctestservicebustopic-230721020014595210"
  namespace_id = azurerm_servicebus_namespace.test.id
}

resource "azurerm_servicebus_subscription" "test" {
  name               = "acctestservicebussubscription-230721020014595210"
...
resource "azurerm_servicebus_subscription_rule" "test" {
  name            = "acctestservicebusrule-230721020014595210"
