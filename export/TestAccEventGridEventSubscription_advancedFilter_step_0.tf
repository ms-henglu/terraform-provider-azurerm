...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-eg-220408051251197107"
  location = "West Europe"
}

resource "azurerm_storage_account" "test" {
  name                     = "acctestaccr18jq"
...
resource "azurerm_storage_queue" "test" {
  name                 = "mysamplequeue-220408051251197107"
  storage_account_name = azurerm_storage_account.test.name
}

resource "azurerm_eventgrid_event_subscription" "test1" {
  name  = "acctesteg-220408051251197107-1"
...
resource "azurerm_eventgrid_event_subscription" "test2" {
  name  = "acctesteg-220408051251197107-2"
  scope = azurerm_storage_account.test.id
