...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-221019060319769700"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-221019060319769700"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-221019060319769700"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-221019060319769700"
  ...
  schedule {
    ...
    start_time          = "2022-10-19T16:03:19Z"
    expiry_time         = "2022-10-21T08:03:19Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
