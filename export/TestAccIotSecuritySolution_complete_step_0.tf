...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-security-220715014944855930"
  location = "West Europe"
}

resource "azurerm_iothub" "test" {
  name                = "acctestIoTHub-220715014944855930"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-220715014944855930"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_iot_security_solution" "test" {
  name                       = "acctest-Iot-Security-Solution-220715014944855930"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
