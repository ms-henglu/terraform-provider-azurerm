...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-networkfw-240311032133917409"
  location = "westeurope"
}

resource "azurerm_key_vault" "test" {
  name                            = "tlskv240311032133917409"
...
resource "azurerm_user_assigned_identity" "test" {
  name                = "acctestUAI-240311032133917409"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
data "azurerm_storage_account_sas" "test" {
  ...
  start  = "2024-03-11T03:21:33Z"
  expiry = "2024-03-12T03:21:33Z"
...
resource "azurerm_firewall_policy" "test" {
  name                     = "acctest-networkfw-Policy-240311032133917409"
  resource_group_name      = azurerm_resource_group.test.name
  location                 = azurerm_resource_group.test.location
  sku                      = "Premium"
