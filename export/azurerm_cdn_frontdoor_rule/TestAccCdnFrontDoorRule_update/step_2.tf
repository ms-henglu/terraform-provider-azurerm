...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-231218071346963937"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-231218071346963937"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "accTestOriginGroup-231218071346963937"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "accTestOrigin-231218071346963937"
...
resource "azurerm_cdn_frontdoor_rule_set" "test" {
  name                     = "accTestRuleSet231218071346963937"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
}
...
resource "azurerm_cdn_frontdoor_rule" "test" {
  ...
  name                      = "accTestRule231218071346963937"
