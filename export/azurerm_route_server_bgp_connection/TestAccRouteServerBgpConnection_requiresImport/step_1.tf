...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-231218072256438169"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-231218072256438169"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip-231218072256438169"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_route_server" "test" {
  name                 = "acctestrs-231218072256438169"
  resource_group_name  = azurerm_resource_group.test.name
  location             = azurerm_resource_group.test.location
  sku                  = "Standard"
...
resource "azurerm_route_server_bgp_connection" "test" {
  name            = "acctest-rs-bgp-231218072256438169"
