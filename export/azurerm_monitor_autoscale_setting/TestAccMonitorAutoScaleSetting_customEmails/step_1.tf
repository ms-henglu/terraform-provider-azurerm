...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-231016034326105718"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-231016034326105718"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                            = "acctvmss-231016034326105718"
  location                        = azurerm_resource_group.test.location
  resource_group_name             = azurerm_resource_group.test.name
  upgrade_mode                    = "Manual"
  sku                             = "Standard_F2"
  ...
  computer_name_prefix            = "testvm-231016034326105718"
  ...
  network_interface {
    name    = "TestNetworkProfile-231016034326105718"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-231016034326105718"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-231016034326105718@example.com", "acctest2-231016034326105718@example.com"]
    }
  }
}
