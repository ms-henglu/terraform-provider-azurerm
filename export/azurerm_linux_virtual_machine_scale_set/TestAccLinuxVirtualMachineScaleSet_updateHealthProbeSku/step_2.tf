...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-231218071435528422"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-231218071435528422"
...
resource "azurerm_public_ip" "test" {
  ...
  name                    = "acctestpip${count.index}-231218071435528422"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_lb" "test" {
  ...
  name                = "acctestlb${count.index}-231218071435528422"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = count.index == 0 ? "Basic" : "Standard"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-231218071435528422"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard_F2"
  instances           = 1
  ...
  health_probe_id     = true ? azurerm_lb_probe.test[1].id : azurerm_lb_probe.test[0].id
  ...
  network_interface {
    ...
    ip_configuration {
      ...
      load_balancer_backend_address_pool_ids = true ? [azurerm_lb_backend_address_pool.test[1].id] : [azurerm_lb_backend_address_pool.test[0].id]
      load_balancer_inbound_nat_rules_ids    = true ? [azurerm_lb_nat_pool.test[1].id] : [azurerm_lb_nat_pool.test[0].id]
    }
  }
