...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-security-211105030512320265"
  location = "West Europe"
}

resource "azurerm_iothub" "test" {
  name                = "acctestIoTHub-211105030512320265"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-211105030512320265"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_iot_security_solution" "test" {
  name                       = "acctest-Iot-Security-Solution-211105030512320265"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
