...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220211043152959509"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestVNET-220211043152959509"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestSNET-220211043152959509"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "acctest-NSG-220211043152959509"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-220211043152959509-2"
  location = "West US 2"
}

// subnet2 from the second location
resource "azurerm_virtual_network" "test2" {
  name                = "acctestVNET2-220211043152959509"
...
resource "azurerm_subnet" "test2" {
  name                 = "acctestSNET2-220211043152959509"
  resource_group_name  = azurerm_resource_group.test2.name
...
resource "azurerm_network_security_group" "test2" {
  name                = "acctest-NSG2-220211043152959509"
  location            = azurerm_resource_group.test2.location
  resource_group_name = azurerm_resource_group.test2.name
}
...
resource "azurerm_public_ip" "test1" {
  name                = "acctest-IP1-ukqrv"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctest-ip1-ukqrv"
}

resource "azurerm_public_ip" "test2" {
  name                = "acctest-IP2-ukqrv"
  resource_group_name = azurerm_resource_group.test2.name
  location            = azurerm_resource_group.test2.location
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctest-ip2-ukqrv"
}

resource "azurerm_api_management" "test" {
  name                = "acctestAM-220211043152959509"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  publisher_name      = "pub1"
