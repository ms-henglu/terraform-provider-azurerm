...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220128082545318264"
...
resource "azurerm_kusto_database" "test" {
  name                = "acctestkd-220128082545318264"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-220128082545318264"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-220128082545318264"
...
resource "azurerm_eventhub_consumer_group" "test" {
  name                = "acctesteventhubcg-220128082545318264"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_eventgrid_event_subscription" "test" {
  name                  = "acctest-eg-220128082545318264"
...
resource "azurerm_kusto_eventgrid_data_connection" "test" {
  name                         = "acctestkrgdc-220128082545318264"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  cluster_name                 = azurerm_kusto_cluster.test.name
