...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230609090723311238"
...
resource "azurerm_aadb2c_directory" "test" {
  ...
  display_name            = "acctest230609090723311238"
  domain_name             = "acctest230609090723311238.onmicrosoft.com"
...
resource "azurerm_resource_group" "duplicate" {
  name     = "acctestRG-duplicate-230609090723311238"
  location = "West US 2"
}
...
resource "azurerm_aadb2c_directory" "duplicate" {
  ...
  display_name            = "acctest-duplicate-230609090723311238"
