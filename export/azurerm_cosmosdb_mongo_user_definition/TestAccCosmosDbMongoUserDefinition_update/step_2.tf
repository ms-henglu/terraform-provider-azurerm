...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mongouserdef-231020040831285961"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca-231020040831285961"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-231020040831285961"
...
resource "azurerm_cosmosdb_mongo_role_definition" "test" {
  ...
  role_name                = "acctestmongoroledef231020040831285961"
}

resource "azurerm_cosmosdb_mongo_role_definition" "test2" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  role_name                = "acctestmongoroledef2231020040831285961"
}

resource "azurerm_cosmosdb_mongo_user_definition" "test" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  username                 = "myUserName-231020040831285961"
  password                 = "myPassword2-231020040831285961"
