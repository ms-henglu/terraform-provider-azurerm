...
locals {
  vm_name = "acctestvmdyj2j"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240119021734562344"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-240119021734562344"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-240119021734562344"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig240119021734562344"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_gallery_application" "test" {
  name              = "acctest-app-240119021734562344"
...
resource "azurerm_gallery_application" "test2" {
  name              = "acctest-app2-240119021734562344"
  gallery_id        = azurerm_shared_image_gallery.test.id
  location          = azurerm_shared_image_gallery.test.location
  supported_os_type = "Windows"
}
