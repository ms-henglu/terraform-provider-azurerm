...
resource "azuread_application" "test" {
  display_name = "acctest-aro-240315123909740730"
}
...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-aro-240315123909740730"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet240315123909740730"
...
resource "azurerm_subnet" "main_subnet" {
  name                 = "main-subnet-240315123909740730"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "worker_subnet" {
  name                 = "worker-subnet-240315123909740730"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_redhat_openshift_cluster" "test" {
  name                = "acctestaro240315123909740730"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name

  cluster_profile {
    domain       = "aro-dhii2.com"
    version      = "4.13.23"
    fips_enabled = true
  }
