...
var _ resourceid.Formatter = NetworkSecurityConfigurationId{}

func TestNetworkSecurityConfigurationIDFormatter(t *testing.T) {
	actual := NewNetworkSecurityConfigurationID("12345678-1234-9876-4563-123456789012", "resGroup1", "networkManager1", "securityConfiguration1").ID()
...
func TestNetworkSecurityConfigurationID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *NetworkSecurityConfigurationId
	}{
	...
	for _, v := range testData {
		...
		actual, err := NetworkSecurityConfigurationID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
