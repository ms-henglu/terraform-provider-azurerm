...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-210825041348850350"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctestehn-210825041348850350"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteh-210825041348850350"
...
resource "azurerm_eventhub_consumer_group" "test" {
  name                = "acctesteventhubcg-210825041348850350"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-210825041348850350"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_eventhub_namespace" "updated" {
  name                = "acctestehn2-210825041348850350"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  capacity            = 1
...
resource "azurerm_eventhub" "updated" {
  name                = "acctesteh2-210825041348850350"
...
resource "azurerm_eventhub_consumer_group" "updated" {
  name                = "acctesteventhubcg2-210825041348850350"
  namespace_name      = azurerm_eventhub_namespace.updated.name
...
resource "azurerm_stream_analytics_stream_input_eventhub" "test" {
  name                         = "acctestinput-210825041348850350"
