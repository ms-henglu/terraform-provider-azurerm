...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220527024845646507"
  location = "West Europe"
}

resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-220527024845646507"
...
resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-220527024845646507"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  version                      = "12.0"
...
resource "azurerm_mssql_database" "test" {
  name      = "acctest-db-220527024845646507"
  server_id = azurerm_mssql_server.test.id
}


resource "azurerm_stream_analytics_reference_input_mssql" "test" {
  name                      = "acctestinput-220527024845646507"
