...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211203162015813483"
  location = "West Europe"
}

resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-211203162015813483"
...
resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-211203162015813483"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  version                      = "12.0"
...
resource "azurerm_mssql_database" "test" {
  name      = "acctest-db-211203162015813483"
  server_id = azurerm_mssql_server.test.id
}


resource "azurerm_stream_analytics_reference_input_mssql" "test" {
  name                      = "acctestinput-211203162015813483"
