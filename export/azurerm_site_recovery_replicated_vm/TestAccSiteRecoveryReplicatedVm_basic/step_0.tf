...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-recovery-221104005819201389-1"
  location = "West Europe"
}

resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-recovery-221104005819201389-2"
  location = "West US 2"
}

resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-vault-221104005819201389"
...
resource "azurerm_site_recovery_fabric" "test1" {
  ...
  name                = "acctest-fabric1-221104005819201389"
...
resource "azurerm_site_recovery_fabric" "test2" {
  resource_group_name = azurerm_resource_group.test2.name
  recovery_vault_name = azurerm_recovery_services_vault.test.name
  name                = "acctest-fabric2-221104005819201389"
  location            = azurerm_resource_group.test2.location
...
resource "azurerm_site_recovery_protection_container" "test1" {
  ...
  name                 = "acctest-protection-cont1-221104005819201389"
}
...
resource "azurerm_site_recovery_protection_container" "test2" {
  ...
  name                 = "acctest-protection-cont2-221104005819201389"
}

resource "azurerm_site_recovery_replication_policy" "test" {
  resource_group_name                                  = azurerm_resource_group.test2.name
  recovery_vault_name                                  = azurerm_recovery_services_vault.test.name
  name                                                 = "acctest-policy-221104005819201389"
...
resource "azurerm_site_recovery_protection_container_mapping" "test" {
  ...
  name                                      = "mapping-221104005819201389"
}

resource "azurerm_virtual_network" "test1" {
  name                = "net-221104005819201389"
...
resource "azurerm_subnet" "test2_1" {
  name                 = "acctest-snet-221104005819201389_1"
  resource_group_name  = "${azurerm_resource_group.test2.name}"
...
resource "azurerm_subnet" "test2_2" {
  name                 = "snet-221104005819201389_2"
  resource_group_name  = "${azurerm_resource_group.test2.name}"
  virtual_network_name = "${azurerm_virtual_network.test2.name}"
...
resource "azurerm_virtual_machine" "test" {
  ...
  os_profile {
    ...
    computer_name  = "vm-221104005819201389"
  }
...
resource "azurerm_public_ip" "test-source" {
  name                = "pubip221104005819201389-source"
...
resource "azurerm_public_ip" "test-recovery" {
  name                = "pubip221104005819201389-recovery"
  allocation_method   = "Static"
  location            = azurerm_resource_group.test2.location
  resource_group_name = azurerm_resource_group.test2.name
  sku                 = "Basic"
...
resource "azurerm_site_recovery_replicated_vm" "test" {
  ...
  network_interface {
    ...
    target_subnet_name            = "snet-221104005819201389_2"
