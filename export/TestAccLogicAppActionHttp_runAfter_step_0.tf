...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220722035540719233"
  location = "West Europe"
}

resource "azurerm_logic_app_workflow" "test" {
  name                = "acctestlaw-220722035540719233"
...
resource "azurerm_logic_app_action_http" "testp1" {
  name         = "action220722035540719233p1"
...
resource "azurerm_logic_app_action_http" "testp2" {
  name         = "action220722035540719233p2"
  logic_app_id = azurerm_logic_app_workflow.test.id
  method       = "GET"
  uri          = "http://example.com/hello"
}
