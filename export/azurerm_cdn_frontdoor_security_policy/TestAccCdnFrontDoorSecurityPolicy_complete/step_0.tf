...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-240311031501945591"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF240311031501945591"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone240311031501945591.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-240311031501945591"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-240311031501945591"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol240311031501945591"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
