...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211022002222069343"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-211022002222069343"
...
resource "azurerm_virtual_machine_scale_set" "test" {
  name                = "acctvmss-211022002222069343"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    computer_name_prefix = "testvm-211022002222069343"
  ...
  network_profile {
    name    = "TestNetworkProfile-211022002222069343"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-211022002222069343"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-211022002222069343@example.com"]
    }
  }
}
