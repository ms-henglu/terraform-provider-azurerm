...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-221019053910609152"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-221019053910609152"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-221019053910609152"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-221019053910609152"
  ...
  schedule {
    ...
    start_time          = "2022-10-19T15:39:10Z"
    expiry_time         = "2022-10-21T07:39:10Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
