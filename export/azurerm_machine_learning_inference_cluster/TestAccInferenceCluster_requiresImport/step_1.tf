...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ml-240112224747893439"
...
resource "azurerm_application_insights" "test" {
  name                = "acctestai-240112224747893439"
...
resource "azurerm_machine_learning_workspace" "test" {
  name                    = "acctest-MLW2401122247478934"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet240112224747893439"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet240112224747893439"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks240112224747893439"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = join("", ["acctestaks", azurerm_resource_group.test.location])
  node_resource_group = "acctestRGAKS-240112224747893439"
