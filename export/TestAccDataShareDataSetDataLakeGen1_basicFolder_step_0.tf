...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-datashare-220211130509256599"
  location = "West Europe"
}

resource "azurerm_data_share_account" "test" {
  name                = "acctest-DSA-220211130509256599"
...
resource "azurerm_data_share" "test" {
  name       = "acctest_DS_220211130509256599"
  account_id = azurerm_data_share_account.test.id
  kind       = "CopyBased"
}

resource "azurerm_data_lake_store" "test" {
  name                = "acctestdlsx3elw"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_data_share_dataset_data_lake_gen1" "test" {
  name               = "acctest-DSDL1-220211130509256599"
