...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-210910021430266307"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet210910021430266307"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip210910021430266307"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall210910021430266307"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall_nat_rule_collection" "test" {
  name                = "acctestnrc-210910021430266307"
