...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-211112020507565289"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvnet211112020507565289"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet211112020507565289"
  resource_group_name  = "${azurerm_resource_group.test.name}"
...
resource "azurerm_public_ip" "test1" {
  name                = "acctpip1211112020507565289"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctpip1211112020507565289"
}

resource "azurerm_public_ip" "test2" {
  name                = "acctpip2211112020507565289"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctpip2211112020507565289"
}
...
resource "azurerm_sql_server" "test" {
  name                         = "acctestsql211112020507565289"
  resource_group_name          = "${azurerm_resource_group.test.name}"
  location                     = "${azurerm_resource_group.test.location}"
  version                      = "12.0"
...
resource "azurerm_data_factory" "test" {
  name                = "acctestdfirm211112020507565289"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
}

resource "azurerm_data_factory_linked_custom_service" "test" {
  name                 = "acctestls211112020507565289"
...
resource "azurerm_data_factory_linked_custom_service" "file_share_linked_service" {
  name                 = "acctestls1211112020507565289"
  data_factory_id      = azurerm_data_factory.test.id
...
resource "azurerm_data_factory_linked_service_key_vault" "test" {
  name                = "acctestlinkkv211112020507565289"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "test" {
  name                = "acctestSIRsh211112020507565289"
  data_factory_name   = azurerm_data_factory.test.name
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_data_factory_integration_runtime_azure_ssis" "test" {
  name                = "acctestiras211112020507565289"
  description         = "acctest"
  data_factory_name   = azurerm_data_factory.test.name
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
