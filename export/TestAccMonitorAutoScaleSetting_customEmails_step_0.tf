...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220121044747150124"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-220121044747150124"
...
resource "azurerm_virtual_machine_scale_set" "test" {
  name                = "acctvmss-220121044747150124"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    computer_name_prefix = "testvm-220121044747150124"
  ...
  network_profile {
    name    = "TestNetworkProfile-220121044747150124"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-220121044747150124"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-220121044747150124@example.com"]
    }
  }
}
