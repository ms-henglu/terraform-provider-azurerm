...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-220627132144201202"
  location = "West Europe"
}

resource "azurerm_sql_server" "test" {
  name                         = "acctest-sqlserver-220627132144201202"
...
resource "azurerm_mssql_database" "test" {
  name         = "acctest-db-220627132144201202"
...
resource "azurerm_resource_group" "second" {
  name     = "acctestRG-mssql2-220627132144201202"
  location = "West US 2"
}

resource "azurerm_sql_server" "second" {
  name                         = "acctest-sqlserver2-220627132144201202"
...
resource "azurerm_sql_failover_group" "failover_group" {
  ...
  name                = "acctest-fog-220627132144201202"
  resource_group_name = azurerm_resource_group.test.name
