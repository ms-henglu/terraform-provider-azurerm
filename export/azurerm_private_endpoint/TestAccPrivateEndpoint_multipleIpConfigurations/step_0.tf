...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-privatelink-230106034829573317"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvnet-230106034829573317"
...
resource "azurerm_subnet" "service" {
  name                 = "acctestsnetservice-230106034829573317"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "endpoint" {
  name                 = "acctestsnetendpoint-230106034829573317"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip-230106034829573317"
  sku                 = "Standard"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_lb" "test" {
  name                = "acctestlb-230106034829573317"
  sku                 = "Standard"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_private_link_service" "test" {
  name                           = "acctestPLS-230106034829573317"
  location                       = azurerm_resource_group.test.location
  resource_group_name            = azurerm_resource_group.test.name
  ...
  nat_ip_configuration {
    name      = "primaryIpConfiguration-230106034829573317"
    primary   = true
...
resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-vault-230106034829573317"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
...
resource "azurerm_private_endpoint" "test" {
  name                = "acctest-privatelink-230106034829573317"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
