...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-storage-221117230957383848"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-VN-221117230957383848"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsub-221117230957383848"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_resource_group_template_deployment" "storage-containers" {
  name                = "acctest-strgctn-deployment-221117230957383848"
  resource_group_name = azurerm_storage_account.test.resource_group_name
  deployment_mode     = "Incremental"

  parameters_content = jsonencode({
    name = {
      value = "acctest-strgctn-hpc-221117230957383848"
    }
  })
...
resource "azurerm_hpc_cache" "test" {
  name                = "acctest-HPCC-221117230957383848"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
