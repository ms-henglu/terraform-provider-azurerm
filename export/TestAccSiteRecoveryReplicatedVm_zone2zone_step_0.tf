...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-recovery-220603022534888993-1"
  location = "West Europe"
}

resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-recovery-220603022534888993-2"
  location = "West US 2"
}

resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-vault-220603022534888993"
...
resource "azurerm_site_recovery_fabric" "test1" {
  ...
  name                = "acctest-fabric1-220603022534888993"
...
resource "azurerm_site_recovery_protection_container" "test1" {
  ...
  name                 = "acctest-protection-cont1-220603022534888993"
}
...
resource "azurerm_site_recovery_protection_container" "test2" {
  ...
  recovery_vault_name  = azurerm_recovery_services_vault.test.name
  recovery_fabric_name = azurerm_site_recovery_fabric.test1.name
  name                 = "acctest-protection-cont2-t-220603022534888993"
}

resource "azurerm_site_recovery_replication_policy" "test" {
  resource_group_name                                  = azurerm_resource_group.test2.name
  recovery_vault_name                                  = azurerm_recovery_services_vault.test.name
  name                                                 = "acctest-policy-220603022534888993"
...
resource "azurerm_site_recovery_protection_container_mapping" "test" {
  ...
  name                                      = "mapping-220603022534888993"
}

resource "azurerm_virtual_network" "test1" {
  name                = "net-220603022534888993"
...
resource "azurerm_subnet" "test2_1" {
  name                 = "acctest-snet-220603022534888993_1"
  resource_group_name  = "${azurerm_resource_group.test2.name}"
...
resource "azurerm_subnet" "test2_2" {
  name                 = "snet-220603022534888993_2"
  resource_group_name  = "${azurerm_resource_group.test2.name}"
  virtual_network_name = "${azurerm_virtual_network.test2.name}"
...
resource "azurerm_virtual_machine" "test" {
  ...
  os_profile {
    ...
    computer_name  = "vm-220603022534888993"
  }
...
resource "azurerm_site_recovery_replicated_vm" "test" {
  ...
  network_interface {
    ...
    target_subnet_name          = "snet-220603022534888993"
  }
