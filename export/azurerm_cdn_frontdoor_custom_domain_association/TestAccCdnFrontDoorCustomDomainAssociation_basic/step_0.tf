...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-240112224050175196"
...
resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "acctest-profile-240112224050175196"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "acctest-origin-group-240112224050175196"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "acctest-origin-240112224050175196"
  cdn_frontdoor_origin_group_id = azurerm_cdn_frontdoor_origin_group.test.id
  enabled                       = true

  certificate_name_check_enabled = false
  host_name                      = join(".", ["bk62x6obtv", azurerm_dns_zone.test.name])
  priority                       = 1
  weight                         = 1
}

resource "azurerm_cdn_frontdoor_endpoint" "test" {
  name                     = "acctest-endpoint-240112224050175196"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
  enabled                  = true
}

resource "azurerm_cdn_frontdoor_route" "contoso" {
  name                          = "acctest-contoso-240112224050175196"
