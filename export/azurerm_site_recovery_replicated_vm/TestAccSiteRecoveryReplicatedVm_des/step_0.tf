...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-recovery-221028172651782153-1"
  location = "West Europe"
}

resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-recovery-221028172651782153-2"
  location = "West US 2"
}


resource "azurerm_key_vault" "test" {
  name                        = "kv221028172651782153"
...
resource "azurerm_disk_encryption_set" "test" {
  name                = "acctestdes-221028172651782153"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_managed_disk" "test" {
  name                   = "acctestd-221028172651782153"
  location               = azurerm_resource_group.test.location
  resource_group_name    = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctvn-221028172651782153"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-221028172651782153"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "acctni-221028172651782153"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "test" {
  name                  = "acctvm-221028172651782153"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_data_disk {
    name              = "acctmd-221028172651782153"
    create_option     = "Empty"
    disk_size_gb      = "1"
    lun               = 0
    managed_disk_type = "Standard_LRS"
  ...
  os_profile {
    computer_name  = "hn221028172651782153"
...
resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-vault-221028172651782153"
...
resource "azurerm_site_recovery_fabric" "test1" {
  ...
  name                = "acctest-fabric1-221028172651782153"
  location            = azurerm_resource_group.test.location
}
...
resource "azurerm_site_recovery_fabric" "test2" {
  resource_group_name = azurerm_resource_group.test2.name
  recovery_vault_name = azurerm_recovery_services_vault.test.name
  name                = "acctest-fabric2-221028172651782153"
  location            = azurerm_resource_group.test2.location
...
resource "azurerm_site_recovery_protection_container" "test1" {
  ...
  name                 = "acctest-protection-cont1-221028172651782153"
}
...
resource "azurerm_site_recovery_protection_container" "test2" {
  ...
  name                 = "acctest-protection-cont2-221028172651782153"
}

resource "azurerm_site_recovery_replication_policy" "test" {
  resource_group_name                                  = azurerm_resource_group.test2.name
  recovery_vault_name                                  = azurerm_recovery_services_vault.test.name
  name                                                 = "acctest-policy-221028172651782153"
...
resource "azurerm_site_recovery_protection_container_mapping" "test" {
  ...
  name                                      = "mapping-221028172651782153"
}

resource "azurerm_virtual_network" "test2" {
  name                = "net-221028172651782153-2"
  resource_group_name = azurerm_resource_group.test2.name
...
resource "azurerm_subnet" "test2_1" {
  name                 = "acctest-snet-221028172651782153-2"
  resource_group_name  = "${azurerm_resource_group.test2.name}"
...
resource "azurerm_subnet" "test2_2" {
  name                 = "snet-221028172651782153-3"
  resource_group_name  = "${azurerm_resource_group.test2.name}"
  virtual_network_name = "${azurerm_virtual_network.test2.name}"
...
resource "azurerm_public_ip" "test-recovery" {
  name                = "pubip221028172651782153-recovery"
...
resource "azurerm_disk_encryption_set" "test2" {
  name                = "acctestdes-2210281726517821532"
  resource_group_name = azurerm_resource_group.test2.name
  location            = azurerm_resource_group.test2.location
