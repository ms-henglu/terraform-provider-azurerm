...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211203161152230548"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-211203161152230548"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-211203161152230548"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine_scale_set" "test" {
  name                = "acctvmss-211203161152230548"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    computer_name_prefix = "testvm-211203161152230548"
...
resource "azurerm_subnet" "gwtest" {
  name                 = "gw-subnet-211203161152230548"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_public_ip" "test" {
  name                = "acctest-pubip-211203161152230548"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_application_gateway" "test" {
  name                = "acctestgw-211203161152230548"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
