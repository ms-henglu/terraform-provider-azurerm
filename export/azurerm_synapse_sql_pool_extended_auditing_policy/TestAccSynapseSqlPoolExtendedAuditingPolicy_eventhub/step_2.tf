...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-synapse-240119023015980719"
...
resource "azurerm_storage_data_lake_gen2_filesystem" "test" {
  name               = "acctest-240119023015980719"
  storage_account_id = azurerm_storage_account.sw.id
}

resource "azurerm_synapse_workspace" "test" {
  name                                 = "acctestsw240119023015980719"
  resource_group_name                  = azurerm_resource_group.test.name
  location                             = azurerm_resource_group.test.location
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctest-LAW-240119023015980719"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctest-EHN-240119023015980719"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_eventhub" "test" {
  name                = "acctest-EH-240119023015980719"
...
resource "azurerm_monitor_diagnostic_setting" "test" {
  name                           = "acctest-DS-240119023015980719"
