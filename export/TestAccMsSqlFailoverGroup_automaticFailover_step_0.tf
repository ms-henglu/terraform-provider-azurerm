...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-210924004612276459"
  location = "West Europe"
}

resource "azurerm_mssql_server" "test_primary" {
  name                         = "acctestmssql210924004612276459-primary"
...
resource "azurerm_mssql_server" "test_secondary" {
  name                         = "acctestmssql210924004612276459-secondary"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = "West US 2"
  version                      = "12.0"
  administrator_login          = "mradministrator"
...
resource "azurerm_mssql_database" "test" {
  name        = "acctestdb210924004612276459"
...
resource "azurerm_mssql_failover_group" "test" {
  name      = "acctestsfg210924004612276459"
  server_id = azurerm_mssql_server.test_primary.id
