...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-220506020233847698"
  location = "West Europe"
}

resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-220506020233847698"
...
resource "azurerm_mssql_database" "primary" {
  name        = "acctest-db-220506020233847698"
...
resource "azurerm_resource_group" "secondary" {
  name     = "acctestRG-mssql2-220506020233847698"
  location = "West US 2"
}

resource "azurerm_mssql_server" "secondary" {
  name                         = "acctest-sqlserver2-220506020233847698"
