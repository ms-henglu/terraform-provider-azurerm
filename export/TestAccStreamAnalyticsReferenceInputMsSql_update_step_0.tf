...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220812015850218573"
  location = "West Europe"
}

resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-220812015850218573"
...
resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-220812015850218573"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  version                      = "12.0"
...
resource "azurerm_mssql_database" "test" {
  name      = "acctest-db-220812015850218573"
  server_id = azurerm_mssql_server.test.id
}


resource "azurerm_stream_analytics_reference_input_mssql" "test" {
  name                      = "acctestinput-220812015850218573"
