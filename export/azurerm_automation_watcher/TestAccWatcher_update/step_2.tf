...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-231016033425317991"
  location = "West Europe"
}

resource "azurerm_automation_account" "test" {
  name                = "acctestAA-231016033425317991"
...
resource "azurerm_automation_credential" "test" {
  name                    = "acctest-231016033425317991"
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-231016033425317991"
...
resource "azurerm_network_interface" "test" {
  name                = "acctni-231016033425317991"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-231016033425317991"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  size                = "Standard_F2"
...
resource "azurerm_automation_runbook" "test" {
  name                    = "acc-runbook-231016033425317991"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
  automation_account_name = azurerm_automation_account.test.name
...
resource "azurerm_automation_watcher" "test" {
  ...
  name                  = "acctest-watcher-231016033425317991"
  location              = "West Europe"
