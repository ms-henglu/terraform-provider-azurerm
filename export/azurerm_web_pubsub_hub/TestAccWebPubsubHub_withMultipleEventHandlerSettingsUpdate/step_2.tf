...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-wps-231013044308564217"
  location = "West Europe"
}

resource "azurerm_web_pubsub" "test" {
  name                = "acctest-webpubsub-231013044308564217"
...
resource "azurerm_user_assigned_identity" "test1" {
  name                = "acctest-uai1-231013044308564217"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
resource "azurerm_user_assigned_identity" "test2" {
  name                = "acctest-uai2-231013044308564217"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
resource "azurerm_web_pubsub_hub" "test" {
  name          = "acctestwpsh231013044308564217"
