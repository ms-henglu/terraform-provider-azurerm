...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-CAE-240315122626537545"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-240315122626537545"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet240315122626537545"
...
resource "azurerm_subnet" "control" {
  ...
  delegation {
    name = "acctestdelegation240315122626537545"
    service_delegation {
...
resource "azurerm_container_app_environment" "test" {
  name                       = "acctest-CAEnv240315122626537545"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
...
resource "azurerm_container_registry" "test" {
  name                = "testacccr240315122626537545"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Basic"
  admin_enabled       = true
...
resource "azurerm_container_app_environment_storage" "test" {
  name                         = "testacc-caes-240315122626537545"
...
resource "azurerm_container_app" "test" {
  name                         = "acctest-capp-240315122626537545"
  resource_group_name          = azurerm_resource_group.test.name
  container_app_environment_id = azurerm_container_app_environment.test.id
  ...
  template {
    container {
      name   = "acctest-cont-240315122626537545"
