...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230106034757770399"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-230106034757770399"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                            = "acctvmss-230106034757770399"
  location                        = azurerm_resource_group.test.location
  resource_group_name             = azurerm_resource_group.test.name
  upgrade_mode                    = "Manual"
  sku                             = "Standard_F2"
  ...
  computer_name_prefix            = "testvm-230106034757770399"
  ...
  network_interface {
    name    = "TestNetworkProfile-230106034757770399"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-230106034757770399"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-230106034757770399@example.com", "acctest2-230106034757770399@example.com"]
    }
  }
}
