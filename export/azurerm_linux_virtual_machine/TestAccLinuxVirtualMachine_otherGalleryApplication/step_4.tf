...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240105063503993135"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-240105063503993135"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-240105063503993135"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig240105063503993135"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_gallery_application" "test" {
  name              = "acctest-app-240105063503993135"
...
resource "azurerm_gallery_application" "test2" {
  name              = "acctest-app2-240105063503993135"
  gallery_id        = azurerm_shared_image_gallery.test.id
  location          = azurerm_shared_image_gallery.test.location
  supported_os_type = "Linux"
}
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-240105063503993135"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  size                = "Standard_F2"
