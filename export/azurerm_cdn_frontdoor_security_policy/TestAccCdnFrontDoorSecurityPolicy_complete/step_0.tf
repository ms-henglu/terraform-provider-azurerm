...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-221202035244571225"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF221202035244571225"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone221202035244571225.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-221202035244571225"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-221202035244571225"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol221202035244571225"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
