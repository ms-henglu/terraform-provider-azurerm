...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-apim-230106034052101905"
  location = "West Europe"
}

resource "azurerm_api_management" "test" {
  name                = "acctestAM-230106034052101905"
...
resource "azurerm_redis_cache" "test" {
  name                = "acctestRedis-230106034052101905"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  capacity            = 1
  family              = "C"
...
resource "azurerm_redis_cache" "test2" {
  name                = "acctestRedis2-230106034052101905"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  capacity            = 1
  family              = "C"
...
resource "azurerm_api_management_redis_cache" "test" {
  name              = "acctest-Redis-Cache-230106034052101905"
