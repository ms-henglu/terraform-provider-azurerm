...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-netapp-210825031928076378"
  location = "East US 2"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-VirtualNetwork-210825031928076378"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-Subnet-210825031928076378"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_netapp_account" "test" {
  name                = "acctest-NetAppAccount-210825031928076378"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_netapp_pool" "test" {
  name                = "acctest-NetAppPool-210825031928076378"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_netapp_volume" "test" {
  name                = "acctest-NetAppVolume-210825031928076378"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  account_name        = azurerm_netapp_account.test.name
  pool_name           = azurerm_netapp_pool.test.name
  volume_path         = "my-unique-file-path-210825031928076378"
  service_level       = "Standard"
