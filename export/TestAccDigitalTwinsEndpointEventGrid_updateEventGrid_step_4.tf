...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-dtwin-211029015539232469"
  location = "West Europe"
}


resource "azurerm_digital_twins_instance" "test" {
  name                = "acctest-DT-211029015539232469"
...
resource "azurerm_eventgrid_topic" "test" {
  name                = "acctesteg-211029015539232469"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_eventgrid_topic" "test_alt" {
  name                = "acctesteg-alt-211029015539232469"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_digital_twins_endpoint_eventgrid" "test" {
  name                                 = "acctest-EG-211029015539232469"
