...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ase-231016034930986314"
  location = "West Europe"
}

resource "azurerm_resource_group" "test2" {
  name     = "acctestRG2-ase-231016034930986314"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-231016034930986314"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-subnet-231016034930986314"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_app_service_environment_v3" "test" {
  name                = "acctest-ase-231016034930986314"
  resource_group_name = azurerm_resource_group.test.name
