...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-iothub-230721015302216177"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-230721015302216177"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-230721015302216177"
...
resource "azurerm_eventhub_authorization_rule" "test" {
  name                = "acctest-230721015302216177"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_user_assigned_identity" "test" {
  name                = "acctestuai-230721015302216177"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
...
resource "azurerm_iothub" "test" {
  name                = "acctestIoTHub-230721015302216177"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
