...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-220107064409788525"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvnet-220107064409788525"
...
resource "azurerm_subnet" "service" {
  name                 = "acctestsnetservice-220107064409788525"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "endpoint" {
  name                 = "acctestsnetendpoint-220107064409788525"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_mssql_server" "test" {
  name                         = "acctestsqlserver220107064409788525"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  version                      = "12.0"
...
resource "azurerm_private_endpoint" "test" {
  name                = "acctest-privatelink-220107064409788525"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  subnet_id           = azurerm_subnet.endpoint.id

  private_service_connection {
    name                           = "acctest-privatelink-mssc-220107064409788525"
