...
resource "azurerm_resource_group" "test" {
  name     = "acctest-rg-240315122643970131"
  location = "West Europe"
}

resource "azurerm_kubernetes_fleet_manager" "test" {
  location            = azurerm_resource_group.test.location
  name                = "acctestkfm-240315122643970131"
  resource_group_name = azurerm_resource_group.test.name
  hub_profile {
    dns_prefix = "val-240315122643970131"
  }
}

resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestkc-240315122643970131"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = "acctestkc-240315122643970131"
...
resource "azurerm_kubernetes_fleet_member" "test" {
  ...
  group                 = "acctestfus-240315122643970131"
}

resource "azurerm_kubernetes_fleet_update_strategy" "test" {
  name                        = "acctestfus-240315122643970131"
