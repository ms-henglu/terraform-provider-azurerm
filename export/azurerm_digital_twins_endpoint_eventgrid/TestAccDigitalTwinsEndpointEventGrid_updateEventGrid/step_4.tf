...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-dtwin-230526085026317399"
  location = "West Europe"
}


resource "azurerm_digital_twins_instance" "test" {
  name                = "acctest-DT-230526085026317399"
...
resource "azurerm_eventgrid_topic" "test" {
  name                = "acctesteg-230526085026317399"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_eventgrid_topic" "test_alt" {
  name                = "acctesteg-alt-230526085026317399"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_digital_twins_endpoint_eventgrid" "test" {
  name                                 = "acctest-EG-230526085026317399"
