...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-traffic-220204093706290740"
  location = "West Europe"
}

resource "azurerm_traffic_manager_profile" "parent" {
  name                   = "acctesttmpparent220204093706290740"
  resource_group_name    = azurerm_resource_group.test.name
  traffic_routing_method = "Priority"

  dns_config {
    relative_name = "acctestparent220204093706290740"
    ttl           = 30
  }
...
resource "azurerm_traffic_manager_profile" "child" {
  name                   = "acctesttmpchild220204093706290740"
  resource_group_name    = azurerm_resource_group.test.name
  traffic_routing_method = "Priority"

  dns_config {
    relative_name = "acctesttmpchild220204093706290740"
    ttl           = 30
  }
...
resource "azurerm_traffic_manager_endpoint" "nested" {
  name                                  = "acctestend-parent220204093706290740"
...
resource "azurerm_traffic_manager_endpoint" "externalChild" {
  name                = "acctestend-child220204093706290740"
