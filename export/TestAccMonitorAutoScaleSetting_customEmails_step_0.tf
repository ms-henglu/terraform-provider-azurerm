...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220513023535020523"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-220513023535020523"
...
resource "azurerm_virtual_machine_scale_set" "test" {
  name                = "acctvmss-220513023535020523"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    computer_name_prefix = "testvm-220513023535020523"
  ...
  network_profile {
    name    = "TestNetworkProfile-220513023535020523"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-220513023535020523"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-220513023535020523@example.com"]
    }
  }
}
