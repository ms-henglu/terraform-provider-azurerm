...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-230519074622955788"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230519074622955788"
...
resource "azurerm_public_ip" "test" {
  name                = "acctpip-230519074622955788"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-230519074622955788"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine_extension" "test" {
  name                 = "acctestExt-230519074622955788"
...
resource "azurerm_resource_group" "host" {
  name     = "acctesthostRG-df-230519074622955788"
  location = "West Europe"
}

resource "azurerm_data_factory" "host" {
  name                = "acctestdfirshh230519074622955788"
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "host" {
  name            = "acctestirshh230519074622955788"
  data_factory_id = azurerm_data_factory.host.id
}

resource "azurerm_resource_group" "target" {
  name     = "acctesttargetRG-230519074622955788"
  location = "West Europe"
}
...
resource "azurerm_data_factory" "target" {
  name                = "acctestdfirsht230519074622955788"
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "target" {
  name            = "acctestirsht230519074622955788"
