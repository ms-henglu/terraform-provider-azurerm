...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-signalr-230203064154252224"
  location = "West Europe"
}

resource "azurerm_signalr_service" "test" {
  name                = "acctestSignalr-230203064154252224"
...
resource "azurerm_service_plan" "test" {
  name                = "acctestASP-230203064154252224"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  os_type             = "Windows"
  sku_name            = "S1"
...
resource "azurerm_windows_web_app" "test" {
  name                = "acctestWA-230203064154252224"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_signalr_shared_private_link_resource" "test" {
  name               = "acctest-230203064154252224"
