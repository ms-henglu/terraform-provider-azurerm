...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220627124801981045"
  location = "West Europe"
}

resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-220627124801981045"
...
resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-220627124801981045"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  version                      = "12.0"
...
resource "azurerm_mssql_database" "test" {
  name      = "acctest-db-220627124801981045"
  server_id = azurerm_mssql_server.test.id
}


resource "azurerm_stream_analytics_reference_input_mssql" "test" {
  name                      = "acctestinput-220627124801981045"
