...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-PAN-231013044029359041"
  location = "West Europe"
}

resource "azurerm_palo_alto_local_rulestack_certificate" "test" {
  name         = "testacc-palc-231013044029359041"
...
resource "azurerm_palo_alto_local_rulestack_certificate" "trust" {
  name         = "testacc-palcT-231013044029359041"
  rulestack_id = azurerm_palo_alto_local_rulestack.test.id
...
resource "azurerm_palo_alto_local_rulestack_certificate" "untrust" {
  name         = "testacc-palcU-231013044029359041"
  rulestack_id = azurerm_palo_alto_local_rulestack.test.id
...
resource "azurerm_palo_alto_local_rulestack" "test" {
  name                = "testAcc-palrs-231013044029359041"
  resource_group_name = azurerm_resource_group.test.name
  location            = "West Europe"
}
...
resource "azurerm_palo_alto_local_rulestack_rule" "test" {
  name         = "testacc-palr-231013044029359041"
  rulestack_id = azurerm_palo_alto_local_rulestack.test.id
  priority     = 100
  action       = "Allow"
