...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-230512010245118211"
  location = "West Europe"
}

resource "azurerm_automation_account" "test" {
  name                = "acctestAA-230512010245118211"
...
resource "azurerm_automation_credential" "test" {
  name                    = "acctest-230512010245118211"
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230512010245118211"
...
resource "azurerm_network_interface" "test" {
  name                = "acctni-230512010245118211"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-230512010245118211"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  size                = "Standard_F2"
...
resource "azurerm_automation_runbook" "test" {
  name                    = "acc-runbook-230512010245118211"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
  automation_account_name = azurerm_automation_account.test.name
...
resource "azurerm_automation_watcher" "test" {
  ...
  name                  = "acctest-watcher-230512010245118211"
  location              = "West Europe"
