...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-211105025858643990"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvnet211105025858643990"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet211105025858643990"
  resource_group_name  = "${azurerm_resource_group.test.name}"
...
resource "azurerm_public_ip" "test1" {
  name                = "acctpip1211105025858643990"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctpip1211105025858643990"
}

resource "azurerm_public_ip" "test2" {
  name                = "acctpip2211105025858643990"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctpip2211105025858643990"
}
...
resource "azurerm_sql_server" "test" {
  name                         = "acctestsql211105025858643990"
  resource_group_name          = "${azurerm_resource_group.test.name}"
  location                     = "${azurerm_resource_group.test.location}"
  version                      = "12.0"
...
resource "azurerm_data_factory" "test" {
  name                = "acctestdfirm211105025858643990"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
}

resource "azurerm_data_factory_linked_custom_service" "test" {
  name                 = "acctestls211105025858643990"
...
resource "azurerm_data_factory_linked_custom_service" "file_share_linked_service" {
  name                 = "acctestls1211105025858643990"
  data_factory_id      = azurerm_data_factory.test.id
...
resource "azurerm_data_factory_linked_service_key_vault" "test" {
  name                = "acctestlinkkv211105025858643990"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "test" {
  name                = "acctestSIRsh211105025858643990"
  data_factory_name   = azurerm_data_factory.test.name
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_data_factory_integration_runtime_azure_ssis" "test" {
  name                = "acctestiras211105025858643990"
  description         = "acctest"
  data_factory_name   = azurerm_data_factory.test.name
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
