...
resource "azurerm_resource_group" "test" {
  name     = "acctest220812015546229069"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-220812015546229069"
...
resource "azurerm_policy_definition" "test" {
  name         = "acctestpol-220812015546229069"
  policy_type  = "Custom"
  mode         = "All"
  display_name = "acctestpol-220812015546229069"
...
resource "azurerm_resource_policy_assignment" "test" {
  name                 = "acctestpa-220812015546229069"
