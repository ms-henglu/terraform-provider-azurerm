...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221111020649451404"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-221111020649451404"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsn-221111020649451404"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_storage_container" "test" {
  name                  = "acctestsc-221111020649451404"
  storage_account_name  = azurerm_storage_account.test.name
  container_access_type = "private"
}

resource "azurerm_public_ip" "test" {
  name                = "acctestpip-221111020649451404"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_lb" "test" {
  name                = "acctestlb-221111020649451404"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_lb_backend_address_pool" "test" {
  name            = "acctestbap-221111020649451404"
  loadbalancer_id = azurerm_lb.test.id
}

resource "azurerm_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-221111020649451404"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
