...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230721015711347321"
  location = "West Europe"
}

resource "azurerm_virtual_wan" "test" {
  name                = "acctestvwan-230721015711347321"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_virtual_hub" "test" {
  name                = "acctestvhub-230721015711347321"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_vpn_server_configuration" "test" {
  name                     = "acctestvpnsc-230721015711347321"
  resource_group_name      = azurerm_resource_group.test.name
  location                 = azurerm_resource_group.test.location
...
resource "azurerm_point_to_site_vpn_gateway" "test" {
  name                        = "acctestp2sVPNG-230721015711347321"
  location                    = azurerm_resource_group.test.location
  resource_group_name         = azurerm_resource_group.test.name
