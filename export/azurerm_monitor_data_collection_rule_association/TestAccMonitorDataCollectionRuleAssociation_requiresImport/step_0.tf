...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-DCRA-230428050149221089"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "network-230428050149221089"
...
resource "azurerm_subnet" "test" {
  name                 = "subnet-230428050149221089"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "machine-230428050149221089"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  size                = "Standard_B1ls"
...
resource "azurerm_monitor_data_collection_rule" "test" {
  name                = "acctestmdcr-230428050149221089"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  destinations {
...
resource "azurerm_monitor_data_collection_rule_association" "test" {
  name                    = "test-dcra-230428050149221089"
