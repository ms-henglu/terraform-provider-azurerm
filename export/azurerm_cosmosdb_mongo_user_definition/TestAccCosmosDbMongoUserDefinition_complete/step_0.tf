...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mongouserdef-240119021821178949"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca-240119021821178949"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-240119021821178949"
...
resource "azurerm_cosmosdb_mongo_role_definition" "test" {
  ...
  role_name                = "acctestmongoroledef240119021821178949"
}

resource "azurerm_cosmosdb_mongo_user_definition" "test" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  username                 = "myUserName-240119021821178949"
  password                 = "myPassword-240119021821178949"
