...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211217075027271139"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-211217075027271139"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-211217075027271139"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_proximity_placement_group" "test" {
  name                = "acctestPPG-211217075027271139"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_proximity_placement_group" "second" {
  name                = "acctestPPG2-211217075027271139"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_linux_virtual_machine" "test" {
  name                         = "acctestVM-211217075027271139"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  size                         = "Standard_F2"
