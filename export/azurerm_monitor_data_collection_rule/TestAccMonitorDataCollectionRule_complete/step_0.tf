...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-DataCollectionRule-240112224854968463"
  location = "westeurope"
}
...
resource "azurerm_user_assigned_identity" "test" {
  name                = "acctestuai-240112224854968463"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_log_analytics_workspace" "test" {
  name                               = "acctest-law-240112224854968463"
  location                           = azurerm_resource_group.test.location
  resource_group_name                = azurerm_resource_group.test.name
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acceventn240112224854968463"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  capacity            = 1
...
resource "azurerm_eventhub" "test" {
  name                = "accevent240112224854968463"
...
resource "azurerm_storage_container" "test" {
  name                  = "acccontainer240112224854968463"
  storage_account_name  = azurerm_storage_account.test.name
  container_access_type = "private"
}

resource "azurerm_monitor_data_collection_endpoint" "test" {
  name                = "acctestmdcr-240112224854968463"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
