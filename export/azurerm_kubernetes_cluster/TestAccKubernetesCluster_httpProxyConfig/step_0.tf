...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230407023134927696"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet230407023134927696"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet230407023134927696"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "test-nic230407023134927696"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                            = "vm-test-proxy230407023134927696"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_B1s"
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks230407023134927696"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = "acctestaks230407023134927696"
  kubernetes_version  = "1.25.5"

  linux_profile {
    admin_username = "acctestuser230407023134927696"
    ssh_key {
