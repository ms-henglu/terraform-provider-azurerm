...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-security-240119022805270791"
  location = "West Europe"
}

resource "azurerm_iothub" "test" {
  name                = "acctestIoTHub-240119022805270791"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctest-law-240119022805270791"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
}
...
resource "azurerm_log_analytics_workspace" "test2" {
  name                = "acctest-law2-240119022805270791"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
}
...
resource "azurerm_iot_security_solution" "test" {
  name                = "acctest-Iot-Security-Solution-240119022805270791"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
