...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-tsi-220527034259277288"
  location = "West Europe"
}

resource "azurerm_iothub" "test" {
  name                = "acctestIoTHub-220527034259277288"
...
resource "azurerm_iot_time_series_insights_gen2_environment" "test" {
  name                = "acctest_tsie220527034259277288"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "L1"
  id_properties       = ["id"]
...
resource "azurerm_iot_time_series_insights_event_source_iothub" "test" {
  name                     = "acctest_tsiesi220527034259277288"
  location                 = azurerm_resource_group.test.location
