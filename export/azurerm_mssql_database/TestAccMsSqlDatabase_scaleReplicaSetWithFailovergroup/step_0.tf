...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-230428050203477557"
  location = "West Europe"
}

resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-230428050203477557"
...
resource "azurerm_mssql_database" "test" {
  name         = "acctest-db-230428050203477557"
...
resource "azurerm_resource_group" "second" {
  name     = "acctestRG-mssql2-230428050203477557"
  location = "West US 2"
}

resource "azurerm_mssql_server" "second" {
  name                         = "acctest-sqlserver2-230428050203477557"
...
resource "azurerm_mssql_failover_group" "failover_group" {
  name      = "acctest-fog-230428050203477557"
  server_id = azurerm_mssql_server.test.id
