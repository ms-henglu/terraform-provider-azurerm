...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-220811053138031094"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-220811053138031094"
...
resource "azurerm_public_ip" "test" {
  name                = "acctpip-220811053138031094"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-220811053138031094"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine_extension" "test" {
  name                 = "acctestExt-220811053138031094"
...
resource "azurerm_resource_group" "host" {
  name     = "acctesthostRG-df-220811053138031094"
  location = "West Europe"
}

resource "azurerm_data_factory" "host" {
  name                = "acctestdfirshh220811053138031094"
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "host" {
  name            = "acctestirshh220811053138031094"
  data_factory_id = azurerm_data_factory.host.id
}

resource "azurerm_resource_group" "target" {
  name     = "acctesttargetRG-220811053138031094"
  location = "West Europe"
}
...
resource "azurerm_data_factory" "target" {
  name                = "acctestdfirsht220811053138031094"
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "target" {
  name            = "acctestirsht220811053138031094"
