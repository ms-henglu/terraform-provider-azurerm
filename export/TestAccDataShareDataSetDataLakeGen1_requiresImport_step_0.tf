...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-datashare-220311042320293691"
  location = "West Europe"
}

resource "azurerm_data_share_account" "test" {
  name                = "acctest-DSA-220311042320293691"
...
resource "azurerm_data_share" "test" {
  name       = "acctest_DS_220311042320293691"
  account_id = azurerm_data_share_account.test.id
  kind       = "CopyBased"
}

resource "azurerm_data_lake_store" "test" {
  name                = "acctestdlshzy8m"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_data_share_dataset_data_lake_gen1" "test" {
  name               = "acctest-DSDL1-220311042320293691"
