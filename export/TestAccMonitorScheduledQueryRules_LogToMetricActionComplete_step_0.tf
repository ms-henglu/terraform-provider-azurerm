...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-monitor-210924011231732305"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestWorkspace-210924011231732305"
...
resource "azurerm_monitor_action_group" "test" {
  name                = "acctestActionGroup-210924011231732305"
  resource_group_name = "${azurerm_resource_group.test.name}"
  short_name          = "acctestag"
}

resource "azurerm_monitor_scheduled_query_rules_log" "test" {
  name                = "acctestsqr-210924011231732305"
  resource_group_name = "${azurerm_resource_group.test.name}"
  location            = "${azurerm_resource_group.test.location}"
...
resource "azurerm_monitor_metric_alert" "test" {
  name                = "acctestmal-210924011231732305"
  resource_group_name = "${azurerm_resource_group.test.name}"
