...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mongouserdef-230602030332792571"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca-230602030332792571"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-230602030332792571"
...
resource "azurerm_cosmosdb_mongo_role_definition" "test" {
  ...
  role_name                = "acctestmongoroledef230602030332792571"
}

resource "azurerm_cosmosdb_mongo_user_definition" "test" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  username                 = "myUserName-230602030332792571"
  password                 = "myPassword-230602030332792571"
