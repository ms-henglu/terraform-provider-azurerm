...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-220225034258399904"
...
resource "azurerm_storage_data_lake_gen2_filesystem" "test" {
  name               = "acctest-datalake-220225034258399904"
  storage_account_id = azurerm_storage_account.test.id
}

resource "azurerm_data_factory" "test" {
  name                = "acctestdf220225034258399904"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_linked_service_data_lake_storage_gen2" "test" {
  name                 = "acctestDataLakeStorage220225034258399904"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_dataset_delimited_text" "test" {
  name                = "acctestds220225034258399904"
  resource_group_name = azurerm_resource_group.test.name
