...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-221216013159918263"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-221216013159918263"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "accTestOriginGroup-221216013159918263"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "accTestOrigin-221216013159918263"
...
resource "azurerm_cdn_frontdoor_rule_set" "test" {
  name                     = "accTestRuleSet221216013159918263"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
}
...
resource "azurerm_cdn_frontdoor_rule" "test" {
  ...
  name                      = "accTestRule221216013159918263"
