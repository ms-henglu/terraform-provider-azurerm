...
resource "azurerm_resource_group" "test" {
  name     = "acctest-dataprotection-221117230758545833"
  location = "West Europe"
}

resource "azurerm_postgresql_server" "test" {
  name                = "acctest-postgresql-server-221117230758545833"
...
resource "azurerm_postgresql_database" "test" {
  name                = "acctest-postgresql-database-221117230758545833"
  resource_group_name = azurerm_resource_group.test.name
  server_name         = azurerm_postgresql_server.test.name
  charset             = "UTF8"
...
resource "azurerm_data_protection_backup_vault" "test" {
  name                = "acctest-dataprotection-vault-221117230758545833"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_key_vault_secret" "test" {
  name         = "acctestsecret221117230758545833"
...
resource "azurerm_data_protection_backup_policy_postgresql" "test" {
  name                            = "acctest-dp-221117230758545833"
  resource_group_name             = azurerm_resource_group.test.name
...
resource "azurerm_data_protection_backup_policy_postgresql" "another" {
  name                            = "acctest-dp-second-221117230758545833"
  resource_group_name             = azurerm_resource_group.test.name
  vault_name                      = azurerm_data_protection_backup_vault.test.name
  backup_repeating_time_intervals = ["R/2021-05-23T02:30:00+00:00/P1W"]
...
resource "azurerm_data_protection_backup_instance_postgresql" "test" {
  name                                    = "acctest-dbi-221117230758545833"
  location                                = azurerm_resource_group.test.location
