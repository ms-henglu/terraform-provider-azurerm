...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-210830084017499076"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet210830084017499076"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip210830084017499076"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test_mgmt" {
  name                = "acctestmgmtpip210830084017499076"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  sku                 = "Standard"
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall210830084017499076"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
