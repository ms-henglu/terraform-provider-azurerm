...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-databricks-220726001759652916"
  location = "West US 2"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-220726001759652916"
...
resource "azurerm_subnet" "public" {
  name                 = "acctest-sn-public-220726001759652916"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "private" {
  name                 = "acctest-sn-private-220726001759652916"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_subnet" "privatelink" {
  name                 = "acctest-snpl-220726001759652916"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_network_security_group" "nsg" {
  name                = "acctest-nsg-220726001759652916"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_databricks_workspace" "test" {
  ...
  name                        = "acctestDBW-220726001759652916"
  resource_group_name         = azurerm_resource_group.test.name
  location                    = azurerm_resource_group.test.location
  sku                         = "premium"
  managed_resource_group_name = "acctestRG-DBW-220726001759652916-managed"
...
resource "azurerm_private_endpoint" "databricks" {
  name                = "acctest-endpoint-220726001759652916"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  subnet_id           = azurerm_subnet.privatelink.id

  private_service_connection {
    name                           = "acctest-psc-220726001759652916"
