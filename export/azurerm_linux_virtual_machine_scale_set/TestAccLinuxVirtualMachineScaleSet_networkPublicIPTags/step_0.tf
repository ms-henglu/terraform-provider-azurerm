...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230106034233434383"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230106034233434383"
...
resource "azurerm_public_ip" "test" {
  name                = "test-ip-230106034233434383"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  domain_name_label   = "acctest-p9mj6frz1"

  sku = "Standard"
}

resource "azurerm_lb" "test" {
  name                = "acctestlb-230106034233434383"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-230106034233434383"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard_F2"
  instances           = 1
