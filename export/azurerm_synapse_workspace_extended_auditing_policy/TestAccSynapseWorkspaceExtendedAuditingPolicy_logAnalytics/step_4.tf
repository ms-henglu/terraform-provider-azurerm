...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-synapse-231016034855259224"
...
resource "azurerm_storage_data_lake_gen2_filesystem" "test" {
  name               = "acctest-231016034855259224"
  storage_account_id = azurerm_storage_account.sw.id
}

resource "azurerm_synapse_workspace" "test" {
  name                                 = "acctestsw231016034855259224"
  resource_group_name                  = azurerm_resource_group.test.name
  location                             = azurerm_resource_group.test.location
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctest-LAW-231016034855259224"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctest-EHN-231016034855259224"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_eventhub" "test" {
  name                = "acctest-EH-231016034855259224"
...
resource "azurerm_monitor_diagnostic_setting" "test" {
  name                       = "acctest-DS-231016034855259224"
