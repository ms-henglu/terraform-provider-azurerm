...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230818023401759354"
...
resource "azurerm_aadb2c_directory" "test" {
  ...
  display_name            = "acctest230818023401759354"
  domain_name             = "acctest230818023401759354.onmicrosoft.com"
...
resource "azurerm_resource_group" "duplicate" {
  name     = "acctestRG-duplicate-230818023401759354"
  location = "West US 2"
}
...
resource "azurerm_aadb2c_directory" "duplicate" {
  ...
  display_name            = "acctest-duplicate-230818023401759354"
