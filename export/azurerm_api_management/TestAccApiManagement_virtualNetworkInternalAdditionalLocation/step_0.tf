...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230728024941894933"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestVNET-230728024941894933"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestSNET-230728024941894933"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "acctest-NSG-230728024941894933"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-230728024941894933-2"
  location = "West US 2"
}

// subnet2 from the second location
resource "azurerm_virtual_network" "test2" {
  name                = "acctestVNET2-230728024941894933"
...
resource "azurerm_subnet" "test2" {
  name                 = "acctestSNET2-230728024941894933"
  resource_group_name  = azurerm_resource_group.test2.name
...
resource "azurerm_network_security_group" "test2" {
  name                = "acctest-NSG2-230728024941894933"
  location            = azurerm_resource_group.test2.location
  resource_group_name = azurerm_resource_group.test2.name
}
...
resource "azurerm_public_ip" "test1" {
  name                = "acctest-IP1-jppvf"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctest-ip1-jppvf"
}

resource "azurerm_public_ip" "test2" {
  name                = "acctest-IP2-jppvf"
  resource_group_name = azurerm_resource_group.test2.name
  location            = azurerm_resource_group.test2.location
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctest-ip2-jppvf"
}

resource "azurerm_api_management" "test" {
  name                = "acctestAM-230728024941894933"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  publisher_name      = "pub1"
