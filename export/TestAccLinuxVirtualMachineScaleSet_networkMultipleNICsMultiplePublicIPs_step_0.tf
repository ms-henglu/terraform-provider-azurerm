...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220812014802144586"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-220812014802144586"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-220812014802144586"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard_F2"
  instances           = 1
  ...
  network_interface {
    ...
    ip_configuration {
      ...
      public_ip_address {
        name                    = "first"
        domain_name_label       = "acctest1-220812014802144586"
  ...
  network_interface {
    ...
    ip_configuration {
      ...
      public_ip_address {
        name                    = "second"
        domain_name_label       = "acctest2-220812014802144586"
        idle_timeout_in_minutes = 4
      }
    }
  }
