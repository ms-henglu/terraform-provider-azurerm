...
locals {
  vm_name = "acctvm-7dpmq"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230414020945169939"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230414020945169939"
...
resource "azurerm_public_ip" "test" {
  name                = "acctpip-230414020945169939"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_network_interface" "public" {
  name                = "acctnicsource-230414020945169939"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-230414020945169939"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig230414020945169939"
  resource_group_name = "${azurerm_resource_group.test.name}"
  location            = "${azurerm_resource_group.test.location}"
}
