...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220527033959180679"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet220527033959180679"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet220527033959180679"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "test-nic220527033959180679"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                            = "vm-test-proxy220527033959180679"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_B1s"
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks220527033959180679"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = "acctestaks220527033959180679"
  kubernetes_version  = "1.22.4"

  linux_profile {
    admin_username = "acctestuser220527033959180679"
    ssh_key {
