...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211112021340061219"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctestehn-211112021340061219"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteh-211112021340061219"
...
resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-211112021340061219"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_eventhub_namespace" "updated" {
  name                = "acctestehn2-211112021340061219"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  capacity            = 1
...
resource "azurerm_eventhub" "updated" {
  name                = "acctesteh2-211112021340061219"
...
resource "azurerm_stream_analytics_output_eventhub" "test" {
  name                      = "acctestinput-211112021340061219"
