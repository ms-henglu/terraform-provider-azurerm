...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-220114064156400025"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet220114064156400025"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip220114064156400025"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test_mgmt" {
  name                = "acctestmgmtpip220114064156400025"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  sku                 = "Standard"
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall220114064156400025"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
