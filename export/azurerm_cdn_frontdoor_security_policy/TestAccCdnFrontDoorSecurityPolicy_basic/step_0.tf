...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-230519074315123797"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF230519074315123797"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone230519074315123797.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-230519074315123797"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-230519074315123797"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol230519074315123797"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
