...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cassandra-240105063548820552"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-240105063548820552"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-240105063548820552"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_cosmosdb_cassandra_cluster" "test" {
  name                           = "acctca-mi-cluster-240105063548820552"
  resource_group_name            = azurerm_resource_group.test.name
  location                       = azurerm_resource_group.test.location
...
resource "azurerm_cosmosdb_cassandra_datacenter" "test" {
  name                           = "acctca-mi-dc-240105063548820552"
