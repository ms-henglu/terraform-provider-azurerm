...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-220107064101307658"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet220107064101307658"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip220107064101307658"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall220107064101307658"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall_nat_rule_collection" "test" {
  name                = "acctestnrc-220107064101307658"
