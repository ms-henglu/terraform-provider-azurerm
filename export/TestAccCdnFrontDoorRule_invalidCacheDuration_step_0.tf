...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-220905045516155752"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-220905045516155752"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "accTestOriginGroup-220905045516155752"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "accTestOrigin-220905045516155752"
...
resource "azurerm_cdn_frontdoor_rule_set" "test" {
  name                     = "accTestRuleSet220905045516155752"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
}
...
resource "azurerm_cdn_frontdoor_rule" "test" {
  ...
  name                      = "accTestRule220905045516155752"
