...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-dtwin-210910021353547207"
  location = "West Europe"
}


resource "azurerm_digital_twins_instance" "test" {
  name                = "acctest-DT-210910021353547207"
...
resource "azurerm_eventgrid_topic" "test" {
  name                = "acctesteg-210910021353547207"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_eventgrid_topic" "test_alt" {
  name                = "acctesteg-alt-210910021353547207"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_digital_twins_endpoint_eventgrid" "test" {
  name                                 = "acctest-EG-210910021353547207"
