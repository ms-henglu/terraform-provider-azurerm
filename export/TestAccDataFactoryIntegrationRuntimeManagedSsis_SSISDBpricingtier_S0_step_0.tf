...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-220429075326802644"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvnet220429075326802644"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet220429075326802644"
  resource_group_name  = "${azurerm_resource_group.test.name}"
...
resource "azurerm_public_ip" "test1" {
  name                = "acctpip1220429075326802644"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctpip1220429075326802644"
}

resource "azurerm_public_ip" "test2" {
  name                = "acctpip2220429075326802644"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctpip2220429075326802644"
}
...
resource "azurerm_sql_server" "test" {
  name                         = "acctestsql220429075326802644"
  resource_group_name          = "${azurerm_resource_group.test.name}"
  location                     = "${azurerm_resource_group.test.location}"
  version                      = "12.0"
...
resource "azurerm_data_factory" "test" {
  name                = "acctestdfirm220429075326802644"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
}

resource "azurerm_data_factory_linked_custom_service" "test" {
  name                 = "acctestls220429075326802644"
...
resource "azurerm_data_factory_linked_custom_service" "file_share_linked_service" {
  name                 = "acctestls1220429075326802644"
  data_factory_id      = azurerm_data_factory.test.id
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "test" {
  name            = "acctestSIRsh220429075326802644"
  data_factory_id = azurerm_data_factory.test.id
}

resource "azurerm_data_factory_integration_runtime_azure_ssis" "test" {
  name            = "acctestiras220429075326802644"
  description     = "acctest"
  data_factory_id = azurerm_data_factory.test.id
  location        = azurerm_resource_group.test.location
