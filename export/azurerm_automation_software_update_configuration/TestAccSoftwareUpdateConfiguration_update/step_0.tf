...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-221028171744557199"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-221028171744557199"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-221028171744557199"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-221028171744557199"
  ...
  schedule {
    ...
    start_time          = "2022-10-29T03:17:44Z"
    expiry_time         = "2022-10-30T19:17:44Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
