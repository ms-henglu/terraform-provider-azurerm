...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220826002453876630"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-220826002453876630"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-220826002453876630"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard_F2"
  instances           = 1
  ...
  network_interface {
    ...
    ip_configuration {
      ...
      public_ip_address {
        name                    = "first"
        domain_name_label       = "acctest1-220826002453876630"
  ...
  network_interface {
    ...
    ip_configuration {
      ...
      public_ip_address {
        name                    = "second"
        domain_name_label       = "acctest2-220826002453876630"
        idle_timeout_in_minutes = 4
      }
    }
  }
