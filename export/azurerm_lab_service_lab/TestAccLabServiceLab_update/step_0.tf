...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-lab-230512010851432356"
  location = "West Europe"
}


resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig230512010851432356"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_shared_image" "test" {
  name                = "acctestimg230512010851432356"
  ...
  identifier {
    publisher = "AccTesPublisher230512010851432356"
    offer     = "AccTesOffer230512010851432356"
    sku       = "AccTesSku230512010851432356"
  }
}
...
resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-230512010851432356"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-sn-230512010851432356"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-230512010851432356"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-linux-230512010851432356"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  size                = "Standard_F2"
...
resource "azurerm_lab_service_plan" "test" {
  name                = "acctest-lslp-230512010851432356"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_lab_service_lab" "test" {
  name                = "acctest-lab-230512010851432356"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  title               = "Test Title"
