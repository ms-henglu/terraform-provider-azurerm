...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-synapse-211203162027038020"
  location = "West Europe"
}

resource "azurerm_storage_account" "test" {
  name                     = "acctestacct0u6n"
...
resource "azurerm_storage_data_lake_gen2_filesystem" "test" {
  name               = "acctest-211203162027038020"
  storage_account_id = azurerm_storage_account.test.id
}

resource "azurerm_synapse_workspace" "test" {
  name                                 = "acctestsw211203162027038020"
  resource_group_name                  = azurerm_resource_group.test.name
  location                             = azurerm_resource_group.test.location
...
resource "azurerm_synapse_managed_private_endpoint" "test" {
  name                 = "acctestEndpoint211203162027038020"
  synapse_workspace_id = azurerm_synapse_workspace.test.id
