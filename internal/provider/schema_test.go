...
func getCoverageReport(results CoverageResults) string {
	...
	content += "### Summary\n"
	content += "| Resource Provider | Uncovered | Covered | Properties total | Coverage |\n"
	content += "| --- | --- | --- | --- | --- |\n"
	sort.Sort(results)
	for _, res := range results {
		uncovered := len(res.UncoveredPropertyList)
		covered := res.PropertyCount - uncovered
		content += fmt.Sprintf("| %s | %d | %d | %d | %.2f%% |\n", res.ResourceProviderName, uncovered, covered, res.PropertyCount, float32(covered)/float32(res.PropertyCount)*100)
	}
...
func Test_coverage(t *testing.T) {
	...
	for i, f := range files {
		filename := workingDirectory + f.Name()
		for _, prop := range getTestedProperties(filename) {
			testedProps[prop] = f.Name()
		}
		if i%500 == 0 {
