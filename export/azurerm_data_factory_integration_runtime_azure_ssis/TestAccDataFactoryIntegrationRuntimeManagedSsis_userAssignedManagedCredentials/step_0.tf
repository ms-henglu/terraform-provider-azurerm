...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-240311031853893370"
  location = "West Europe"
}

resource "azurerm_user_assigned_identity" "test" {
  location            = azurerm_resource_group.test.location
  name                = "testuser240311031853893370"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_data_factory" "test" {
  name                = "acctestdfirm240311031853893370"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_credential_user_managed_identity" "test" {
  name            = "credential240311031853893370"
  data_factory_id = azurerm_data_factory.test.id
  identity_id     = azurerm_user_assigned_identity.test.id
}

resource "azurerm_sql_server" "test" {
  name                         = "acctestsql240311031853893370"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  version                      = "12.0"
...
resource "azurerm_data_factory_integration_runtime_azure_ssis" "test" {
  name            = "managed-integration-runtime-240311031853893370"
  data_factory_id = azurerm_data_factory.test.id
  location        = azurerm_resource_group.test.location
