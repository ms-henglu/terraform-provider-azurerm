...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-la-220818235327617051"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-220818235327617051"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctest-EHN-220818235327617051"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_eventhub" "test" {
  name                = "acctest-EH-220818235327617051"
...
resource "azurerm_log_analytics_data_export_rule" "test" {
  name                    = "acctest-DER-220818235327617051"
  resource_group_name     = azurerm_resource_group.test.name
