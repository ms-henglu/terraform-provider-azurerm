...
func resourceSynapseWorkspace() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		...
		Schema: map[string]*pluginsdk.Schema{
			...
			"linked_access_check_on_target_resource_enabled": {
				Type:     pluginsdk.TypeBool,
				Optional: true,
			},
...
func resourceSynapseWorkspaceCreate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	linkedAccessCheckOnTargetResourceEnabled := d.Get("linked_access_check_on_target_resource_enabled").(bool)
	if linkedAccessCheckOnTargetResourceEnabled {
		if workspaceInfo.ManagedVirtualNetworkSettings == nil {
			workspaceInfo.ManagedVirtualNetworkSettings = &synapse.ManagedVirtualNetworkSettings{}
		}
		workspaceInfo.ManagedVirtualNetworkSettings.LinkedAccessCheckOnTargetResource = utils.Bool(linkedAccessCheckOnTargetResourceEnabled)
	}
	...
	j, _ := json.Marshal(workspaceInfo)
	...
	if props := resp.WorkspaceProperties; props != nil {
		...
		if props.ManagedVirtualNetwork != nil && strings.EqualFold(*props.ManagedVirtualNetwork, "default") {
			...
			if props.ManagedVirtualNetworkSettings != nil {
				...
				d.Set("linked_access_check_on_target_resource_enabled", props.ManagedVirtualNetworkSettings.LinkedAccessCheckOnTargetResource)
...
func resourceSynapseWorkspaceUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.HasChanges("tags", "sql_administrator_login_password", "github_repo", "azure_devops_repo", "customer_managed_key_versionless_id") {
		...
		if linkedAccessCheckOnTargetResourceEnabled, ok := d.GetOk("linked_access_check_on_target_resource_enabled"); ok {
			settings := workspacePatchInfo.ManagedVirtualNetworkSettings
			if settings == nil {
				settings = &synapse.ManagedVirtualNetworkSettings{}
			}
			settings.LinkedAccessCheckOnTargetResource = utils.Bool(linkedAccessCheckOnTargetResourceEnabled.(bool))
		}
		if allowedLinkingTenantIds, ok := d.GetOk("allowed_linking_tenant_ids"); ok {
			settings := workspacePatchInfo.ManagedVirtualNetworkSettings
			if settings == nil {
			...
			settings.AllowedAadTenantIdsForLinking = utils.ExpandStringSlice(allowedLinkingTenantIds.([]interface{}))
		}
