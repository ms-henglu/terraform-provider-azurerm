...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-210826023411679707"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet210826023411679707"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip210826023411679707"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test_mgmt" {
  name                = "acctestmgmtpip210826023411679707"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  sku                 = "Standard"
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall210826023411679707"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
