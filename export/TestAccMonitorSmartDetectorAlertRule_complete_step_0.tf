...
resource "azurerm_resource_group" "test" {
  name     = "acctestrg-monitor-210928075711794188"
  location = "West Europe"
}

resource "azurerm_application_insights" "test" {
  name                = "acctestappinsights-210928075711794188"
...
resource "azurerm_monitor_action_group" "test" {
  name                = "acctestActionGroup-210928075711794188"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_monitor_smart_detector_alert_rule" "test" {
  name                = "acctestSDAR-210928075711794188"
  resource_group_name = azurerm_resource_group.test.name
  severity            = "Sev0"
