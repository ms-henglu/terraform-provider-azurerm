...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-220114064035708647"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvnet220114064035708647"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet220114064035708647"
  resource_group_name  = "${azurerm_resource_group.test.name}"
...
resource "azurerm_public_ip" "test1" {
  name                = "acctpip1220114064035708647"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctpip1220114064035708647"
}

resource "azurerm_public_ip" "test2" {
  name                = "acctpip2220114064035708647"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctpip2220114064035708647"
}
...
resource "azurerm_sql_server" "test" {
  name                         = "acctestsql220114064035708647"
  resource_group_name          = "${azurerm_resource_group.test.name}"
  location                     = "${azurerm_resource_group.test.location}"
  version                      = "12.0"
...
resource "azurerm_data_factory" "test" {
  name                = "acctestdfirm220114064035708647"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
}

resource "azurerm_data_factory_linked_custom_service" "test" {
  name                 = "acctestls220114064035708647"
...
resource "azurerm_data_factory_linked_custom_service" "file_share_linked_service" {
  name                 = "acctestls1220114064035708647"
  data_factory_id      = azurerm_data_factory.test.id
...
resource "azurerm_data_factory_linked_service_key_vault" "test" {
  name                = "acctestlinkkv220114064035708647"
  resource_group_name = azurerm_resource_group.test.name
  data_factory_id     = azurerm_data_factory.test.id
  key_vault_id        = azurerm_key_vault.test.id
}
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "test" {
  name                = "acctestSIRsh220114064035708647"
  data_factory_id     = azurerm_data_factory.test.id
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_data_factory_integration_runtime_azure_ssis" "test" {
  name                = "acctestiras220114064035708647"
  description         = "acctest"
  data_factory_id     = azurerm_data_factory.test.id
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
