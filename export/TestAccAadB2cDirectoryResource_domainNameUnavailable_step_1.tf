...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220610022144595963"
...
resource "azurerm_aadb2c_directory" "test" {
  ...
  display_name            = "acctest220610022144595963"
  domain_name             = "acctest220610022144595963.onmicrosoft.com"
...
resource "azurerm_resource_group" "duplicate" {
  name     = "acctestRG-duplicate-220610022144595963"
  location = "West US 2"
}
...
resource "azurerm_aadb2c_directory" "duplicate" {
  ...
  display_name            = "acctest-duplicate-220610022144595963"
