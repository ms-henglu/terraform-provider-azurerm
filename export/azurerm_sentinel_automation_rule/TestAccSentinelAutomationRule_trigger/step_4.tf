...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-sentinel-230324052724354952"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctest-workspace-230324052724354952"
...
resource "azurerm_api_connection" "test" {
  ...
  name                = "azuresentinel-230324052724354952"
  resource_group_name = azurerm_resource_group.test.name
  parameter_values = {
...
resource "azurerm_logic_app_workflow" "test" {
  name                = "acctestLogicApp-230324052724354952"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_sentinel_automation_rule" "test" {
  name                       = "5173ffb6-e21b-448b-8fe3-a56defcafb3b"
  log_analytics_workspace_id = azurerm_sentinel_log_analytics_workspace_onboarding.test.workspace_id
  display_name               = "acctest-SentinelAutoRule-230324052724354952-update"
  order                      = 1
  triggers_on                = "Alerts"
  action_playbook {
    logic_app_id = azurerm_logic_app_workflow.test.id
