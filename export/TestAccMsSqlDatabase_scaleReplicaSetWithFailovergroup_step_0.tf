...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-210924004612262789"
  location = "West Europe"
}

resource "azurerm_sql_server" "test" {
  name                         = "acctest-sqlserver-210924004612262789"
...
resource "azurerm_mssql_database" "test" {
  name         = "acctest-db-210924004612262789"
...
resource "azurerm_resource_group" "second" {
  name     = "acctestRG-mssql2-210924004612262789"
  location = "West US 2"
}

resource "azurerm_sql_server" "second" {
  name                         = "acctest-sqlserver2-210924004612262789"
...
resource "azurerm_sql_failover_group" "failover_group" {
  ...
  name                = "acctest-fog-210924004612262789"
  resource_group_name = azurerm_resource_group.test.name
