...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-recovery-220225034850745378-1"
  location = "West Europe"
}

resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-vault-220225034850745378"
...
resource "azurerm_site_recovery_fabric" "test1" {
  ...
  name                = "acctest-fabric1-220225034850745378"
  location            = azurerm_resource_group.test.location
}
...
resource "azurerm_site_recovery_fabric" "test2" {
  resource_group_name = azurerm_resource_group.test.name
  recovery_vault_name = azurerm_recovery_services_vault.test.name
  name                = "acctest-fabric2-220225034850745378"
  location            = "West US 2"
  depends_on          = [azurerm_site_recovery_fabric.test1]
}

resource "azurerm_virtual_network" "test1" {
  name                = "network1-220225034850745378"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test2" {
  name                = "network2-220225034850745378"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_site_recovery_network_mapping" "test" {
  resource_group_name         = azurerm_resource_group.test.name
  recovery_vault_name         = azurerm_recovery_services_vault.test.name
  name                        = "mapping-220225034850745378"
