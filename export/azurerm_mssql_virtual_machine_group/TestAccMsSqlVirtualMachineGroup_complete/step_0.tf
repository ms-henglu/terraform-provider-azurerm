...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-231020041508309306"
  location = "West Europe"
}

resource "azurerm_storage_account" "test" {
  name                     = "unlikely23exst2acctlomar"
...
resource "azurerm_mssql_virtual_machine_group" "test" {
  ...
  wsfc_domain_profile {
    ...
    cluster_bootstrap_account_name = "bootstrapacclomar"
    cluster_operator_account_name  = "opacclomar"
    sql_service_account_name       = "sqlsrvacclomar"
