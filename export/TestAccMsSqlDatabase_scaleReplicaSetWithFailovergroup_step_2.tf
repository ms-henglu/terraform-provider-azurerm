...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-210825045016131443"
  location = "West Europe"
}

resource "azurerm_sql_server" "test" {
  name                         = "acctest-sqlserver-210825045016131443"
...
resource "azurerm_mssql_database" "test" {
  name         = "acctest-db-210825045016131443"
...
resource "azurerm_resource_group" "second" {
  name     = "acctestRG-mssql2-210825045016131443"
  location = "West US 2"
}

resource "azurerm_sql_server" "second" {
  name                         = "acctest-sqlserver2-210825045016131443"
...
resource "azurerm_sql_failover_group" "failover_group" {
  ...
  name                = "acctest-fog-210825045016131443"
  resource_group_name = azurerm_resource_group.test.name
