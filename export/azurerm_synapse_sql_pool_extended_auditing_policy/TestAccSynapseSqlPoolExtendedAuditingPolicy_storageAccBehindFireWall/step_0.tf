...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-synapse-231016034855245282"
...
resource "azurerm_storage_data_lake_gen2_filesystem" "test" {
  name               = "acctest-231016034855245282"
  storage_account_id = azurerm_storage_account.sw.id
}

resource "azurerm_synapse_workspace" "test" {
  name                                 = "acctestsw231016034855245282"
  resource_group_name                  = azurerm_resource_group.test.name
  location                             = azurerm_resource_group.test.location
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet231016034855245282"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet231016034855245282"
  resource_group_name  = azurerm_resource_group.test.name
