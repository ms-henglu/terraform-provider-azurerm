...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-dtwin-220630223639455955"
  location = "West Europe"
}


resource "azurerm_digital_twins_instance" "test" {
  name                = "acctest-DT-220630223639455955"
...
resource "azurerm_servicebus_namespace" "test" {
  name                = "acctestservicebusnamespace-220630223639455955"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_servicebus_topic" "test" {
  name         = "acctestservicebustopic-220630223639455955"
  namespace_id = azurerm_servicebus_namespace.test.id
}

resource "azurerm_servicebus_topic_authorization_rule" "test" {
  name     = "acctest-rule-220630223639455955"
...
resource "azurerm_digital_twins_endpoint_servicebus" "test" {
  name                                   = "acctest-EndpointSB-220630223639455955"
