...
func (r NetworkManagerResource) Exists(ctx context.Context, client *clients.Client, state *terraform.InstanceState) (*bool, error) {
	...
	if err != nil {
		...
		return nil, fmt.Errorf("retrieving Network Manager %q (Resource Group %q): %+v", id.Name, id.ResourceGroup, err)
	}
...
resource "azurerm_network_manager" "test" {
  ...
  network_manager_scope_accesses = ["Security", "Routing", "Connectivity"]
