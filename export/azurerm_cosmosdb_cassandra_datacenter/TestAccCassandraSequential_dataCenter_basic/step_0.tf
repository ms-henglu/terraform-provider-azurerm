...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cassandra-231013043220265010"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-231013043220265010"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-231013043220265010"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_cosmosdb_cassandra_cluster" "test" {
  name                           = "acctca-mi-cluster-231013043220265010"
  resource_group_name            = azurerm_resource_group.test.name
  location                       = azurerm_resource_group.test.location
...
resource "azurerm_cosmosdb_cassandra_datacenter" "test" {
  name                           = "acctca-mi-dc-231013043220265010"
