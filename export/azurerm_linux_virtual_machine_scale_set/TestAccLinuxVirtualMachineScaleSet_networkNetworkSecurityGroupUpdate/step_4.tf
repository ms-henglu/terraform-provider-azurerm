...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240315122549946242"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-240315122549946242"
...
resource "azurerm_network_security_group" "test" {
  name                = "acctestnsg-240315122549946242"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_network_security_group" "other" {
  name                = "acctestnsg2-240315122549946242"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-240315122549946242"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard_F2"
  instances           = 1
