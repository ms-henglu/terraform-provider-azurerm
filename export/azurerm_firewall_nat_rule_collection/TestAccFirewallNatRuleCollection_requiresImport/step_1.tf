...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-230707003930999056"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet230707003930999056"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip230707003930999056"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall230707003930999056"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "AZFW_VNet"
  sku_tier            = "Standard"
...
resource "azurerm_firewall_nat_rule_collection" "test" {
  name                = "acctestnrc-230707003930999056"
