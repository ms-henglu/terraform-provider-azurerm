...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-CAE-230421021859972145"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-230421021859972145"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet230421021859972145"
...
resource "azurerm_container_app_environment" "test" {
  name                       = "accTest-CAEnv230421021859972145"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
...
resource "azurerm_container_registry" "test" {
  name                = "testacccr230421021859972145"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Basic"
  admin_enabled       = true
...
resource "azurerm_container_app_environment_storage" "test" {
  name                         = "testacc-caes-230421021859972145"
...
resource "azurerm_container_app" "test" {
  name                         = "acctest-capp-230421021859972145"
  resource_group_name          = azurerm_resource_group.test.name
  container_app_environment_id = azurerm_container_app_environment.test.id
  ...
  template {
    container {
      name   = "acctest-cont-230421021859972145"
