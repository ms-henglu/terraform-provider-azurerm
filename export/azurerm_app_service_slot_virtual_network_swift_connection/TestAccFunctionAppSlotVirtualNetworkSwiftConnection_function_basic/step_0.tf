...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-functionapp-221222035504616845"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-VNET-221222035504616845"
...
resource "azurerm_app_service_plan" "test" {
  name                = "acctest-ASP-221222035504616845"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_function_app" "test" {
  name                       = "acctest-FA-221222035504616845"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
...
resource "azurerm_function_app_slot" "test-staging" {
  name                       = "acctest-FA-221222035504616845-staging"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
  app_service_plan_id        = azurerm_app_service_plan.test.id
