...
resource "azurerm_resource_group" "test" {
  name     = "acctestRGhdi-220726014852713168"
...
resource "azurerm_storage_container" "test" {
  name                  = "acctestsc-220726014852713168"
  storage_account_name  = azurerm_storage_account.test.name
  container_access_type = "private"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestVnet-220726014852713168"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "test" {
  name                 = "acctestSubnet-220726014852713168"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "acctestNSG-220726014852713168"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azuread_group" "test" {
  display_name     = "AAD DC Administrators bwopg"
...
resource "azuread_user" "test" {
  user_principal_name = "acctestAADDSAdminUser-bwopg@${data.azuread_domains.test.domains.0.domain_name}"
  display_name        = "acctestAADDSAdminUser-bwopg"
...
resource "azurerm_hdinsight_spark_cluster" "test" {
  name                = "acctesthdispark-220726014852713168"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  cluster_version     = "4.0"
  tier                = "Premium"
