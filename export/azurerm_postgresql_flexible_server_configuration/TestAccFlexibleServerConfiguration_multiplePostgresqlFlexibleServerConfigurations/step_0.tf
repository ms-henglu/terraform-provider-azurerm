...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-postgresql-230519075421556842"
  location = "West Europe"
}


resource "azurerm_virtual_network" "test" {
  name                = "acctest-vn-230519075421556842"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-sn-230519075421556842"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_private_dns_zone" "test" {
  name                = "acc230519075421556842.postgres.database.azure.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_private_dns_zone_virtual_network_link" "test" {
  name                  = "acctestVnetZone230519075421556842.com"
...
resource "azurerm_postgresql_flexible_server" "test" {
  name                   = "acctest-fs-230519075421556842"
  resource_group_name    = azurerm_resource_group.test.name
  location               = azurerm_resource_group.test.location
