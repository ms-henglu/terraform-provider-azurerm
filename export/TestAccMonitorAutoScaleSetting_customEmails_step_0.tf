...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-210928055705404178"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-210928055705404178"
...
resource "azurerm_virtual_machine_scale_set" "test" {
  name                = "acctvmss-210928055705404178"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    computer_name_prefix = "testvm-210928055705404178"
  ...
  network_profile {
    name    = "TestNetworkProfile-210928055705404178"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-210928055705404178"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-210928055705404178@example.com"]
    }
  }
}
