...
resource "azurerm_network_interface" "client_single_subnet" {
  ...
  name                = "acctestnic-client-${count.index}-240112224909465177"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_availability_set" "test" {
  name                = "acctestavset-240112224909465177"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_windows_virtual_machine" "client" {
  ...
  name                = "acctest-${count.index}-1huiy"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  size                = "Standard_F2"
  admin_username      = local.admin_username
