...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-OVMSS-220225034148131094"
  location = "West Europe"
}


resource "azurerm_public_ip" "test" {
  name                = "acctpip-220225034148131094"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctvn-220225034148131094"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-220225034148131094"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_nat_gateway" "test" {
  name                    = "acctng-220225034148131094"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
  sku_name                = "Standard"
...
resource "azurerm_orchestrated_virtual_machine_scale_set" "test" {
  name                = "acctestOVMSS-220225034148131094"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    linux_configuration {
      computer_name_prefix = "testvm-220225034148131094"
  ...
  network_interface {
    name    = "TestNetworkProfile-220225034148131094"
    primary = true
