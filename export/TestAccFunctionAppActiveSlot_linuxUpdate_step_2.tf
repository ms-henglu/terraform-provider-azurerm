...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220527023834434143"
...
resource "azurerm_service_plan" "test" {
  name                = "acctestASP-WAS-220527023834434143"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  os_type             = "Linux"
  sku_name            = "EP1"
...
resource "azurerm_linux_function_app" "test" {
  name                = "acctestWA-220527023834434143"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_function_app_slot" "test" {
  name                       = "acctest-LFAS-220527023834434143"
...
resource "azurerm_linux_function_app_slot" "update" {
  name                       = "acctestWAS2-220527023834434143"
  function_app_id            = azurerm_linux_function_app.test.id
  storage_account_name       = azurerm_storage_account.test.name
  storage_account_access_key = azurerm_storage_account.test.primary_access_key
