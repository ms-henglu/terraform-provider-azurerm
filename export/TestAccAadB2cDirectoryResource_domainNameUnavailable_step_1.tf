...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220916011027416023"
...
resource "azurerm_aadb2c_directory" "test" {
  ...
  display_name            = "acctest220916011027416023"
  domain_name             = "acctest220916011027416023.onmicrosoft.com"
...
resource "azurerm_resource_group" "duplicate" {
  name     = "acctestRG-duplicate-220916011027416023"
  location = "West US 2"
}
...
resource "azurerm_aadb2c_directory" "duplicate" {
  ...
  display_name            = "acctest-duplicate-220916011027416023"
