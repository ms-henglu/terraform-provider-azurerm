...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-231013044237022603"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctestcosmosdb231013044237022603"
...
resource "azurerm_app_service_plan" "test" {
  name                = "acctestASP-231013044237022603"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_function_app" "test" {
  name                       = "acctest-231013044237022603-func"
  location                   = azurerm_resource_group.test.location
  resource_group_name        = azurerm_resource_group.test.name
...
resource "azurerm_cosmosdb_sql_container" "update" {
  name                = "test-containerupdatevecxv"
  resource_group_name = azurerm_resource_group.test.name
  account_name        = azurerm_cosmosdb_account.test.name
  database_name       = azurerm_cosmosdb_sql_database.test.name
...
resource "azurerm_service_plan" "update" {
  location            = azurerm_resource_group.test.location
  name                = "testserviceplanupdatevecxv"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "P1v2"
  os_type             = "Linux"
}
...
resource "azurerm_function_app_connection" "test" {
  name               = "acctestserviceconnector231013044237022603"
