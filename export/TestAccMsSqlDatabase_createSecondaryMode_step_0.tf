...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-210830084244235071"
  location = "West Europe"
}

resource "azurerm_sql_server" "test" {
  name                         = "acctest-sqlserver-210830084244235071"
...
resource "azurerm_mssql_database" "test" {
  name         = "acctest-db-210830084244235071"
...
resource "azurerm_resource_group" "second" {
  name     = "acctestRG-mssql2-210830084244235071"
  location = "West US 2"
}

resource "azurerm_sql_server" "second" {
  name                         = "acctest-sqlserver2-210830084244235071"
...
resource "azurerm_mssql_database" "secondary" {
  name                        = "acctest-dbs-210830084244235071"
