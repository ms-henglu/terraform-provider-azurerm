...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-backup-210928075830569590"
...
resource "azurerm_storage_share" "testshare1" {
  name                 = "acctest-ss-210928075830569590-1"
...
resource "azurerm_storage_share" "testshare2" {
  name                 = "acctest-ss-210928075830569590-2"
  storage_account_name = "${azurerm_storage_account.test1.name}"
  metadata             = {}
...
resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-VAULT-210928075830569590"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
  sku                 = "Standard"
...
resource "azurerm_backup_policy_file_share" "test" {
  name                = "acctest-PFS-210928075830569590"
  resource_group_name = "${azurerm_resource_group.test.name}"
