...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220121044643476043"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvnet-220121044643476043"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet-220121044643476043"
...
resource "azurerm_lb" "test" {
  name                = "acctestlb-220121044643476043"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Gateway"
...
resource "azurerm_public_ip" "c1" {
  name                = "acctestpip1-220121044643476043"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
...
resource "azurerm_lb" "c1" {
  name                = "acctestlb-consumer1-220121044643476043"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  frontend_ip_configuration {
...
resource "azurerm_public_ip" "c2" {
  name                = "acctestpip2-220121044643476043"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  allocation_method   = "Static"
...
resource "azurerm_lb" "c2" {
  name                = "acctestlb-consumer2-220121044643476043"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  frontend_ip_configuration {
...
resource "azurerm_lb_backend_address_pool" "test1" {
  name            = "acctestbap1-220121044643476043"
...
resource "azurerm_lb_backend_address_pool" "test2" {
  name            = "acctestbap2-220121044643476043"
  loadbalancer_id = azurerm_lb.test.id
  tunnel_interface {
    identifier = 901
    type       = "External"
