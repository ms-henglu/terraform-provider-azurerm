...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230414022151249112"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctestaccl01l9"
...
resource "azurerm_cosmosdb_sql_container" "update" {
  name                = "test-containerupdatel01l9"
  resource_group_name = azurerm_cosmosdb_account.test.resource_group_name
  account_name        = azurerm_cosmosdb_account.test.name
...
resource "azurerm_service_plan" "update" {
  location            = azurerm_resource_group.test.location
  name                = "testserviceplanupdatel01l9"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "P1v2"
  os_type             = "Linux"
}
...
resource "azurerm_app_service_connection" "test" {
  name               = "acctestserviceconnector230414022151249112"
