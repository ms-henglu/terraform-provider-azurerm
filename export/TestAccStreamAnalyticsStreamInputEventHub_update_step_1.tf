...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220520041251039244"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctestehn-220520041251039244"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteh-220520041251039244"
...
resource "azurerm_eventhub_consumer_group" "test" {
  name                = "acctesteventhubcg-220520041251039244"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-220520041251039244"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_eventhub_namespace" "updated" {
  name                = "acctestehn2-220520041251039244"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  capacity            = 1
...
resource "azurerm_eventhub" "updated" {
  name                = "acctesteh2-220520041251039244"
...
resource "azurerm_eventhub_consumer_group" "updated" {
  name                = "acctesteventhubcg2-220520041251039244"
  namespace_name      = azurerm_eventhub_namespace.updated.name
...
resource "azurerm_stream_analytics_stream_input_eventhub" "test" {
  name                         = "acctestinput-220520041251039244"
