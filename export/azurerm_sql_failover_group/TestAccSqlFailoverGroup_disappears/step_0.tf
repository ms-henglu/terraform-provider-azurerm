...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230818024832096662"
  location = "West Europe"
}

resource "azurerm_sql_server" "test_primary" {
  name                         = "acctestmssql230818024832096662-primary"
...
resource "azurerm_sql_server" "test_secondary" {
  name                         = "acctestmssql230818024832096662-secondary"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = "West US 2"
  version                      = "12.0"
  administrator_login          = "mradministrator"
...
resource "azurerm_sql_database" "test" {
  name                             = "acctestdb230818024832096662"
  resource_group_name              = azurerm_resource_group.test.name
...
resource "azurerm_sql_failover_group" "test" {
  name                = "acctestsfg230818024832096662"
  resource_group_name = azurerm_resource_group.test.name
  server_name         = azurerm_sql_server.test_primary.name
