...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-storage-220712042346318696"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-VN-220712042346318696"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsub-220712042346318696"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_resource_group_template_deployment" "storage-containers" {
  name                = "acctest-strgctn-deployment-220712042346318696"
  resource_group_name = azurerm_storage_account.test.resource_group_name
  deployment_mode     = "Incremental"

  parameters_content = jsonencode({
    name = {
      value = "acctest-strgctn-hpc-220712042346318696"
    }
  })
...
resource "azurerm_hpc_cache" "test" {
  name                = "acctest-HPCC-220712042346318696"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
