...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-spring-220722040024997997"
  location = "West Europe"
}

resource "azurerm_spring_cloud_service" "test" {
  name                = "acctest-sc-220722040024997997"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_spring_cloud_app" "test" {
  name                = "acctest-sca-220722040024997997"
  resource_group_name = azurerm_spring_cloud_service.test.resource_group_name
  service_name        = azurerm_spring_cloud_service.test.name
}

resource "azurerm_mysql_server" "test" {
  name                             = "acctestmysqlsvr-220722040024997997"
  location                         = azurerm_resource_group.test.location
  resource_group_name              = azurerm_resource_group.test.name
  sku_name                         = "GP_Gen5_2"
...
resource "azurerm_mysql_database" "test" {
  name                = "acctest-db-220722040024997997"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_spring_cloud_app_mysql_association" "test" {
  name                = "acctestscamb-220722040024997997"
