...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-240105060637585703"
...
resource "azurerm_storage_data_lake_gen2_filesystem" "test" {
  name               = "acctest-datalake-240105060637585703"
  storage_account_id = azurerm_storage_account.test.id
}

resource "azurerm_data_factory" "test" {
  name                = "acctestdf240105060637585703"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_linked_service_data_lake_storage_gen2" "test" {
  name                 = "acctestDataLakeStorage240105060637585703"
...
resource "azurerm_data_factory_dataset_delimited_text" "test" {
  name                = "acctestds240105060637585703"
  data_factory_id     = azurerm_data_factory.test.id
