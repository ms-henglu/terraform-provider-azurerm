...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-network-manager-230609091743198946"
...
resource "azurerm_network_manager" "test" {
  name                = "acctest-nm-230609091743198946"
...
resource "azurerm_network_manager_network_group" "test" {
  name               = "acctest-nmng-230609091743198946"
  network_manager_id = azurerm_network_manager.test.id
}

resource "azurerm_virtual_network" "test" {
  name                    = "acctest-vnet-230609091743198946"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_network_manager_network_group" "test2" {
  name               = "acctest-nmng2-230609091743198946"
  network_manager_id = azurerm_network_manager.test.id
}

resource "azurerm_network_manager_connectivity_configuration" "test" {
  name                            = "acctest-nmcc-230609091743198946"
  network_manager_id              = azurerm_network_manager.test.id
