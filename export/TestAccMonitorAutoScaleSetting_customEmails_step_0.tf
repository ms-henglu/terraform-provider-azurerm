...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220128082706832210"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-220128082706832210"
...
resource "azurerm_virtual_machine_scale_set" "test" {
  name                = "acctvmss-220128082706832210"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    computer_name_prefix = "testvm-220128082706832210"
  ...
  network_profile {
    name    = "TestNetworkProfile-220128082706832210"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-220128082706832210"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-220128082706832210@example.com"]
    }
  }
}
