...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ml-221222034921999342"
...
resource "azurerm_application_insights" "test" {
  name                = "acctestai-221222034921999342"
...
resource "azurerm_machine_learning_workspace" "test" {
  name                    = "acctest-MLW2212220349219993"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet221222034921999342"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet221222034921999342"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks221222034921999342"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = join("", ["acctestaks", azurerm_resource_group.test.location])
  node_resource_group = "acctestRGAKS-221222034921999342"
