...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230324052437896517"
  location = "West Europe"
}

resource "azurerm_storage_account" "test" {
  name                     = "acctestsa11rx07"
...
resource "azurerm_monitor_action_group" "test1" {
  name                = "acctestActionGroup1-230324052437896517"
  resource_group_name = azurerm_resource_group.test.name
  short_name          = "acctestag1"
}

resource "azurerm_monitor_action_group" "test2" {
  name                = "acctestActionGroup2-230324052437896517"
  resource_group_name = azurerm_resource_group.test.name
  short_name          = "acctestag2"
}

resource "azurerm_monitor_metric_alert" "test" {
  name                = "acctestMetricAlert-230324052437896517"
  resource_group_name = azurerm_resource_group.test.name
