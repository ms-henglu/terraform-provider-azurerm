...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-211001053637610462"
  location = "West Europe"
}

resource "azurerm_data_factory" "test" {
  name                = "acctestdf211001053637610462"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_data_factory_pipeline" "test" {
  name                = "acctestdfp211001053637610462"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_trigger_tumbling_window" "dependency" {
  name            = "acctestdft2211001053637610462"
...
resource "azurerm_data_factory_trigger_tumbling_window" "test" {
  name            = "acctestdft211001053637610462"
  data_factory_id = azurerm_data_factory.test.id
  start_time      = "2022-09-21T00:00:00Z"
