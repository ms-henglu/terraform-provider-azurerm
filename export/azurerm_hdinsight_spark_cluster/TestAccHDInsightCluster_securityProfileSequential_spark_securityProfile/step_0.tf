...
resource "azurerm_resource_group" "test" {
  name     = "acctestRGhdi-230630033251812156"
...
resource "azurerm_storage_container" "test" {
  name                  = "acctestsc-230630033251812156"
  storage_account_name  = azurerm_storage_account.test.name
  container_access_type = "private"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestVnet-230630033251812156"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "test" {
  name                 = "acctestSubnet-230630033251812156"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "acctestNSG-230630033251812156"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azuread_group" "test" {
  display_name     = "AAD DC Administrators 6c3nh"
...
resource "azuread_user" "test" {
  user_principal_name = "acctestAADDSAdminUser-6c3nh@${data.azuread_domains.test.domains.0.domain_name}"
  display_name        = "acctestAADDSAdminUser-6c3nh"
...
resource "azurerm_hdinsight_spark_cluster" "test" {
  name                = "acctesthdispark-230630033251812156"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  cluster_version     = "4.0"
  tier                = "Premium"
