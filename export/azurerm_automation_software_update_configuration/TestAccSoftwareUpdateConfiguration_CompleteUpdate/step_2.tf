...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-240311031415484601"
  location = "West Europe"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-240311031415484601"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-240311031415484601"
  location            = "west europe"
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-240311031415484601"
  ...
  schedule {
    ...
    expiry_time        = "2024-03-11T05:14:00Z"
    is_enabled         = true
    interval           = 2
    frequency          = "Hour"
    time_zone          = "Etc/UTC"
