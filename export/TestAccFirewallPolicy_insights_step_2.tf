...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-networkfw-220114014249987124"
  location = "West Europe"
}


resource "azurerm_log_analytics_workspace" "default" {
  name                = "acctestLAW-220114014249987124"
...
resource "azurerm_log_analytics_workspace" "regional" {
  name                = "acctestLAW-region-220114014249987124"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
  retention_in_days   = 30
...
resource "azurerm_firewall_policy" "test" {
  name                = "acctest-networkfw-Policy-220114014249987124"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  insights {
    enabled                            = true
