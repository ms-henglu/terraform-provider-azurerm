...
resource "azurerm_resource_group" "test1" {
  name     = "acctestRG-api1-230602030057648051"
  location = "West Europe"
}

resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-api2-230602030057648051"
  location = "West US 2"
}

resource "azurerm_resource_group" "test3" {
  name     = "acctestRG-api3-230602030057648051"
  location = "East US 2"
}

resource "azurerm_api_management" "test" {
  name                      = "acctestAM-230602030057648051"
  publisher_name            = "pub1"
  ...
  hostname_configuration {
    proxy {
      host_name                    = "acctestAM-230602030057648051.azure-api.net"
