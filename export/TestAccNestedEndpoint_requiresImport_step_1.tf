...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-traffic-220520041302787088"
  location = "West Europe"
}

resource "azurerm_traffic_manager_profile" "parent" {
  name                   = "acctest-TMP-220520041302787088"
  resource_group_name    = azurerm_resource_group.test.name
  traffic_routing_method = "Weighted"

  dns_config {
    relative_name = "acctest-tmp-220520041302787088"
    ttl           = 30
  }
...
resource "azurerm_traffic_manager_profile" "child" {
  name                   = "acctesttmpchild220520041302787088"
  resource_group_name    = azurerm_resource_group.test.name
  traffic_routing_method = "Priority"

  dns_config {
    relative_name = "acctesttmpchild220520041302787088"
    ttl           = 30
  }
...
resource "azurerm_traffic_manager_nested_endpoint" "test" {
  name                    = "acctestend-parent220520041302787088"
