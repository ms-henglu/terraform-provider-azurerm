...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-wps-220326011234439602"
  location = "West Europe"
}

resource "azurerm_web_pubsub" "test" {
  name                = "acctest-webpubsub-220326011234439602"
...
resource "azurerm_user_assigned_identity" "test1" {
  name                = "acctest-uai1-220326011234439602"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
resource "azurerm_user_assigned_identity" "test2" {
  name                = "acctest-uai2-220326011234439602"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
resource "azurerm_web_pubsub_hub" "test" {
  name          = "acctestwpsh220326011234439602"
