...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mapp-210924011158273349"
  location = "West Europe"
}

resource "azurerm_managed_application_definition" "test" {
  name                = "acctestManagedAppDef210924011158273349"
...
resource "azurerm_managed_application" "test" {
  name                        = "acctestCompleteManagedApp210924011158273349"
  location                    = azurerm_resource_group.test.location
  resource_group_name         = azurerm_resource_group.test.name
  kind                        = "MarketPlace"
  managed_resource_group_name = "completeInfraGroup210924011158273349"
