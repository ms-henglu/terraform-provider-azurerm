...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220114063750745462"
...
resource "azurerm_aadb2c_directory" "test" {
  ...
  display_name            = "acctest220114063750745462"
  domain_name             = "acctest220114063750745462.onmicrosoft.com"
...
resource "azurerm_resource_group" "duplicate" {
  name     = "acctestRG-duplicate-220114063750745462"
  location = "West US 2"
}
...
resource "azurerm_aadb2c_directory" "duplicate" {
  ...
  display_name            = "acctest-duplicate-220114063750745462"
