...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-diskspool-220610092629250306"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-220610092629250306"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-subnet-220610092629250306"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_managed_disk" "test" {
  ...
  name                 = "acctest-diskspool-220610092629250306${count.index}"
  resource_group_name  = azurerm_resource_group.test.name
  location             = azurerm_resource_group.test.location
  create_option        = "Empty"
...
resource "azurerm_disk_pool_iscsi_target_lun" "test0" {
...
resource "azurerm_disk_pool_iscsi_target_lun" "test1" {
  iscsi_target_id            = azurerm_disk_pool_iscsi_target.test.id
  disk_pool_managed_disk_attachment_id = azurerm_disk_pool_managed_disk_attachment.test[1].id
  name                       = "test-1"
}
