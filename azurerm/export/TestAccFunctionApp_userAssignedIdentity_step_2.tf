...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220627132454058152"
...
resource "azurerm_app_service_plan" "test" {
  name                = "acctestASP-220627132454058152"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_user_assigned_identity" "first" {
  name                = "acctest1220627132454058152"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_user_assigned_identity" "second" {
  name                = "acctest2220627132454058152"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_function_app" "test" {
  name                       = "acctest-220627132454058152-func"
  location                   = azurerm_resource_group.test.location
  resource_group_name        = azurerm_resource_group.test.name
