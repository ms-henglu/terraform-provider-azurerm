...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211008044703655855"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-211008044703655855"
...
resource "azurerm_virtual_machine_scale_set" "test" {
  name                = "acctvmss-211008044703655855"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    computer_name_prefix = "testvm-211008044703655855"
  ...
  network_profile {
    name    = "TestNetworkProfile-211008044703655855"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-211008044703655855"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-211008044703655855@example.com"]
    }
  }
}
