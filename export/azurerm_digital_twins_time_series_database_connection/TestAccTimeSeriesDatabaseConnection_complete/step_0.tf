...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-digitaltwin-230316221446697878"
  location = "West Europe"
}

resource "azurerm_digital_twins_instance" "test" {
  name                = "acctest-DT-230316221446697878"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-230316221446697878"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-230316221446697878"
...
resource "azurerm_kusto_database" "test" {
  name                = "acctestkd-230316221446697878"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_kusto_database_principal_assignment" "test" {
  name                = "acctestkdpa230316221446697878"
  resource_group_name = azurerm_resource_group.test.name
  cluster_name        = azurerm_kusto_cluster.test.name
...
resource "azurerm_eventhub_consumer_group" "test" {
  name                = "acctesteventhubcg-230316221446697878"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_digital_twins_time_series_database_connection" "test" {
  name                            = "connection-230316221446697878"
