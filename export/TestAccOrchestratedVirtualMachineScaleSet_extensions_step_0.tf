...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-OVMSS-220722035002601243"
  location = "West Europe"
}


resource "azurerm_public_ip" "test" {
  name                = "acctpip-220722035002601243"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctvn-220722035002601243"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-220722035002601243"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_nat_gateway" "test" {
  name                    = "acctng-220722035002601243"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
  sku_name                = "Standard"
...
resource "azurerm_orchestrated_virtual_machine_scale_set" "test" {
  name                = "acctestOVMSS-220722035002601243"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    linux_configuration {
      computer_name_prefix = "testvm-220722035002601243"
