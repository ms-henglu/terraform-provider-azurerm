...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220506020643491725"
  location = "West Europe"
}

resource "azurerm_app_service_plan" "test" {
  name                = "acctestASP-220506020643491725"
...
resource "azurerm_app_service" "test" {
  name                = "accestAS-220506020643491725"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_key_vault_certificate" "test" {
  name         = "acctest-AS-220506020643491725"
  ...
  certificate_policy {
    ...
    x509_certificate_properties {
      ...
      subject            = "CN=staging.accestAS-220506020643491725.ARM_TEST_DNS_ZONE"
...
resource "azurerm_app_service_certificate" "test" {
  name                = "acctestCert-220506020643491725"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_app_service_slot_custom_hostname_binding" "test" {
  ...
  hostname            = "staging.accestAS-220506020643491725.ARM_TEST_DNS_ZONE"
