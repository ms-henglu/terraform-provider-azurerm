...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-spring-230825025330328035"
  location = "West Europe"
}

resource "azurerm_spring_cloud_service" "test" {
  name                = "acctest-sc-230825025330328035"
...
resource "azurerm_spring_cloud_storage" "test1" {
  name                    = "acctest-test1-230825025330328035"
...
resource "azurerm_spring_cloud_storage" "test2" {
  name                    = "acctest-test2-230825025330328035"
  spring_cloud_service_id = azurerm_spring_cloud_service.test.id
...
resource "azurerm_spring_cloud_app" "test" {
  name                = "acctest-sca-230825025330328035"
