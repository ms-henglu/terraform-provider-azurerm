...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220513023611418957"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-220513023611418957"
...
resource "azurerm_subnet" "test" {
  name                                          = "subnet-220513023611418957"
  resource_group_name                           = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test" {
  name                = "acctest-pubip-220513023611418957"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_user_assigned_identity" "test" {
  ...
  name = "acctest220513023611418957"
}

resource "azurerm_public_ip" "testStd" {
  name                = "acctest-PubIpStd-220513023611418957"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_application_gateway" "test" {
  name                = "acctestag-220513023611418957"
  resource_group_name = "${azurerm_resource_group.test.name}"
  location            = "${azurerm_resource_group.test.location}"
