...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-230707010050125618"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF230707010050125618"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone230707010050125618.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-230707010050125618"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-230707010050125618"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol230707010050125618"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
