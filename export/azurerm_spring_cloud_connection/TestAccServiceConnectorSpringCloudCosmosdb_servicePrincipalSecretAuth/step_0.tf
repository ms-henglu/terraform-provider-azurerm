...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230810144227373359"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctestacc4e9jc"
...
resource "azurerm_spring_cloud_service" "test" {
  name                = "testspringcloudservice-4e9jc"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_spring_cloud_app" "test" {
  name                = "testspringcloud-4e9jc"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_spring_cloud_connection" "test" {
  name               = "acctestserviceconnector230810144227373359"
