...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-spring-240112225309486060"
  location = "West Europe"
}

resource "azurerm_spring_cloud_service" "test" {
  name                     = "acctest-sc-240112225309486060"
...
resource "azurerm_spring_cloud_app" "test" {
  name                = "acctest-sca-240112225309486060"
...
resource "azurerm_spring_cloud_build_deployment" "test" {
  ...
  addon_json = jsonencode({
    applicationConfigurationService = {
      configFilePatterns = "auth-service"
    }
  })
}
