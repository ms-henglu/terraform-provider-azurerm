...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-sentinel-230414022111367985"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctest-workspace-230414022111367985"
...
resource "azurerm_api_connection" "test" {
  ...
  name                = "azuresentinel-230414022111367985"
  resource_group_name = azurerm_resource_group.test.name
  parameter_values = {
...
resource "azurerm_logic_app_workflow" "test" {
  name                = "acctestLogicApp-230414022111367985"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_sentinel_automation_rule" "test" {
  name                       = "7c408142-a307-4f31-8b72-35b471c67ce2"
  log_analytics_workspace_id = azurerm_sentinel_log_analytics_workspace_onboarding.test.workspace_id
  display_name               = "acctest-SentinelAutoRule-230414022111367985-update"
  order                      = 1
  triggers_on                = "Alerts"
  action_playbook {
    logic_app_id = azurerm_logic_app_workflow.test.id
