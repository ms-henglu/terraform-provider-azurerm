...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-221124181249188482"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-221124181249188482"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-221124181249188482"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-221124181249188482"
  ...
  schedule {
    ...
    start_time          = "2022-11-25T04:12:49Z"
    expiry_time         = "2022-11-26T20:12:49Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
