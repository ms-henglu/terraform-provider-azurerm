...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220818235431822475"
  location = "West Europe"
}

resource "azurerm_mssql_server" "test_primary" {
  name                         = "acctestmssql220818235431822475-primary"
...
resource "azurerm_mssql_server" "test_secondary" {
  name                         = "acctestmssql220818235431822475-secondary"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = "West US 2"
  version                      = "12.0"
  administrator_login          = "mradministrator"
...
resource "azurerm_mssql_database" "test" {
  name        = "acctestdb220818235431822475"
...
resource "azurerm_mssql_failover_group" "test" {
  name      = "acctestsfg220818235431822475"
  server_id = azurerm_mssql_server.test_primary.id
