...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-lslp-230203063544569753"
  location = "West Europe"
}


resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig230203063544569753"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-230203063544569753"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-sn-230203063544569753"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_lab_service_plan" "test" {
  name                      = "acctest-lslp-230203063544569753"
  resource_group_name       = azurerm_resource_group.test.name
  location                  = azurerm_resource_group.test.location
