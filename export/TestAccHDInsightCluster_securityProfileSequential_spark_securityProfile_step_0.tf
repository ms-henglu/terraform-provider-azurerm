...
resource "azurerm_resource_group" "test" {
  name     = "acctestRGhdi-211105030047455933"
...
resource "azurerm_storage_container" "test" {
  name                  = "acctestsc-211105030047455933"
  storage_account_name  = azurerm_storage_account.test.name
  container_access_type = "private"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestVnet-211105030047455933"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "test" {
  name                 = "acctestSubnet-211105030047455933"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "acctestNSG-211105030047455933"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azuread_group" "test" {
  display_name = "AAD DC Administrators cjmq1"
...
resource "azuread_user" "test" {
  user_principal_name = "acctestAADDSAdminUser-cjmq1@${data.azuread_domains.test.domains.0.domain_name}"
  display_name        = "acctestAADDSAdminUser-cjmq1"
...
resource "azurerm_hdinsight_spark_cluster" "test" {
  name                = "acctesthdispark-211105030047455933"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  cluster_version     = "4.0"
  tier                = "Premium"
