...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211105030644617882"
  location = "West Europe"
}

resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-211105030644617882"
...
resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-211105030644617882"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  version                      = "12.0"
...
resource "azurerm_mssql_database" "test" {
  name      = "acctest-db-211105030644617882"
...
resource "azurerm_stream_analytics_reference_input_mssql" "test" {
  name                      = "acctestinput-211105030644617882"
