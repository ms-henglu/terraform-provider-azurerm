...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230421022903122628"
  location = "West Europe"
}

resource "azurerm_servicebus_namespace" "test" {
  name                = "acctestservicebusnamespace-230421022903122628"
...
resource "azurerm_servicebus_topic" "test" {
  name         = "acctestservicebustopic-230421022903122628"
  namespace_id = azurerm_servicebus_namespace.test.id
}

resource "azurerm_servicebus_subscription" "test" {
  name               = "acctestservicebussubscription-230421022903122628"
...
resource "azurerm_servicebus_subscription_rule" "test" {
  name            = "acctestservicebusrule-230421022903122628"
