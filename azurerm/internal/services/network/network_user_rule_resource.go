...
func resourceNetworkUserRuleCreateUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	id := parse.NewNetworkUserRuleID(subscriptionId, resourceGroup, networkManagerName, configurationName, name)

	if d.IsNewResource() {
		existing, err := client.Get(ctx, id.ResourceGroup, id.NetworkManagerName, id.SecurityConfigurationName, id.UserRuleName)
		if err != nil {
	...
	userRule := network.UserRule{
		UserRulePropertiesFormat: &network.UserRulePropertiesFormat{
			...
			Destination:           expandUserRuleAddressPrefixItemArray(d.Get("destination").([]interface{})),
			...
			Source:                expandUserRuleAddressPrefixItemArray(d.Get("source").([]interface{})),
			SourcePortRanges:      utils.ExpandStringSlice(d.Get("source_port_ranges").([]interface{})),
		},
	}
	if _, err := client.CreateOrUpdate(ctx, userRule, id.ResourceGroup, id.NetworkManagerName, id.SecurityConfigurationName, id.UserRuleName); err != nil {
...
func resourceNetworkUserRuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	resp, err := client.Get(ctx, id.ResourceGroup, id.NetworkManagerName, id.SecurityConfigurationName, id.UserRuleName)
	if err != nil {
	...
	d.Set("name", id.UserRuleName)
	d.Set("resource_group_name", id.ResourceGroup)
	d.Set("configuration_name", id.SecurityConfigurationName)
	d.Set("network_manager_name", id.NetworkManagerName)
	if props := resp.UserRulePropertiesFormat; props != nil {
		d.Set("description", props.Description)
		d.Set("direction", props.Direction)
		d.Set("display_name", props.DisplayName)
		d.Set("protocol", props.Protocol)
		if err := d.Set("source", flattenUserRuleAddressPrefixItemArray(props.Source)); err != nil {
			return fmt.Errorf("setting `source`: %+v", err)
		}
		if err := d.Set("destination", flattenUserRuleAddressPrefixItemArray(props.Destination)); err != nil {
...
func resourceNetworkUserRuleDelete(d *schema.ResourceData, meta interface{}) error {
	...
	if _, err := client.Delete(ctx, id.ResourceGroup, id.NetworkManagerName, id.SecurityConfigurationName, id.UserRuleName); err != nil {
