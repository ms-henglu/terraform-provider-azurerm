...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230728032709101158"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-230728032709101158"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                            = "acctvmss-230728032709101158"
  location                        = azurerm_resource_group.test.location
  resource_group_name             = azurerm_resource_group.test.name
  upgrade_mode                    = "Manual"
  sku                             = "Standard_F2"
  ...
  computer_name_prefix            = "testvm-230728032709101158"
  ...
  network_interface {
    name    = "TestNetworkProfile-230728032709101158"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-230728032709101158"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-230728032709101158@example.com", "acctest2-230728032709101158@example.com"]
    }
  }
}
