...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-datashare-240311031912856071"
  location = "West Europe"
}

resource "azurerm_data_share_account" "test" {
  name                = "acctest-dsa-240311031912856071"
...
resource "azurerm_data_share" "test" {
  name       = "acctest_ds_240311031912856071"
  account_id = azurerm_data_share_account.test.id
  kind       = "CopyBased"
}

resource "azurerm_storage_account" "test" {
  name                     = "accteststrvr887"
  resource_group_name      = azurerm_resource_group.test.name
  location                 = azurerm_resource_group.test.location
...
resource "azurerm_storage_data_lake_gen2_filesystem" "test" {
  name               = "acctest-240311031912856071"
...
resource "azurerm_data_share_dataset_data_lake_gen2" "test" {
  name               = "acctest-dlds-240311031912856071"
