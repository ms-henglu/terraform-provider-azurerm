...
resource "azurerm_resource_group" "test" {
  name     = "acctestsw210924004951150763"
...
resource "azurerm_storage_data_lake_gen2_filesystem" "test" {
  name               = "acctest-210924004951150763"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctest-LAW-210924004951150763"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctest-EHN-210924004951150763"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_eventhub" "test" {
  name                = "acctest-EH-210924004951150763"
...
resource "azurerm_monitor_diagnostic_setting" "test" {
  name                       = "acctest-DS-210924004951150763"
