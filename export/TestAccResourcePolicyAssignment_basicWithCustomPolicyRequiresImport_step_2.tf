...
resource "azurerm_resource_group" "test" {
  name     = "acctest220726015130726084"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-220726015130726084"
...
resource "azurerm_policy_definition" "test" {
  name         = "acctestpol-220726015130726084"
  policy_type  = "Custom"
  mode         = "All"
  display_name = "acctestpol-220726015130726084"
...
resource "azurerm_resource_policy_assignment" "test" {
  name                 = "acctestpa-220726015130726084"
