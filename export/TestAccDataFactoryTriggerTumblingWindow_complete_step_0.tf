...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-220225034258433801"
  location = "West Europe"
}

resource "azurerm_data_factory" "test" {
  name                = "acctestdf220225034258433801"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_data_factory_pipeline" "test" {
  name                = "acctestdfp220225034258433801"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_trigger_tumbling_window" "dependency" {
  name            = "acctestdft2220225034258433801"
...
resource "azurerm_data_factory_trigger_tumbling_window" "test" {
  name            = "acctestdft220225034258433801"
  data_factory_id = azurerm_data_factory.test.id
  start_time      = "2022-09-21T00:00:00Z"
