...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240119022447667974"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-240119022447667974"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                            = "acctvmss-240119022447667974"
  location                        = azurerm_resource_group.test.location
  resource_group_name             = azurerm_resource_group.test.name
  upgrade_mode                    = "Manual"
  sku                             = "Standard_F2"
  ...
  computer_name_prefix            = "testvm-240119022447667974"
  ...
  network_interface {
    name    = "TestNetworkProfile-240119022447667974"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-240119022447667974"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-240119022447667974@example.com", "acctest2-240119022447667974@example.com"]
    }
  }
}
