...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-netapp-240315123643832389"
...
resource "azurerm_key_vault_key" "test" {
  name         = "anfenckey240315123643832389"
  key_vault_id = azurerm_key_vault.test.id
  key_type     = "RSA"
  key_size     = 2048
...
resource "azurerm_netapp_account" "test" {
  name                = "acctest-NetAppAccount-240315123643832389"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctest-VirtualNetwork-240315123643832389"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "test-delegated" {
  name                 = "acctest-Delegated-Subnet-240315123643832389"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "test-non-delegated" {
  name                 = "acctest-Non-Delegated-Subnet-240315123643832389"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_private_endpoint" "test" {
  name                = "acctest-pe-akv-240315123643832389"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  subnet_id           = azurerm_subnet.test-non-delegated.id

  private_service_connection {
    name                           = "acctest-pe-sc-akv-240315123643832389"
...
resource "azurerm_netapp_pool" "test" {
  name                = "acctest-NetAppPool-240315123643832389"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_netapp_volume" "test" {
  name                          = "acctest-NetAppVolume-240315123643832389"
  location                      = azurerm_resource_group.test.location
  resource_group_name           = azurerm_resource_group.test.name
  account_name                  = azurerm_netapp_account.test.name
  pool_name                     = azurerm_netapp_pool.test.name
  volume_path                   = "my-unique-file-path-240315123643832389"
  service_level                 = "Standard"
