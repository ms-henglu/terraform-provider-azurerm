...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cosmos-220311042214168923"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test1" {
  name                = "acctest-ca-220311042214168923"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-220311042214168923"
...
resource "azurerm_cosmosdb_mongo_collection" "test" {
  name                = "acctest-mongodb-coll-220311042214168923"
...
resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca2-220311042214168923"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  offer_type          = "Standard"
  kind                = "MongoDB"
