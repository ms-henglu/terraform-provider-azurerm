...
resource "azurerm_resource_group" "test" {
  name     = "acctestRGhdi-221019060651943133"
...
resource "azurerm_storage_container" "test" {
  name                  = "acctestsc-221019060651943133"
  storage_account_name  = azurerm_storage_account.test.name
  container_access_type = "private"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestVnet-221019060651943133"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "test" {
  name                 = "acctestSubnet-221019060651943133"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "acctestNSG-221019060651943133"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azuread_group" "test" {
  display_name     = "AAD DC Administrators ihau8"
...
resource "azuread_user" "test" {
  user_principal_name = "acctestAADDSAdminUser-ihau8@${data.azuread_domains.test.domains.0.domain_name}"
  display_name        = "acctestAADDSAdminUser-ihau8"
...
resource "azurerm_hdinsight_spark_cluster" "test" {
  name                = "acctesthdispark-221019060651943133"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  cluster_version     = "4.0"
  tier                = "Premium"
