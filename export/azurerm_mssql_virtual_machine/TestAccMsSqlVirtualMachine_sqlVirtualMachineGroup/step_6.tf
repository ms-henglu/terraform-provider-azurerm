...
locals {
  domain_controller_vm_name     = "acctestjymbadc"
  active_directory_netbios_name = "acctestjymba"
  active_directory_domain_name  = "acctestjymba.local"
...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-231013043903046931"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-231013043903046931"
...
resource "azurerm_network_interface" "domain_controller" {
  name                = "acctestnic-231013043903046931-dc"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "client" {
  name                = "acctestnic-client-231013043903046931"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
