	...
	return &schema.Resource{
		...
		Schema: map[string]*schema.Schema{
			...
			"network_manager_scope_accesses": {
				...
				Elem: &schema.Schema{
					...
					ValidateFunc: validation.StringInSlice([]string{
						...
						string(network.AdminSecurity),
						string(network.UserSecurity),
					}, false),
				},
			},
			...
			"management_groups": {
				...
				ForceNew:     true,
				MinItems:     1,
				AtLeastOneOf: []string{"management_groups", "subscriptions"},
				Elem: &schema.Schema{
					Type: schema.TypeString,
				},
			},
...
func resourceNetworkManagerCreate(d *schema.ResourceData, meta interface{}) error {
	...
	parameters := network.Manager{
		...
		ManagerProperties: &network.ManagerProperties{
			...
			NetworkManagerScopes: &network.ManagerPropertiesNetworkManagerScopes{
				ManagementGroups: utils.ExpandStringSlice(d.Get("management_groups").([]interface{})),
				Subscriptions:    utils.ExpandStringSlice(d.Get("subscriptions").([]interface{})),
			},
		},
...
func resourceNetworkManagerRead(d *schema.ResourceData, meta interface{}) error {
	...
	if props := resp.ManagerProperties; props != nil {
		...
		d.Set("management_groups", utils.FlattenStringSlice(props.NetworkManagerScopes.ManagementGroups))
		d.Set("subscriptions", utils.FlattenStringSlice(props.NetworkManagerScopes.Subscriptions))
	}
