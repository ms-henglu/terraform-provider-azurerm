...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-OVMSS-230825024235238739"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-230825024235238739"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsn-230825024235238739"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip-230825024235238739"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_lb" "test" {
  name                = "acctestlb-230825024235238739"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_lb_backend_address_pool" "test" {
  name            = "acctestbap-230825024235238739"
  loadbalancer_id = azurerm_lb.test.id
}

resource "azurerm_orchestrated_virtual_machine_scale_set" "test" {
  name                = "acctestOVMSS-230825024235238739"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
