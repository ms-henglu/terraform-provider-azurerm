...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-recovery-230915024052096731-1"
  location = "West Europe"
}

resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-recovery-230915024052096731-2"
  location = "West US 2"
}


resource "azurerm_key_vault" "test" {
  name                        = "kv230915024052096731"
...
resource "azurerm_disk_encryption_set" "test" {
  name                = "acctestdes-230915024052096731"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_managed_disk" "test" {
  name                   = "acctestd-230915024052096731"
  location               = azurerm_resource_group.test.location
  resource_group_name    = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctvn-230915024052096731"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-230915024052096731"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "acctni-230915024052096731"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "test" {
  name                  = "acctvm-230915024052096731"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_data_disk {
    name              = "acctmd-230915024052096731"
    create_option     = "Empty"
    disk_size_gb      = "1"
    lun               = 0
    managed_disk_type = "Standard_LRS"
  ...
  os_profile {
    computer_name  = "hn230915024052096731"
...
resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-vault-230915024052096731"
...
resource "azurerm_site_recovery_fabric" "test1" {
  ...
  name                = "acctest-fabric1-230915024052096731"
  location            = azurerm_resource_group.test.location
}
...
resource "azurerm_site_recovery_fabric" "test2" {
  resource_group_name = azurerm_resource_group.test2.name
  recovery_vault_name = azurerm_recovery_services_vault.test.name
  name                = "acctest-fabric2-230915024052096731"
  location            = azurerm_resource_group.test2.location
...
resource "azurerm_site_recovery_protection_container" "test1" {
  ...
  name                 = "acctest-protection-cont1-230915024052096731"
}
...
resource "azurerm_site_recovery_protection_container" "test2" {
  ...
  name                 = "acctest-protection-cont2-230915024052096731"
}

resource "azurerm_site_recovery_replication_policy" "test" {
  resource_group_name                                  = azurerm_resource_group.test2.name
  recovery_vault_name                                  = azurerm_recovery_services_vault.test.name
  name                                                 = "acctest-policy-230915024052096731"
...
resource "azurerm_site_recovery_protection_container_mapping" "test" {
  ...
  name                                      = "mapping-230915024052096731"
}

resource "azurerm_virtual_network" "test2" {
  name                = "net-230915024052096731-2"
  resource_group_name = azurerm_resource_group.test2.name
...
resource "azurerm_public_ip" "test-recovery" {
  name                = "pubip230915024052096731-recovery"
...
resource "azurerm_disk_encryption_set" "test2" {
  name                = "acctestdes-2309150240520967312"
  resource_group_name = azurerm_resource_group.test2.name
  location            = azurerm_resource_group.test2.location
