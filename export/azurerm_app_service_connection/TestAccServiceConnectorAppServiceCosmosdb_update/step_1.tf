...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230616075443496881"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctestaccb19zf"
...
resource "azurerm_cosmosdb_sql_container" "update" {
  name                = "test-containerupdateb19zf"
  resource_group_name = azurerm_cosmosdb_account.test.resource_group_name
  account_name        = azurerm_cosmosdb_account.test.name
...
resource "azurerm_service_plan" "update" {
  location            = azurerm_resource_group.test.location
  name                = "testserviceplanupdateb19zf"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "P1v2"
  os_type             = "Linux"
}
...
resource "azurerm_app_service_connection" "test" {
  name               = "acctestserviceconnector230616075443496881"
