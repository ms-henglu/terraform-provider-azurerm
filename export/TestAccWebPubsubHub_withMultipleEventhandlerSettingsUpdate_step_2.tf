...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-wps-220324160905032011"
  location = "West Europe"
}

resource "azurerm_web_pubsub" "test" {
  name                = "acctest-webpubsub-220324160905032011"
...
resource "azurerm_user_assigned_identity" "test1" {
  name                = "acctest-uai1-220324160905032011"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
resource "azurerm_user_assigned_identity" "test2" {
  name                = "acctest-uai2-220324160905032011"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
resource "azurerm_web_pubsub_hub" "test" {
  name          = "acctestwpsh220324160905032011"
