...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-230120054908583673"
  location = "West Europe"
}

resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-230120054908583673"
...
resource "azurerm_mssql_database" "primary" {
  name        = "acctest-db-230120054908583673"
...
resource "azurerm_resource_group" "secondary" {
  name     = "acctestRG-mssql2-230120054908583673"
  location = "West US 2"
}

resource "azurerm_mssql_server" "secondary" {
  name                         = "acctest-sqlserver2-230120054908583673"
