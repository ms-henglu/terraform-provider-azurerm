...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-220610022615326753"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet220610022615326753"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip220610022615326753"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test_mgmt" {
  name                = "acctestmgmtpip220610022615326753"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  sku                 = "Standard"
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall220610022615326753"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "AZFW_VNet"
  sku_tier            = "Standard"
