...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mapp-211021235207641653"
  location = "West Europe"
}

resource "azurerm_managed_application_definition" "test" {
  name                = "acctestManagedAppDef211021235207641653"
...
resource "azurerm_managed_application" "test" {
  name                        = "acctestManagedApp211021235207641653"
  location                    = azurerm_resource_group.test.location
  resource_group_name         = azurerm_resource_group.test.name
  kind                        = "ServiceCatalog"
  managed_resource_group_name = "infraGroup211021235207641653"
  application_definition_id   = azurerm_managed_application_definition.test.id

  parameters = {
    location                 = azurerm_resource_group.test.location
    storageAccountNamePrefix = "storeb6asj"
