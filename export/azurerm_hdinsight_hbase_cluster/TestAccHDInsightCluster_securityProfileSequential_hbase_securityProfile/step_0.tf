...
resource "azurerm_resource_group" "test" {
  name     = "acctestRGhdi-240119025125193590"
...
resource "azurerm_storage_container" "test" {
  name                  = "acctestsc-240119025125193590"
  storage_account_name  = azurerm_storage_account.test.name
  container_access_type = "private"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestVnet-240119025125193590"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "test" {
  name                 = "acctestSubnet-240119025125193590"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "acctestNSG-240119025125193590"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azuread_group" "test" {
  display_name     = "AAD DC Administrators 3c1np"
...
resource "azuread_user" "test" {
  user_principal_name = "acctestAADDSAdminUser-3c1np@${data.azuread_domains.test.domains.0.domain_name}"
  display_name        = "acctestAADDSAdminUser-3c1np"
...
resource "azurerm_hdinsight_hbase_cluster" "test" {
  name                = "acctesthdihbase-240119025125193590"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  cluster_version     = "4.0"
  tier                = "Premium"
