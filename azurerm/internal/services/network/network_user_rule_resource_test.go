...
func (r NetworkUserRuleResource) Exists(ctx context.Context, client *clients.Client, state *terraform.InstanceState) (*bool, error) {
	...
	resp, err := client.Network.UserRuleClient.Get(ctx, id.ResourceGroup, id.NetworkManagerName, id.ConfigurationName, id.Name)
	if err != nil {
		if utils.ResponseWasNotFound(resp.Response) {
			return utils.Bool(false), nil
		}
		return nil, fmt.Errorf("retrieving Network UserRule %q (Resource Group %q / networkManagerName %q / configurationName %q): %+v", id.Name, id.ResourceGroup, id.NetworkManagerName, id.ConfigurationName, err)
	}
