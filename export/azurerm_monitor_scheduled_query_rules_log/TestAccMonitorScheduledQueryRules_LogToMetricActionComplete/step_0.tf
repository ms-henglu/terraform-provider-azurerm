...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-monitor-221124182015291336"
  location = "West Europe"
}

resource "azurerm_application_insights" "test" {
  name                = "acctestAppInsights-221124182015291336"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestWorkspace-221124182015291336"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_monitor_action_group" "test" {
  name                = "acctestActionGroup-221124182015291336"
  resource_group_name = azurerm_resource_group.test.name
  short_name          = "acctestag"
}

resource "azurerm_monitor_scheduled_query_rules_log" "test" {
  name                = "acctestsqr-221124182015291336"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_monitor_metric_alert" "test" {
  name                = "acctestmal-221124182015291336"
  resource_group_name = azurerm_resource_group.test.name
