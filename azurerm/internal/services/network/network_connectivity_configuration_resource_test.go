...
func (r NetworkConnectivityConfigurationResource) Exists(ctx context.Context, client *clients.Client, state *terraform.InstanceState) (*bool, error) {
	...
	resp, err := client.Network.ConnectivityConfigurationClient.Get(ctx, id.ResourceGroup, id.NetworkManagerName, id.Name)
	if err != nil {
		if utils.ResponseWasNotFound(resp.Response) {
			return utils.Bool(false), nil
		}
		return nil, fmt.Errorf("retrieving Network ConnectivityConfiguration %q (Resource Group %q / networkManagerName %q): %+v", id.Name, id.ResourceGroup, id.NetworkManagerName, err)
	}
