...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-230113180811673856"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF230113180811673856"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone230113180811673856.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-230113180811673856"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-230113180811673856"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol230113180811673856"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
