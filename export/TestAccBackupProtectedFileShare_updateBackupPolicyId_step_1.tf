...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-backup-211112021116241624"
...
resource "azurerm_storage_share" "test" {
  name                 = "acctest-ss-211112021116241624"
...
resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-VAULT-211112021116241624"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
  sku                 = "Standard"
...
resource "azurerm_backup_policy_file_share" "test1" {
  name                = "acctest-PFS-211112021116241624"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_backup_policy_file_share" "test2" {
  name                = "acctest-211112021116241624-Secondary"
  resource_group_name = azurerm_resource_group.test.name
  recovery_vault_name = azurerm_recovery_services_vault.test.name
