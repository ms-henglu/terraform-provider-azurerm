...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-medTech-230106031514464191"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctest-ehn-230106031514464191"
...
resource "azurerm_eventhub" "test" {
  name                = "acctest-eh-230106031514464191"
...
resource "azurerm_eventhub" "test1" {
  name                = "acctest-eh1-230106031514464191"
  namespace_name      = azurerm_eventhub_namespace.test.name
  resource_group_name = azurerm_resource_group.test.name
  partition_count     = 1
  message_retention   = 1
...
resource "azurerm_eventhub_consumer_group" "test" {
  name                = "acctestCG-230106031514464191"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_eventhub_consumer_group" "test1" {
  name                = "acctestCG1-230106031514464191"
  namespace_name      = azurerm_eventhub_namespace.test.name
