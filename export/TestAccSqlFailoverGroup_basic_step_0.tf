...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220630211407670125"
  location = "West Europe"
}

resource "azurerm_sql_server" "test_primary" {
  name                         = "acctestmssql220630211407670125-primary"
...
resource "azurerm_sql_server" "test_secondary" {
  name                         = "acctestmssql220630211407670125-secondary"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = "West US 2"
  version                      = "12.0"
  administrator_login          = "mradministrator"
...
resource "azurerm_sql_database" "test" {
  name                             = "acctestdb220630211407670125"
  resource_group_name              = azurerm_resource_group.test.name
...
resource "azurerm_sql_failover_group" "test" {
  name                = "acctestsfg220630211407670125"
  resource_group_name = azurerm_resource_group.test.name
  server_name         = azurerm_sql_server.test_primary.name
