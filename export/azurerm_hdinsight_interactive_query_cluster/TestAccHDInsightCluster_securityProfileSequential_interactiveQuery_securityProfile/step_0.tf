...
resource "azurerm_resource_group" "test" {
  name     = "acctestRGhdi-230707010436847765"
...
resource "azurerm_storage_container" "test" {
  name                  = "acctestsc-230707010436847765"
  storage_account_name  = azurerm_storage_account.test.name
  container_access_type = "private"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestVnet-230707010436847765"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "test" {
  name                 = "acctestSubnet-230707010436847765"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "acctestNSG-230707010436847765"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azuread_group" "test" {
  display_name     = "AAD DC Administrators 20fgm"
...
resource "azuread_user" "test" {
  user_principal_name = "acctestAADDSAdminUser-20fgm@${data.azuread_domains.test.domains.0.domain_name}"
  display_name        = "acctestAADDSAdminUser-20fgm"
...
resource "azurerm_hdinsight_interactive_query_cluster" "test" {
  name                = "acctesthdiiq-230707010436847765"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  cluster_version     = "4.0"
  tier                = "Premium"
