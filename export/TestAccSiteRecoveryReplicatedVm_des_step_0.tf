...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-recovery-220128052945747253-1"
  location = "West Europe"
}

resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-recovery-220128052945747253-2"
  location = "West US 2"
}


resource "azurerm_key_vault" "test" {
  name                        = "kv220128052945747253"
...
resource "azurerm_disk_encryption_set" "test" {
  name                = "acctestdes-220128052945747253"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_managed_disk" "test" {
  name                   = "acctestd-220128052945747253"
  location               = azurerm_resource_group.test.location
  resource_group_name    = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctvn-220128052945747253"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-220128052945747253"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "acctni-220128052945747253"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "test" {
  name                  = "acctvm-220128052945747253"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_data_disk {
    name              = "acctmd-220128052945747253"
    create_option     = "Empty"
    disk_size_gb      = "1"
    lun               = 0
    managed_disk_type = "Standard_LRS"
  ...
  os_profile {
    computer_name  = "hn220128052945747253"
...
resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-vault-220128052945747253"
...
resource "azurerm_site_recovery_fabric" "test1" {
  ...
  name                = "acctest-fabric1-220128052945747253"
  location            = azurerm_resource_group.test.location
}
...
resource "azurerm_site_recovery_fabric" "test2" {
  resource_group_name = azurerm_resource_group.test2.name
  recovery_vault_name = azurerm_recovery_services_vault.test.name
  name                = "acctest-fabric2-220128052945747253"
  location            = azurerm_resource_group.test2.location
...
resource "azurerm_site_recovery_protection_container" "test1" {
  ...
  name                 = "acctest-protection-cont1-220128052945747253"
}
...
resource "azurerm_site_recovery_protection_container" "test2" {
  ...
  name                 = "acctest-protection-cont2-220128052945747253"
}

resource "azurerm_site_recovery_replication_policy" "test" {
  resource_group_name                                  = azurerm_resource_group.test2.name
  recovery_vault_name                                  = azurerm_recovery_services_vault.test.name
  name                                                 = "acctest-policy-220128052945747253"
...
resource "azurerm_site_recovery_protection_container_mapping" "test" {
  ...
  name                                      = "mapping-220128052945747253"
}

resource "azurerm_virtual_network" "test2" {
  name                = "net-220128052945747253-2"
  resource_group_name = azurerm_resource_group.test2.name
...
resource "azurerm_subnet" "test2_1" {
  name                 = "acctest-snet-220128052945747253-2"
  resource_group_name  = "${azurerm_resource_group.test2.name}"
...
resource "azurerm_subnet" "test2_2" {
  name                 = "snet-220128052945747253-3"
  resource_group_name  = "${azurerm_resource_group.test2.name}"
  virtual_network_name = "${azurerm_virtual_network.test2.name}"
...
resource "azurerm_public_ip" "test-recovery" {
  name                = "pubip220128052945747253-recovery"
...
resource "azurerm_disk_encryption_set" "test2" {
  name                = "acctestdes-2201280529457472532"
  resource_group_name = azurerm_resource_group.test2.name
  location            = azurerm_resource_group.test2.location
