...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-240315122436144999"
...
resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "acctest-profile-240315122436144999"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "acctest-origin-group-240315122436144999"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "acctest-origin-240315122436144999"
  cdn_frontdoor_origin_group_id = azurerm_cdn_frontdoor_origin_group.test.id
  enabled                       = true

  certificate_name_check_enabled = false
  host_name                      = join(".", ["0nheogaiwk", azurerm_dns_zone.test.name])
  priority                       = 1
  weight                         = 1
}

resource "azurerm_cdn_frontdoor_endpoint" "test" {
  name                     = "acctest-endpoint-240315122436144999"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
  enabled                  = true
}

resource "azurerm_cdn_frontdoor_route" "contoso" {
  name                          = "acctest-contoso-240315122436144999"
