...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220520054139768879"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-220520054139768879"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsn-220520054139768879"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_storage_container" "test" {
  name                  = "acctestsc-220520054139768879"
  storage_account_name  = azurerm_storage_account.test.name
  container_access_type = "private"
}

resource "azurerm_public_ip" "test" {
  name                = "acctestpip-220520054139768879"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_lb" "test" {
  name                = "acctestlb-220520054139768879"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_lb_backend_address_pool" "test" {
  name            = "acctestbap-220520054139768879"
  loadbalancer_id = azurerm_lb.test.id
}

resource "azurerm_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-220520054139768879"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
