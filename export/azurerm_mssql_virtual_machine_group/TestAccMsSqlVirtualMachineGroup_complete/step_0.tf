...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-230922061545342851"
  location = "West Europe"
}

resource "azurerm_storage_account" "test" {
  name                     = "unlikely23exst2acctqufgn"
...
resource "azurerm_mssql_virtual_machine_group" "test" {
  ...
  wsfc_domain_profile {
    ...
    cluster_bootstrap_account_name = "bootstrapaccqufgn"
    cluster_operator_account_name  = "opaccqufgn"
    sql_service_account_name       = "sqlsrvaccqufgn"
