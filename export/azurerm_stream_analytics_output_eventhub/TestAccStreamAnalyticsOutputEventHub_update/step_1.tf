...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230526090009407792"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctestehn-230526090009407792"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteh-230526090009407792"
...
resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-230526090009407792"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_eventhub_namespace" "updated" {
  name                = "acctestehn2-230526090009407792"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  capacity            = 1
...
resource "azurerm_eventhub" "updated" {
  name                = "acctesteh2-230526090009407792"
...
resource "azurerm_stream_analytics_output_eventhub" "test" {
  name                      = "acctestinput-230526090009407792"
