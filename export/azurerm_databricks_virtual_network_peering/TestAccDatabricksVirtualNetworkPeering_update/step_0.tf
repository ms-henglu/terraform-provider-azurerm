...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-databricks-230609091135194408"
  location = "West Europe"
}

resource "azurerm_virtual_network" "remote" {
  name                = "acctest-vnet-230609091135194408"
...
resource "azurerm_databricks_workspace" "test" {
  name                = "acctest-ws-230609091135194408"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "standard"
}
...
resource "azurerm_databricks_virtual_network_peering" "test" {
  name                = "acctest-230609091135194408"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network_peering" "remote" {
  name                      = "to-acctest-230609091135194408"
  resource_group_name       = azurerm_resource_group.test.name
