...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ca-220204092814859374"
  location = "West US 2"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-220204092814859374"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-220204092814859374"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_cosmosdb_cassandra_cluster" "test" {
  name                           = "acctca-mi-cluster-220204092814859374"
  resource_group_name            = azurerm_resource_group.test.name
  location                       = azurerm_resource_group.test.location
...
resource "azurerm_cosmosdb_cassandra_datacenter" "test" {
  name                           = "acctca-mi-dc-220204092814859374"
