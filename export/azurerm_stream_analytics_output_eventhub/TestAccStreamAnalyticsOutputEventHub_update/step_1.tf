...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221117231557421887"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctestehn-221117231557421887"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteh-221117231557421887"
...
resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-221117231557421887"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_eventhub_namespace" "updated" {
  name                = "acctestehn2-221117231557421887"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  capacity            = 1
...
resource "azurerm_eventhub" "updated" {
  name                = "acctesteh2-221117231557421887"
...
resource "azurerm_stream_analytics_output_eventhub" "test" {
  name                      = "acctestinput-221117231557421887"
