...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-security-220722035923186897"
  location = "West Europe"
}

resource "azurerm_iothub" "test" {
  name                = "acctestIoTHub-220722035923186897"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctest-law-220722035923186897"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
}
...
resource "azurerm_log_analytics_workspace" "test2" {
  name                = "acctest-law2-220722035923186897"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
}
...
resource "azurerm_iot_security_solution" "test" {
  name                = "acctest-Iot-Security-Solution-220722035923186897"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
