...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-221111020338575071"
...
resource "azurerm_storage_data_lake_gen2_filesystem" "test" {
  name               = "acctest-datalake-221111020338575071"
  storage_account_id = azurerm_storage_account.test.id
}

resource "azurerm_data_factory" "test" {
  name                = "acctestdf221111020338575071"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_linked_service_data_lake_storage_gen2" "test" {
  name                 = "acctestDataLakeStorage221111020338575071"
...
resource "azurerm_data_factory_dataset_delimited_text" "test" {
  name                = "acctestds221111020338575071"
  data_factory_id     = azurerm_data_factory.test.id
