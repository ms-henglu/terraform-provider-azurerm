...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-databricks-240119024832330617"
  location = "West US 2"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-240119024832330617"
...
resource "azurerm_subnet" "public" {
  name                 = "acctest-sn-public-240119024832330617"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "private" {
  name                 = "acctest-sn-private-240119024832330617"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_subnet" "privatelink" {
  name                 = "acctest-snpl-240119024832330617"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_network_security_group" "nsg" {
  name                = "acctest-nsg-240119024832330617"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_databricks_workspace" "test" {
  ...
  name                        = "acctestDBW-240119024832330617"
  resource_group_name         = azurerm_resource_group.test.name
  location                    = azurerm_resource_group.test.location
  sku                         = "premium"
  managed_resource_group_name = "acctestRG-DBW-240119024832330617-managed"
...
resource "azurerm_private_endpoint" "databricks" {
  ...
  name                = "acctest-endpoint-240119024832330617"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  subnet_id           = azurerm_subnet.privatelink.id

  private_service_connection {
    name                           = "acctest-psc-240119024832330617"
