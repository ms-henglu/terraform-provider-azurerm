...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-apim-211203013358174285"
  location = "West Europe"
}

resource "azurerm_api_management" "test" {
  name                = "acctestAM-211203013358174285"
...
resource "azurerm_redis_cache" "test" {
  name                = "acctestRedis-211203013358174285"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  capacity            = 1
  family              = "C"
...
resource "azurerm_redis_cache" "test2" {
  name                = "acctestRedis2-211203013358174285"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  capacity            = 1
  family              = "C"
...
resource "azurerm_api_management_redis_cache" "test" {
  name              = "acctest-Redis-Cache-211203013358174285"
