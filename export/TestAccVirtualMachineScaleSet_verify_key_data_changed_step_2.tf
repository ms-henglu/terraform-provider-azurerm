...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211210024414763247"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-211210024414763247"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsn-211210024414763247"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_storage_container" "test" {
  name                  = "acctestsc-211210024414763247"
  storage_account_name  = azurerm_storage_account.test.name
  container_access_type = "private"
}

resource "azurerm_public_ip" "test" {
  name                = "acctestpip-211210024414763247"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_lb" "test" {
  name                = "acctestlb-211210024414763247"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_lb_backend_address_pool" "test" {
  name                = "acctestbap-211210024414763247"
  resource_group_name = azurerm_resource_group.test.name
  loadbalancer_id     = azurerm_lb.test.id
}

resource "azurerm_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-211210024414763247"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
