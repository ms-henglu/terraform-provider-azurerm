...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ase-221019061208684466"
  location = "West Europe"
}

resource "azurerm_resource_group" "test2" {
  name     = "acctestRG2-ase-221019061208684466"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-221019061208684466"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-subnet-221019061208684466"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_app_service_environment_v3" "test" {
  name                = "acctest-ase-221019061208684466"
  resource_group_name = azurerm_resource_group.test.name
