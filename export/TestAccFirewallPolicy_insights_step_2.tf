...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-networkfw-211112020642548769"
  location = "West Europe"
}


resource "azurerm_log_analytics_workspace" "default" {
  name                = "acctestLAW-211112020642548769"
...
resource "azurerm_log_analytics_workspace" "regional" {
  name                = "acctestLAW-region-211112020642548769"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
  retention_in_days   = 30
...
resource "azurerm_firewall_policy" "test" {
  name                = "acctest-networkfw-Policy-211112020642548769"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  insights {
    enabled                            = true
