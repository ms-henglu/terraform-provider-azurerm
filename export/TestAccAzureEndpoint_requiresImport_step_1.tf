...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-traffic-220726002555383359"
  location = "West Europe"
}

resource "azurerm_traffic_manager_profile" "test" {
  name                   = "acctest-TMP-220726002555383359"
  resource_group_name    = azurerm_resource_group.test.name
  traffic_routing_method = "Weighted"

  dns_config {
    relative_name = "acctest-tmp-220726002555383359"
    ttl           = 30
  }
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpublicip-220726002555383359"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  domain_name_label   = "acctestpublicip-220726002555383359"
}

resource "azurerm_traffic_manager_azure_endpoint" "test" {
  name               = "acctestend-azure220726002555383359"
