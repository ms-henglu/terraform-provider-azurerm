...
var _ resourceid.Formatter = SqlUserDefinedFunctionId{}

func TestSqlUserDefinedFunctionIDFormatter(t *testing.T) {
	actual := NewSqlUserDefinedFunctionID("12345678-1234-9876-4563-123456789012", "resourceGroup1", "account1", "database1", "container1", "userDefinedFunction1").ID()
...
func TestSqlUserDefinedFunctionID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *SqlUserDefinedFunctionId
	}{
	...
	for _, v := range testData {
		...
		actual, err := SqlUserDefinedFunctionID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
