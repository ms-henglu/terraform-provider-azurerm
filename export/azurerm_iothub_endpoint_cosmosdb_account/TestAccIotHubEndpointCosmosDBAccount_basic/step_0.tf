...
resource "azurerm_resource_group" "iothub" {
  name     = "acctestRG-iothub-230915023548190140"
  location = "West Europe"
}

resource "azurerm_resource_group" "endpoint" {
  name     = "acctestRG-iothub-endpoint-230915023548190140"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca-230915023548190140"
...
resource "azurerm_cosmosdb_sql_database" "test" {
  name                = "acctest-230915023548190140"
  resource_group_name = azurerm_cosmosdb_account.test.resource_group_name
  account_name        = azurerm_cosmosdb_account.test.name
}

resource "azurerm_cosmosdb_sql_container" "test" {
  name                = "acctest-CSQLC-230915023548190140"
  resource_group_name = azurerm_cosmosdb_account.test.resource_group_name
  account_name        = azurerm_cosmosdb_account.test.name
...
resource "azurerm_iothub" "test" {
  name                = "acctestIoTHub-230915023548190140"
