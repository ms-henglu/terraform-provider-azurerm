...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230609091400168575"
  location = "West Europe"
}

resource "azurerm_storage_account" "gen2test" {
  name                     = "accgen2testj2jrh"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet230609091400168575"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet230609091400168575"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip230609091400168575"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_nat_gateway" "test" {
  name                    = "acctestnat230609091400168575"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
  sku_name                = "Standard"
...
resource "azurerm_hdinsight_spark_cluster" "test" {
  ...
  name                = "acctesthdi-230609091400168575"
  resource_group_name = "${azurerm_resource_group.test.name}"
  location            = "${azurerm_resource_group.test.location}"
  cluster_version     = "4.0"
  tier                = "Standard"
...
resource "azurerm_network_security_group" "test" {
  name                = "acctestnsg-230609091400168575"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
