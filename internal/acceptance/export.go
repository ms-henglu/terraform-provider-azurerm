...
func (td TestData) ExportConfig(t *testing.T, steps []TestStep) {
	...
	for i, step := range steps {
		if len(step.Config) > 0 {
			stepDirectory := path.Join(workingDirectory, fmt.Sprintf("step_%v", i))
			if _, err := os.Stat(stepDirectory); os.IsNotExist(err) {
				err = os.MkdirAll(stepDirectory, 0744)
				if err != nil {
					t.Fatal(err)
				}
			}
			filename := path.Join(workingDirectory, fmt.Sprintf("step_%v", i), "main.tf")
			d1 := []byte(step.Config)
