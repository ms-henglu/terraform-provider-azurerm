...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-240315123556557210"
  location = "West Europe"
}

resource "azurerm_storage_account" "test" {
  name                     = "unlikely23exst2acctlevew"
...
resource "azurerm_mssql_virtual_machine_group" "test" {
  ...
  wsfc_domain_profile {
    ...
    cluster_bootstrap_account_name = "bootstrapacclevew"
    cluster_operator_account_name  = "opacclevew"
    sql_service_account_name       = "sqlsrvacclevew"
