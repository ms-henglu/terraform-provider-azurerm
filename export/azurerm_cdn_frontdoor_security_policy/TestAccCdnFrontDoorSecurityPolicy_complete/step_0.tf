...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-230728031903696905"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF230728031903696905"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone230728031903696905.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-230728031903696905"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-230728031903696905"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol230728031903696905"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
