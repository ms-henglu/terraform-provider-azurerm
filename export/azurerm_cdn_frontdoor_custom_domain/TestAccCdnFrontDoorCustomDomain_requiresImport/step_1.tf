...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-221202035244547154"
  location = "West Europe"
}

resource "azurerm_dns_zone" "test" {
  name                = "acctestzone221202035244547154.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "acctestcdnfdprofile-221202035244547154"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "acctestcustomdomain-221202035244547154"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
  dns_zone_id              = azurerm_dns_zone.test.id
  host_name                = join(".", ["n4uhex2v", azurerm_dns_zone.test.name])
