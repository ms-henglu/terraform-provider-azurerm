...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211217075600976288"
  location = "West Europe"
}

resource "azurerm_mssql_server" "test_primary" {
  name                         = "acctestmssql211217075600976288-primary"
...
resource "azurerm_mssql_server" "test_secondary" {
  name                         = "acctestmssql211217075600976288-secondary"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = "West US 2"
  version                      = "12.0"
  administrator_login          = "mradministrator"
...
resource "azurerm_mssql_database" "test" {
  name        = "acctestdb211217075600976288"
...
resource "azurerm_mssql_failover_group" "test" {
  name      = "acctestsfg211217075600976288"
  server_id = azurerm_mssql_server.test_primary.id
