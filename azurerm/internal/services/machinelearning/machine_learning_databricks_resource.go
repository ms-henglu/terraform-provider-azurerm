...
func resourceDatabricks() *pluginsdk.Resource {
	return &pluginsdk.Resource{
		Create: resourceDatabricksCreateUpdate,
		Update: resourceDatabricksCreateUpdate,
		Read:   resourceDatabricksRead,
		Delete: resourceDatabricksDelete,
...
func resourceDatabricksCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.IsNewResource() {
		...
		if !utils.ResponseWasNotFound(existing.Response) {
			return tf.ImportAsExistsError("azurerm_machine_learning_databricks", id.ID())
		}
	}
	...
	computeParameters := machinelearningservices.ComputeResource{
		Properties: machinelearningservices.Databricks{
			ResourceID:      utils.String(d.Get("databricks_workspace_id").(string)),
			ComputeLocation: utils.String(d.Get("location").(string)),
			Description:     utils.String(d.Get("description").(string)),
			Properties: &machinelearningservices.DatabricksProperties{
				DatabricksAccessToken: utils.String(d.Get("databricks_access_token").(string)),
			},
		},
		Identity: identity,
	...
	return resourceDatabricksRead(d, meta)
}

func resourceDatabricksRead(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).MachineLearning.MachineLearningComputeClient
	subscriptionId := meta.(*clients.Client).Account.SubscriptionId
	...
	if props, ok := resp.Properties.AsDatabricks(); ok && props != nil {
		d.Set("databricks_workspace_id", props.ResourceID)
		d.Set("description", props.Description)
	} else {
		return fmt.Errorf("compute resource %s is not a Databricks Compute", id)
	}

	return tags.FlattenAndSet(d, resp.Tags)
}

func resourceDatabricksDelete(d *pluginsdk.ResourceData, meta interface{}) error {
	client := meta.(*clients.Client).MachineLearning.MachineLearningComputeClient
