...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mongouserdef-230707010203653099"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca-230707010203653099"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-230707010203653099"
...
resource "azurerm_cosmosdb_mongo_role_definition" "test" {
  ...
  role_name                = "acctestmongoroledef230707010203653099"
}

resource "azurerm_cosmosdb_mongo_role_definition" "test2" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  role_name                = "acctestmongoroledef2230707010203653099"
}

resource "azurerm_cosmosdb_mongo_user_definition" "test" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  username                 = "myUserName-230707010203653099"
  password                 = "myPassword2-230707010203653099"
