...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230721020146093796"
  location = "West Europe"
}

resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-230721020146093796"
...
resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-230721020146093796"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  version                      = "12.0"
...
resource "azurerm_mssql_database" "test" {
  name         = "acctest-db-230721020146093796"
...
resource "azurerm_stream_analytics_reference_input_mssql" "test" {
  name                      = "acctestinput-230721020146093796"
