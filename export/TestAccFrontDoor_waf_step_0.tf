...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-frontdoor-210906022308882793"
...
resource "azurerm_frontdoor_firewall_policy" "test" {
  name                = "acctestwafp210906022308882793"
  resource_group_name = azurerm_resource_group.test.name
  mode                = "Prevention"
}

resource "azurerm_frontdoor" "test" {
  name                                         = "acctest-FD-210906022308882793"
  resource_group_name                          = azurerm_resource_group.test.name
  ...
  frontend_endpoint {
    ...
    host_name                               = "acctest-FD-210906022308882793.azurefd.net"
