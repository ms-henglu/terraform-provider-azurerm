...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-221221203956552922"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-221221203956552922"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-221221203956552922"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-221221203956552922"
  ...
  schedule {
    ...
    start_time          = "2022-12-22T06:39:56Z"
    expiry_time         = "2022-12-23T22:39:56Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
