...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220729033348768576"
  location = "West Europe"
}

resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-220729033348768576"
...
resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-220729033348768576"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  version                      = "12.0"
...
resource "azurerm_mssql_database" "test" {
  name      = "acctest-db-220729033348768576"
  server_id = azurerm_mssql_server.test.id
}


resource "azurerm_stream_analytics_reference_input_mssql" "test" {
  name                      = "acctestinput-220729033348768576"
