...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220623223016351306"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-220623223016351306"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-220623223016351306"
...
resource "azurerm_api_management" "test" {
  name                = "acctestAM-220623223016351306"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  publisher_name      = "pub1"
...
resource "azurerm_api_management_logger" "test" {
  name                = "acctestapimnglogger-220623223016351306"
