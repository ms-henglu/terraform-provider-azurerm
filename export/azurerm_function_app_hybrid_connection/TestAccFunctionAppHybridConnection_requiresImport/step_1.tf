...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230810142912222827"
  location = "West Europe"
}

resource "azurerm_service_plan" "test" {
  name                = "acctestASP-230810142912222827"
...
resource "azurerm_relay_namespace" "test" {
  name                = "acctest-RN-230810142912222827"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_relay_hybrid_connection" "test" {
  name                 = "acctest-RHC-230810142912222827"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_windows_function_app" "test" {
  name                = "acctest-WFA-230810142912222827"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_function_app_hybrid_connection" "test" {
  ...
  hostname        = "acctestu3hx4e6s.hostname"
  port            = 8081
}
