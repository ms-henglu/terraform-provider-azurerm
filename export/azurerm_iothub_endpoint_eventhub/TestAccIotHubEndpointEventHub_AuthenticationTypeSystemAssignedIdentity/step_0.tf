...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-iothub-230616074901636574"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-230616074901636574"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-230616074901636574"
...
resource "azurerm_eventhub_authorization_rule" "test" {
  name                = "acctest-230616074901636574"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_user_assigned_identity" "test" {
  name                = "acctestuai-230616074901636574"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
...
resource "azurerm_iothub" "test" {
  name                = "acctestIoTHub-230616074901636574"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
