...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-221111020109007816"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-221111020109007816"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "accTestOriginGroup-221111020109007816"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "accTestOrigin-221111020109007816"
...
resource "azurerm_cdn_frontdoor_rule_set" "test" {
  name                     = "accTestRuleSet221111020109007816"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
}
...
resource "azurerm_cdn_frontdoor_rule" "test" {
  ...
  name                      = "accTestRule221111020109007816"
