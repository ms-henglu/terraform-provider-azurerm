...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-220124125123919687"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet220124125123919687"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip220124125123919687"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test_mgmt" {
  name                = "acctestmgmtpip220124125123919687"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  sku                 = "Standard"
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall220124125123919687"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
