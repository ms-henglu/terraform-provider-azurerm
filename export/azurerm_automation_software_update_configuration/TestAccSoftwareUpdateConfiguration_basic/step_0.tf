...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-230313020743539160"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-230313020743539160"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-230313020743539160"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-230313020743539160"
  ...
  schedule {
    ...
    start_time          = "2023-03-13T12:07:43Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
