...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221021030753141134"
...
resource "azurerm_aadb2c_directory" "test" {
  ...
  display_name            = "acctest221021030753141134"
  domain_name             = "acctest221021030753141134.onmicrosoft.com"
...
resource "azurerm_resource_group" "duplicate" {
  name     = "acctestRG-duplicate-221021030753141134"
  location = "West US 2"
}
...
resource "azurerm_aadb2c_directory" "duplicate" {
  ...
  display_name            = "acctest-duplicate-221021030753141134"
