...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-210928055715815399"
  location = "West Europe"
}

resource "azurerm_mssql_server" "test_primary" {
  name                         = "acctestmssql210928055715815399-primary"
...
resource "azurerm_mssql_server" "test_secondary" {
  name                         = "acctestmssql210928055715815399-secondary"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = "West US 2"
  version                      = "12.0"
  administrator_login          = "mradministrator"
...
resource "azurerm_mssql_database" "test" {
  name        = "acctestdb210928055715815399"
...
resource "azurerm_mssql_failover_group" "test" {
  name      = "acctestsfg210928055715815399"
  server_id = azurerm_mssql_server.test_primary.id
