...
resource "azurerm_resource_group" "test" {
  name     = "acctestrg-monitor-220324160600838144"
  location = "West Europe"
}

resource "azurerm_application_insights" "test" {
  name                = "acctestappinsights-220324160600838144"
...
resource "azurerm_monitor_action_group" "test" {
  name                = "acctestActionGroup-220324160600838144"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_monitor_smart_detector_alert_rule" "test" {
  name                = "acctestSDAR-220324160600838144"
  resource_group_name = azurerm_resource_group.test.name
  severity            = "Sev0"
