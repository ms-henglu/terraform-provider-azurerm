...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-frontdoor-210924011023399597"
...
resource "azurerm_frontdoor_firewall_policy" "test" {
  name                = "acctestwafp210924011023399597"
  resource_group_name = azurerm_resource_group.test.name
  mode                = "Prevention"
}

resource "azurerm_frontdoor" "test" {
  name                                         = "acctest-FD-210924011023399597"
  resource_group_name                          = azurerm_resource_group.test.name
  ...
  frontend_endpoint {
    ...
    host_name                               = "acctest-FD-210924011023399597.azurefd.net"
