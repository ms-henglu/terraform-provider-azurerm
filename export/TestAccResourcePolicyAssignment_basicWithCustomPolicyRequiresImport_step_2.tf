...
resource "azurerm_resource_group" "test" {
  name     = "acctest220603005148741927"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-220603005148741927"
...
resource "azurerm_policy_definition" "test" {
  name         = "acctestpol-220603005148741927"
  policy_type  = "Custom"
  mode         = "All"
  display_name = "acctestpol-220603005148741927"
...
resource "azurerm_resource_policy_assignment" "test" {
  name                 = "acctestpa-220603005148741927"
