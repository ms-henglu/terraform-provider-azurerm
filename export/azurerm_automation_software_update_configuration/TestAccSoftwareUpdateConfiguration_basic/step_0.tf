...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-221216013121385203"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-221216013121385203"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-221216013121385203"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-221216013121385203"
  ...
  schedule {
    ...
    start_time          = "2022-12-16T11:31:21Z"
    expiry_time         = "2022-12-18T03:31:21Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
