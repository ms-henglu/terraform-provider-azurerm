...
resource "azurerm_resource_group" "test" {
  name     = "acctest220715004744466232"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-220715004744466232"
...
resource "azurerm_policy_definition" "test" {
  name         = "acctestpol-220715004744466232"
  policy_type  = "Custom"
  mode         = "All"
  display_name = "acctestpol-220715004744466232"
...
resource "azurerm_resource_policy_assignment" "test" {
  name                 = "acctestpa-220715004744466232"
