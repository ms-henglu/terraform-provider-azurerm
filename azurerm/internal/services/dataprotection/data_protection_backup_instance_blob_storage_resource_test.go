...
resource "azurerm_data_protection_backup_instance_blob_storage" "test" {
  ...
  policy_id           = azurerm_data_protection_backup_policy_blob_storage.test.id
}
...
resource "azurerm_data_protection_backup_instance_blob_storage" "import" {
  ...
  resource_group_name = azurerm_data_protection_backup_instance_blob_storage.test.resource_group_name
  location            = azurerm_resource_group.test.location
  vault_name          = azurerm_data_protection_backup_instance_blob_storage.test.vault_name
  storage_account_id  = azurerm_storage_account.test.id
  policy_id           = azurerm_data_protection_backup_policy_blob_storage.test.id
}
`, config)
...
resource "azurerm_data_protection_backup_instance_blob_storage" "test" {
  ...
  vault_name          = azurerm_data_protection_backup_vault.test.name
  storage_account_id  = azurerm_storage_account.test.id
  policy_id           = azurerm_data_protection_backup_policy_blob_storage.another.id
}
`, template, data.RandomInteger)
}
