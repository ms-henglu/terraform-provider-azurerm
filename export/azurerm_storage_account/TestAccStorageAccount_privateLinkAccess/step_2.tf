...
resource "azurerm_subnet" "blob_endpoint" {
  name                 = "acctestsnetblobendpoint-230106032017364120"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_subnet" "table_endpoint" {
  name                 = "acctestsnettableendpoint-230106032017364120"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_private_endpoint" "blob" {
  name                = "acctest-privatelink-blob-230106032017364120"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  subnet_id           = azurerm_subnet.blob_endpoint.id

  private_service_connection {
    name                           = "acctest-privatelink-mssc-230106032017364120"
...
resource "azurerm_private_endpoint" "table" {
  name                = "acctest-privatelink-table-230106032017364120"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
