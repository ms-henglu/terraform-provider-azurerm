...
resource "azurerm_resource_group" "test" {
  name     = "acctest220121044834092329"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-220121044834092329"
...
resource "azurerm_policy_definition" "test" {
  name         = "acctestpol-220121044834092329"
  policy_type  = "Custom"
  mode         = "All"
  display_name = "acctestpol-220121044834092329"
...
resource "azurerm_resource_policy_assignment" "test" {
  name                 = "acctestpa-220121044834092329"
