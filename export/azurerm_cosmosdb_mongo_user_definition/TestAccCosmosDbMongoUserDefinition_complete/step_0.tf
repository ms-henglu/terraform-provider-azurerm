...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mongouserdef-230728025410670481"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca-230728025410670481"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-230728025410670481"
...
resource "azurerm_cosmosdb_mongo_role_definition" "test" {
  ...
  role_name                = "acctestmongoroledef230728025410670481"
}

resource "azurerm_cosmosdb_mongo_user_definition" "test" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  username                 = "myUserName-230728025410670481"
  password                 = "myPassword-230728025410670481"
