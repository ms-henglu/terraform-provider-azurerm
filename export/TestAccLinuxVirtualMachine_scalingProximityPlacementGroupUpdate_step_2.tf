...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220311042149539169"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-220311042149539169"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-220311042149539169"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_proximity_placement_group" "test" {
  name                = "acctestPPG-220311042149539169"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_proximity_placement_group" "second" {
  name                = "acctestPPG2-220311042149539169"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_linux_virtual_machine" "test" {
  name                         = "acctestVM-220311042149539169"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  size                         = "Standard_F2"
