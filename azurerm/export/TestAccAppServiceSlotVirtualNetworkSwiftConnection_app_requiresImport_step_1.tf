...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-appservice-220627132454044371"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-VNET-220627132454044371"
...
resource "azurerm_app_service_plan" "test" {
  name                = "acctest-ASP-220627132454044371"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_app_service" "test" {
  name                = "acctest-AS-220627132454044371"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_app_service_slot" "test-staging" {
  name                = "acctest-AS-220627132454044371-staging"
