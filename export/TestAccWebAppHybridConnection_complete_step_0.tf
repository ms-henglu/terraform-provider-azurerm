...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220712041934667263"
  location = "West Europe"
}

resource "azurerm_service_plan" "test" {
  name                = "acctestASP-220712041934667263"
...
resource "azurerm_relay_namespace" "test" {
  name                = "acctest-RN-220712041934667263"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_relay_hybrid_connection" "test" {
  name                 = "acctest-RHC-220712041934667263"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_windows_web_app" "test" {
  name                = "acctestWA-220712041934667263"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_web_app_hybrid_connection" "test" {
  ...
  hostname   = "acctest1el0ewyy.hostname"
  port       = 8081
