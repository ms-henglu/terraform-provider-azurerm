...
var _ resourceid.Formatter = NetworkSecurityAdminConfigurationId{}

func TestNetworkSecurityAdminConfigurationIDFormatter(t *testing.T) {
	actual := NewNetworkSecurityAdminConfigurationID("12345678-1234-9876-4563-123456789012", "resGroup1", "networkManager1", "securityConfiguration1").ID()
	expected := "/subscriptions/12345678-1234-9876-4563-123456789012/resourceGroups/resGroup1/providers/Microsoft.Network/networkManagers/networkManager1/securityAdminConfigurations/securityConfiguration1"
	if actual != expected {
		t.Fatalf("Expected %q but got %q", expected, actual)
	}
}

func TestNetworkSecurityAdminConfigurationID(t *testing.T) {
	testData := []struct {
		Input    string
		Error    bool
		Expected *NetworkSecurityAdminConfigurationId
	}{
	...
	for _, v := range testData {
		...
		actual, err := NetworkSecurityAdminConfigurationID(v.Input)
		if err != nil {
			if v.Error {
				continue
			}
