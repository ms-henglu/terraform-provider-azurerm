...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230707010040569604"
  location = "West Europe"
}

resource "azurerm_application_insights" "test" {
  name                = "acctestappinsights-230707010040569604"
...
resource "azurerm_application_insights_api_key" "test" {
  name                    = "acctestappinsightsapikey-230707010040569604"
  application_insights_id = azurerm_application_insights.test.id
  read_permissions        = ["aggregate", "api", "draft", "extendqueries", "search"]
}

resource "azurerm_application_insights" "test2" {
  name                = "acctestappinsights2-230707010040569604"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  application_type    = "web"
}
...
resource "azurerm_application_insights_api_key" "test2" {
  name                    = "acctestappinsightsapikey2-230707010040569604"
  application_insights_id = azurerm_application_insights.test2.id
  read_permissions        = ["aggregate", "api", "draft", "extendqueries", "search"]
}

resource "azurerm_key_vault" "test" {
  name                     = "acctestkv-xyuh4"
  location                 = azurerm_resource_group.test.location
  resource_group_name      = azurerm_resource_group.test.name
...
resource "azurerm_bot_channels_registration" "test" {
  name                = "acctestdf230707010040569604"
  location            = "global"
  resource_group_name = azurerm_resource_group.test.name
