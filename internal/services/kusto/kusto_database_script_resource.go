...
func resourceKustoDatabaseScriptCreateUpdate(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	if d.IsNewResource() {
		...
		if !utils.ResponseWasNotFound(existing.Response) {
			return tf.ImportAsExistsError("azurerm_kusto_database_script", id.ID())
		}
	}
...
func resourceKustoDatabaseScriptRead(d *pluginsdk.ResourceData, meta interface{}) error {
	...
	d.Set("name", id.Name)
	d.Set("database_name", parse.NewDatabaseID(id.SubscriptionId, id.ResourceGroup, id.ClusterName, id.Name).ID())
	if props := resp.ScriptProperties; props != nil {
		...
		d.Set("sas_token", props.ScriptURLSasToken)
	}
	return nil
}
