...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-spring-220326011237658128"
  location = "West Europe"
}

resource "azurerm_spring_cloud_service" "test" {
  name                = "acctest-sc-220326011237658128"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_spring_cloud_app" "test" {
  name                = "acctest-sca-220326011237658128"
  resource_group_name = azurerm_spring_cloud_service.test.resource_group_name
  service_name        = azurerm_spring_cloud_service.test.name
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca-220326011237658128"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  offer_type          = "Standard"
...
resource "azurerm_cosmosdb_cassandra_keyspace" "test" {
  name                = "acctest-ck-220326011237658128"
...
resource "azurerm_spring_cloud_app_cosmosdb_association" "test" {
  name                             = "acctestscac-220326011237658128"
