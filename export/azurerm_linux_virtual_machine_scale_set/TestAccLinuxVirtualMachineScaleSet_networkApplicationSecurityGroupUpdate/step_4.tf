...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-231020040746173287"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-231020040746173287"
...
resource "azurerm_application_security_group" "test" {
  name                = "acctestasg-231020040746173287"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_application_security_group" "other" {
  name                = "acctestasg2-231020040746173287"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-231020040746173287"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard_F2"
  instances           = 1
