...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230428050004990700"
  location = "West Europe"
}

resource "azurerm_public_ip" "test" {
  name                = "test-ip-230428050004990700"
...
resource "azurerm_lb" "test" {
  name                = "arm-test-loadbalancer-230428050004990700"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Basic"

  frontend_ip_configuration {
    name                 = "one-230428050004990700"
...
resource "azurerm_lb_nat_pool" "test" {
  ...
  name                           = "NatPool-230428050004990700"
  protocol                       = "Tcp"
  frontend_port_start            = 80
  frontend_port_end              = 81
  backend_port                   = 3389
  frontend_ip_configuration_name = "one-230428050004990700"
}
