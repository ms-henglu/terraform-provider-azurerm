...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-231013043043816849"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF231013043043816849"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone231013043043816849.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-231013043043816849"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-231013043043816849"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol231013043043816849"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
