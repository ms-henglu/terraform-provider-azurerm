...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-wps-240112035201549842"
  location = "West Europe"
}

resource "azurerm_web_pubsub" "test" {
  name                = "acctest-webpubsub-240112035201549842"
...
resource "azurerm_user_assigned_identity" "test1" {
  name                = "acctest-uai1-240112035201549842"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
resource "azurerm_user_assigned_identity" "test2" {
  name                = "acctest-uai2-240112035201549842"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
resource "azurerm_web_pubsub_hub" "test" {
  name          = "acctestwpsh240112035201549842"
