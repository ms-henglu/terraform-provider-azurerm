...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220610023212912514"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctestehn-220610023212912514"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteh-220610023212912514"
...
resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-220610023212912514"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_eventhub_namespace" "updated" {
  name                = "acctestehn2-220610023212912514"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  capacity            = 1
...
resource "azurerm_eventhub" "updated" {
  name                = "acctesteh2-220610023212912514"
...
resource "azurerm_stream_analytics_output_eventhub" "test" {
  name                      = "acctestinput-220610023212912514"
