...
resource "azurerm_data_protection_backup_policy_disk" "test" {
  ...
  backup_rules {
    name                     = "BackupHourly"
    repeating_time_intervals = ["R/2021-05-20T04:54:23+00:00/PT4H"]
  }
...
resource "azurerm_data_protection_backup_instance_disk" "test" {
  ...
  policy_id                    = azurerm_data_protection_backup_policy_disk.test.id
}
...
resource "azurerm_data_protection_backup_instance_disk" "import" {
  ...
  resource_group_name          = azurerm_data_protection_backup_instance_disk.test.resource_group_name
  location                     = azurerm_resource_group.test.location
  vault_name                   = azurerm_data_protection_backup_instance_disk.test.vault_name
  disk_id                      = azurerm_managed_disk.test.id
  snapshot_resource_group_name = azurerm_resource_group.test.name
  policy_id                    = azurerm_data_protection_backup_policy_disk.test.id
}
