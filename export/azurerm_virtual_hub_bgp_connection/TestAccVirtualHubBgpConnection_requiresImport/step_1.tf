...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-VHub-230810143941017294"
  location = "West Europe"
}

resource "azurerm_virtual_hub" "test" {
  name                = "acctest-VHub-230810143941017294"
...
resource "azurerm_public_ip" "test" {
  name                = "acctest-PIP-230810143941017294"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctest-VNet-230810143941017294"
...
resource "azurerm_virtual_hub_ip" "test" {
  name                         = "acctest-VHub-IP-230810143941017294"
...
resource "azurerm_virtual_hub_bgp_connection" "test" {
  name           = "acctest-VHub-BgpConnection-230810143941017294"
  virtual_hub_id = azurerm_virtual_hub.test.id
  peer_asn       = 65514
