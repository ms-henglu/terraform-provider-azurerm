...
resource "azurerm_resource_group" "test" {
  name     = "acctest-rg-230512004718232339"
  location = "West Europe"
}


resource "azurerm_user_assigned_identity" "test" {
  name                = "acctestUAI-230512004718232339"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_storage_account" "test" {
  name                = "unlikely23exst2acctmex43"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_resource_deployment_script_azure_power_shell" "test" {
  name                = "acctest-rdsaps-230512004718232339"
  resource_group_name = azurerm_resource_group.test.name
  location            = "West Europe"
  version             = "8.3"
...
EOF
  ...
  container {
    container_group_name = "cgn-230512004718232339"
  }
