...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211203013551343671"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-211203013551343671"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-211203013551343671"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_dedicated_host_group" "test" {
  name                        = "acctestDHG-211203013551343671"
  resource_group_name         = azurerm_resource_group.test.name
  location                    = azurerm_resource_group.test.location
...
resource "azurerm_dedicated_host" "test" {
  name                    = "acctestDH-211203013551343671"
...
resource "azurerm_dedicated_host" "second" {
  name                    = "acctestDH2-211203013551343671"
  dedicated_host_group_id = azurerm_dedicated_host_group.test.id
  location                = azurerm_resource_group.test.location
  sku_name                = "DSv3-Type1"
  platform_fault_domain   = 1
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-211203013551343671"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
