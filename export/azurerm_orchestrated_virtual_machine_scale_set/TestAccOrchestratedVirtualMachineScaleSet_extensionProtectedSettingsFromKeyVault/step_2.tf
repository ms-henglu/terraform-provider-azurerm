...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-OVMSS-231020040746208813"
  location = "West Europe"
}


resource "azurerm_public_ip" "test" {
  name                = "acctpip-231020040746208813"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctvn-231020040746208813"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-231020040746208813"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_nat_gateway" "test" {
  name                    = "acctng-231020040746208813"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
  sku_name                = "Standard"
...
resource "azurerm_key_vault" "test" {
  ...
  name                   = "acctestkv${count.index}xhw67"
  location               = azurerm_resource_group.test.location
  resource_group_name    = azurerm_resource_group.test.name
...
resource "azurerm_orchestrated_virtual_machine_scale_set" "test" {
  name                = "acctestOVMSS-231020040746208813"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    linux_configuration {
      computer_name_prefix = "testvm-231020040746208813"
