...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-network-manager-231218072256372128"
...
resource "azurerm_network_manager" "test" {
  name                = "acctest-nm-231218072256372128"
...
resource "azurerm_network_manager_network_group" "test" {
  name               = "acctest-nmng-231218072256372128"
  network_manager_id = azurerm_network_manager.test.id
}

resource "azurerm_virtual_network" "test" {
  name                    = "acctest-vnet-231218072256372128"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_network_manager_network_group" "test2" {
  name               = "acctest-nmng2-231218072256372128"
  network_manager_id = azurerm_network_manager.test.id
}

resource "azurerm_network_manager_connectivity_configuration" "test" {
  name                            = "acctest-nmcc-231218072256372128"
  network_manager_id              = azurerm_network_manager.test.id
