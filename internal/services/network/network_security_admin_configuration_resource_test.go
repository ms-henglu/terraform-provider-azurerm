...
func (r NetworkSecurityAdminConfigurationResource) Exists(ctx context.Context, client *clients.Client, state *terraform.InstanceState) (*bool, error) {
	id, err := parse.NetworkSecurityAdminConfigurationID(state.ID)
	if err != nil {
		return nil, err
	}
	resp, err := client.Network.SecurityAdminConfigurationClient.Get(ctx, id.ResourceGroup, id.NetworkManagerName, id.SecurityAdminConfigurationName)
	if err != nil {
		if utils.ResponseWasNotFound(resp.Response) {
			return utils.Bool(false), nil
		}
		return nil, fmt.Errorf("retrieving Network SecurityAdminConfiguration (%q): %+v", id, err)
	}
...
resource "azurerm_network_manager" "test" {
  ...
  network_manager_scope_accesses = ["SecurityAdmin"]
...
resource "azurerm_network_security_admin_configuration" "import" {
  ...
  network_manager_id = azurerm_network_security_admin_configuration.test.network_manager_id
}
`, config)
}
