...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-iothub-220506020021690256"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-220506020021690256"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-220506020021690256"
...
resource "azurerm_eventhub_authorization_rule" "test" {
  name                = "acctest-220506020021690256"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_user_assigned_identity" "test" {
  name                = "acctestuai-220506020021690256"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
...
resource "azurerm_iothub" "test" {
  name                = "acctestIoTHub-220506020021690256"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
