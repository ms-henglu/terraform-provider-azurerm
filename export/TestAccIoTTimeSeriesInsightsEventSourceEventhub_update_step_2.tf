...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-tsi-220905050011252642"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-220905050011252642"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-220905050011252642"
...
resource "azurerm_eventhub_consumer_group" "test" {
  name                = "acctesteventhubcg-220905050011252642"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_eventhub_authorization_rule" "test" {
  name                = "acctest-220905050011252642"
  namespace_name      = azurerm_eventhub_namespace.test.name
  eventhub_name       = azurerm_eventhub.test.name
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_iot_time_series_insights_gen2_environment" "test" {
  name                = "acctest_tsie220905050011252642"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "L1"
  id_properties       = ["id"]
...
resource "azurerm_iot_time_series_insights_event_source_eventhub" "test" {
  name                     = "acctest_tsiesi220905050011252642"
  location                 = azurerm_resource_group.test.location
