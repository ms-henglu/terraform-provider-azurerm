...
resource "azurerm_data_protection_backup_instance_blob_storage" "example" {
  ...
  policy_id                = azurerm_data_protection_backup_policy_blob_storage.example.id
}
```
...
* `vault_name` - (Required) The name of the Backup Vault where the Backup Instance Blob Storage should exist. Changing this forces a new Backup Instance Blob Storage to be created.
