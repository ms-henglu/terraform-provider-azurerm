...
locals {
  vm_name = "acctestvmbmrmo"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240112224137982090"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-240112224137982090"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-240112224137982090"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_capacity_reservation" "test" {
  name                          = "acctest-ccr-240112224137982090"
...
resource "azurerm_capacity_reservation_group" "test2" {
  name                = "acctest-ccrg2-240112224137982090"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}
