...
resource "azurerm_resource_group" "test" {
  name     = "acctestrg-vmware-nat-230203064310668225"
  location = "centralus"
}
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip-230203064310668225"
  location            = "centralus"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctest-VirtualNetwork-230203064310668225"
  location            = "centralus"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network_gateway" "test" {
  name                = "acctestvnetgw-230203064310668225"
...
resource "azurerm_vmware_private_cloud" "test" {
  name                = "acctest-PC-230203064310668225"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku_name            = "av36"
...
resource "azurerm_vmware_express_route_authorization" "test" {
  name             = "acctest-VmwareAuthorization-230203064310668225"
  private_cloud_id = azurerm_vmware_private_cloud.test.id
}

resource "azurerm_virtual_network_gateway_connection" "test" {
  name                = "acctestvnetgwconn-230203064310668225"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "netappSubnet" {
  name                 = "acctest-Subnet-230203064310668225"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_netapp_account" "test" {
  name                = "acctest-NetAppAccount-230203064310668225"
  location            = "central us"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_netapp_pool" "test" {
  name                = "acctest-NetAppPool-230203064310668225"
  location            = "centralus"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_netapp_volume" "test" {
  name                            = "acctest-NetAppVolume-230203064310668225"
  location                        = "centralus"
  resource_group_name             = azurerm_resource_group.test.name
  account_name                    = azurerm_netapp_account.test.name
  pool_name                       = azurerm_netapp_pool.test.name
  volume_path                     = "my-unique-file-path-230203064310668225"
  service_level                   = "Standard"
...
resource "azurerm_vmware_netapp_volume_attachment" "test" {
  name              = "acctest-vmwareattachment-230203064310668225"
