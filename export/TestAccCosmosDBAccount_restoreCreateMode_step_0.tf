...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cosmos-220422024958024572"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test1" {
  name                = "acctest-ca-220422024958024572"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-220422024958024572"
...
resource "azurerm_cosmosdb_mongo_collection" "test" {
  name                = "acctest-mongodb-coll-220422024958024572"
...
resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca2-220422024958024572"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  offer_type          = "Standard"
  kind                = "MongoDB"
