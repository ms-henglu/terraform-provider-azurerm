...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220603005116930630"
  location = "West Europe"
}

resource "azurerm_mssql_server" "test_primary" {
  name                         = "acctestmssql220603005116930630-primary"
...
resource "azurerm_mssql_server" "test_secondary" {
  name                         = "acctestmssql220603005116930630-secondary"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = "West US 2"
  version                      = "12.0"
  administrator_login          = "mradministrator"
...
resource "azurerm_mssql_database" "test" {
  name        = "acctestdb220603005116930630"
...
resource "azurerm_mssql_failover_group" "test" {
  name      = "acctestsfg220603005116930630"
  server_id = azurerm_mssql_server.test_primary.id
