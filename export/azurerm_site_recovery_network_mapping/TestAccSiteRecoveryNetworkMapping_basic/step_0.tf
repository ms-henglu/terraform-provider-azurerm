...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-recovery-230407023959752265-1"
  location = "West Europe"
}

resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-vault-230407023959752265"
...
resource "azurerm_site_recovery_fabric" "test1" {
  ...
  name                = "acctest-fabric1-230407023959752265"
  location            = azurerm_resource_group.test.location
}
...
resource "azurerm_site_recovery_fabric" "test2" {
  resource_group_name = azurerm_resource_group.test.name
  recovery_vault_name = azurerm_recovery_services_vault.test.name
  name                = "acctest-fabric2-230407023959752265"
  location            = "West US 2"
  depends_on          = [azurerm_site_recovery_fabric.test1]
}

resource "azurerm_virtual_network" "test1" {
  name                = "network1-230407023959752265"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test2" {
  name                = "network2-230407023959752265"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_site_recovery_network_mapping" "test" {
  resource_group_name         = azurerm_resource_group.test.name
  recovery_vault_name         = azurerm_recovery_services_vault.test.name
  name                        = "mapping-230407023959752265"
