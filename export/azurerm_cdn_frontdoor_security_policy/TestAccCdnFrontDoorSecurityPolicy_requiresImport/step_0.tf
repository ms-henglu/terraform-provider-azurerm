...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-221216013159912323"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF221216013159912323"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone221216013159912323.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-221216013159912323"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-221216013159912323"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol221216013159912323"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
