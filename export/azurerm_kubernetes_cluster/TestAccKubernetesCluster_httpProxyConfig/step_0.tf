...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230106031252270306"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet230106031252270306"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet230106031252270306"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "test-nic230106031252270306"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                            = "vm-test-proxy230106031252270306"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_B1s"
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks230106031252270306"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = "acctestaks230106031252270306"
  kubernetes_version  = "1.24.3"

  linux_profile {
    admin_username = "acctestuser230106031252270306"
    ssh_key {
