...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220527024513768893"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-220527024513768893"
...
resource "azurerm_virtual_machine_scale_set" "test" {
  name                = "acctvmss-220527024513768893"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    computer_name_prefix = "testvm-220527024513768893"
  ...
  network_profile {
    name    = "TestNetworkProfile-220527024513768893"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-220527024513768893"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-220527024513768893@example.com"]
    }
  }
}
