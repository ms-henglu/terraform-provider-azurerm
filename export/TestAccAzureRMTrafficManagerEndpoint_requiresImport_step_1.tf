...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-traffic-210928060008661220"
  location = "West Europe"
}

resource "azurerm_traffic_manager_profile" "test" {
  name                   = "acctest-TMP-210928060008661220"
  resource_group_name    = azurerm_resource_group.test.name
  traffic_routing_method = "Weighted"

  dns_config {
    relative_name = "acctest-tmp-210928060008661220"
    ttl           = 30
  }
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpublicip-210928060008661220"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  domain_name_label   = "acctestpublicip-210928060008661220"
}

resource "azurerm_traffic_manager_endpoint" "testAzure" {
  name                = "acctestend-azure210928060008661220"
  type                = "azureEndpoints"
...
resource "azurerm_traffic_manager_endpoint" "testExternal" {
  name                = "acctestend-external210928060008661220"
