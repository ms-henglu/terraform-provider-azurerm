...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230421022833611157"
  location = "West Europe"
}

resource "azurerm_logic_app_workflow" "test" {
  name                = "acctestlogicapp-230421022833611157"
  location            = "West Europe"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_logic_app_workflow" "test2" {
  name                = "acctestlogicapp2-230421022833611157"
  location            = "West Europe"
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_security_center_automation" "test" {
  name                = "acctestautomation-230421022833611157"
  location            = "West Europe"
  resource_group_name = azurerm_resource_group.test.name
