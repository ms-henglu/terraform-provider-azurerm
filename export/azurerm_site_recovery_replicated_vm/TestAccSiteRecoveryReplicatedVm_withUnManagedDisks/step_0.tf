...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-recovery-240119022723232831-1"
  location = "West Europe"
}

resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-recovery-240119022723232831-2"
  location = "West US 2"
}

resource "azurerm_managed_disk" "test" {
  name                 = "acctestd-240119022723232831"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctvn-240119022723232831"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-240119022723232831"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "acctni-240119022723232831"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "test" {
  name                  = "acctvm-240119022723232831"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  os_profile {
    computer_name  = "hn240119022723232831"
...
resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-vault-240119022723232831"
...
resource "azurerm_site_recovery_fabric" "test1" {
  ...
  name                = "acctest-fabric1-240119022723232831"
  location            = azurerm_resource_group.test.location
}
...
resource "azurerm_site_recovery_fabric" "test2" {
  resource_group_name = azurerm_resource_group.test2.name
  recovery_vault_name = azurerm_recovery_services_vault.test.name
  name                = "acctest-fabric2-240119022723232831"
  location            = azurerm_resource_group.test2.location
...
resource "azurerm_site_recovery_protection_container" "test1" {
  ...
  name                 = "acctest-protection-cont1-240119022723232831"
}
...
resource "azurerm_site_recovery_protection_container" "test2" {
  ...
  name                 = "acctest-protection-cont2-240119022723232831"
}

resource "azurerm_site_recovery_replication_policy" "test" {
  resource_group_name                                  = azurerm_resource_group.test2.name
  recovery_vault_name                                  = azurerm_recovery_services_vault.test.name
  name                                                 = "acctest-policy-240119022723232831"
...
resource "azurerm_site_recovery_protection_container_mapping" "test" {
  ...
  name                                      = "mapping-240119022723232831"
}

resource "azurerm_virtual_network" "test2" {
  name                = "net-240119022723232831-2"
  resource_group_name = azurerm_resource_group.test2.name
...
resource "azurerm_public_ip" "test-recovery" {
  name                = "pubip240119022723232831-recovery"
