...
locals {
  vm_name = "acctestvmnh3r4"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221216013235572923"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-221216013235572923"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-221216013235572923"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig221216013235572923"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_gallery_application" "test" {
  name              = "acctest-app-221216013235572923"
...
resource "azurerm_gallery_application" "test2" {
  name              = "acctest-app2-221216013235572923"
  gallery_id        = azurerm_shared_image_gallery.test.id
  location          = azurerm_shared_image_gallery.test.location
  supported_os_type = "Windows"
}
