...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-230414020853858047"
  location = "West Europe"
}

resource "azurerm_dns_zone" "test" {
  name                = "acctest-dns-zone230414020853858047.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "acctest-profile-230414020853858047"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "accTest-origin-group-230414020853858047"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "acctest-origin-230414020853858047"
...
resource "azurerm_cdn_frontdoor_endpoint" "test" {
  name                     = "acctest-endpoint-230414020853858047"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
}

resource "azurerm_cdn_frontdoor_route" "test" {
  name                          = "acctest-route-230414020853858047"
...
resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "acctest-custom-domain-230414020853858047"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
  dns_zone_id              = azurerm_dns_zone.test.id
  host_name                = join(".", ["tihja4wb", azurerm_dns_zone.test.name])
