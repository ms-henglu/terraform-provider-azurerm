...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-230505045910147042"
  location = "West Europe"
}

resource "azurerm_automation_account" "test" {
  name                = "acctestAA-230505045910147042"
...
resource "azurerm_automation_credential" "test" {
  name                    = "acctest-230505045910147042"
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230505045910147042"
...
resource "azurerm_network_interface" "test" {
  name                = "acctni-230505045910147042"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-230505045910147042"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_automation_hybrid_runbook_worker" "test" {
  ...
  worker_id               = "1e476593-c6bb-4af5-8763-a3ef2f19f68e"
