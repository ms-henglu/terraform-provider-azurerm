...
func TestAccDNSResolverInboundEndpoint_basic(t *testing.T) {
	data := acceptance.BuildTestData(t, "azurerm_dns_resolver_inbound_endpoint", "test")
...
func (r DNSResolverInboundEndpointResource) Exists(ctx context.Context, clients *clients.Client, state *pluginsdk.InstanceState) (*bool, error) {
	...
	client := clients.DNSResolver.InboundEndpointsClient
...
resource "azurerm_dns_resolver_inbound_endpoint" "test" {
  name            = "acctest-drie-%d"
  dns_resolver_id = azurerm_dns_resolver.test.id
...
resource "azurerm_dns_resolver_inbound_endpoint" "import" {
  name            = azurerm_dns_resolver_inbound_endpoint.test.name
  dns_resolver_id = azurerm_dns_resolver_inbound_endpoint.test.dns_resolver_id
  location        = azurerm_dns_resolver_inbound_endpoint.test.location
  ip_configurations {
    subnet_id = azurerm_subnet.test.id
  }
}
