...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-231013044401013581"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctestehn-231013044401013581"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteh-231013044401013581"
...
resource "azurerm_eventhub_consumer_group" "test" {
  name                = "acctesteventhubcg-231013044401013581"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-231013044401013581"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_stream_analytics_stream_input_eventhub" "test" {
  name                         = "acctestinput-231013044401013581"
