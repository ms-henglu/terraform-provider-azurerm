...
resource "azurerm_resource_group" "test" {
  name     = "acctestRGhdi-220712042337179300"
...
resource "azurerm_storage_container" "test" {
  name                  = "acctestsc-220712042337179300"
  storage_account_name  = azurerm_storage_account.test.name
  container_access_type = "private"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestVnet-220712042337179300"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "test" {
  name                 = "acctestSubnet-220712042337179300"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "acctestNSG-220712042337179300"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azuread_group" "test" {
  display_name     = "AAD DC Administrators za17i"
...
resource "azuread_user" "test" {
  user_principal_name = "acctestAADDSAdminUser-za17i@${data.azuread_domains.test.domains.0.domain_name}"
  display_name        = "acctestAADDSAdminUser-za17i"
...
resource "azurerm_hdinsight_kafka_cluster" "test" {
  name                = "acctesthdikafka-220712042337179300"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  cluster_version     = "4.0"
  tier                = "Premium"
