...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-OVMSS-220422011655543973"
  location = "West Europe"
}


resource "azurerm_public_ip" "test" {
  name                = "acctpip-220422011655543973"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctvn-220422011655543973"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-220422011655543973"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_nat_gateway" "test" {
  name                    = "acctng-220422011655543973"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
  sku_name                = "Standard"
...
resource "azurerm_orchestrated_virtual_machine_scale_set" "test" {
  name                = "acctestOVMSS-220422011655543973"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    linux_configuration {
      computer_name_prefix = "testvm-220422011655543973"
  ...
  network_interface {
    name    = "TestNetworkProfile-220422011655543973"
    primary = true
