...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-230825024148506072"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF230825024148506072"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone230825024148506072.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-230825024148506072"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-230825024148506072"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol230825024148506072"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
