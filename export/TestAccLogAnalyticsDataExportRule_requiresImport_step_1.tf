...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-la-220909034532006943"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-220909034532006943"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctest-EHN-220909034532006943"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_eventhub" "test" {
  name                = "acctest-EH-220909034532006943"
...
resource "azurerm_log_analytics_data_export_rule" "test" {
  name                    = "acctest-DER-220909034532006943"
  resource_group_name     = azurerm_resource_group.test.name
