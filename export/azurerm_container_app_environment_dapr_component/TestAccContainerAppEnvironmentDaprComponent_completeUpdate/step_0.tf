...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-CAEnv-230324051819531799"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestCAEnv-230324051819531799"
...
resource "azurerm_container_app_environment" "test" {
  name                       = "accTest-CAEnv230324051819531799"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
...
resource "azurerm_container_app_environment_dapr_component" "test" {
  name                         = "acctest-dapr-230324051819531799"
