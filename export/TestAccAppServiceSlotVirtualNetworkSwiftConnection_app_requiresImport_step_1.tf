...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-appservice-220520041325153235"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-VNET-220520041325153235"
...
resource "azurerm_app_service_plan" "test" {
  name                = "acctest-ASP-220520041325153235"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_app_service" "test" {
  name                = "acctest-AS-220520041325153235"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_app_service_slot" "test-staging" {
  name                = "acctest-AS-220520041325153235-staging"
