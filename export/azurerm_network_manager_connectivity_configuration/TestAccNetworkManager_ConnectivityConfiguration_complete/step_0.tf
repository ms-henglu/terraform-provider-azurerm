...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-network-manager-230324052517207506"
...
resource "azurerm_network_manager" "test" {
  name                = "acctest-nm-230324052517207506"
...
resource "azurerm_network_manager_network_group" "test" {
  name               = "acctest-nmng-230324052517207506"
  network_manager_id = azurerm_network_manager.test.id
}

resource "azurerm_virtual_network" "test" {
  name                    = "acctest-vnet-230324052517207506"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_network_manager_network_group" "test2" {
  name               = "acctest-nmng2-230324052517207506"
  network_manager_id = azurerm_network_manager.test.id
}

resource "azurerm_network_manager_connectivity_configuration" "test" {
  name                            = "acctest-nmcc-230324052517207506"
  network_manager_id              = azurerm_network_manager.test.id
