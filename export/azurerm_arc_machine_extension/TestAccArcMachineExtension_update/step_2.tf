...
locals {
  random_uuid = "d6676e23-b6e9-270d-e6f5-176cc015d0eb"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230721011803160877"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230721011803160877"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-230721011803160877"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip-230721011803160877"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                            = "acctestVM-230721011803160877"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_F2"
  admin_username                  = "adminuser"
  admin_password                  = "6v6#5Y178g"
...
resource "azurerm_arc_machine_extension" "test" {
  name                      = "acctest-hcme-230721011803160877"
