...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230120051707878348"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230120051707878348"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-230120051707878348"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-230120051707878348"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  size                = "Standard_F2"
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig230120051707878348"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_shared_image" "test" {
  name                = "acctestimg230120051707878348"
  ...
  identifier {
    publisher = "AccTesPublisher230120051707878348"
    offer     = "AccTesOffer230120051707878348"
    sku       = "AccTesSku230120051707878348"
  }
}
...
resource "azurerm_managed_disk" "test" {
  name                       = "acctestd-230120051707878348"
  location                   = azurerm_resource_group.test.location
  resource_group_name        = azurerm_resource_group.test.name
  os_type                    = "Linux"
