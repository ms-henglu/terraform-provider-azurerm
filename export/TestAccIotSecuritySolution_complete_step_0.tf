...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-security-211203161841352391"
  location = "West Europe"
}

resource "azurerm_iothub" "test" {
  name                = "acctestIoTHub-211203161841352391"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-211203161841352391"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_iot_security_solution" "test" {
  name                       = "acctest-Iot-Security-Solution-211203161841352391"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
