...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-spring-220923012354198865"
  location = "West Europe"
}

resource "azurerm_spring_cloud_service" "test" {
  name                = "acctest-sc-220923012354198865"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_spring_cloud_app" "test" {
  name                = "acctest-sca-220923012354198865"
  resource_group_name = azurerm_spring_cloud_service.test.resource_group_name
  service_name        = azurerm_spring_cloud_service.test.name
}

resource "azurerm_mysql_server" "test" {
  name                             = "acctestmysqlsvr-220923012354198865"
  location                         = azurerm_resource_group.test.location
  resource_group_name              = azurerm_resource_group.test.name
  sku_name                         = "GP_Gen5_2"
...
resource "azurerm_mysql_database" "test" {
  name                = "acctest-db-220923012354198865"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_mysql_database" "updated" {
  name                = "acctest-db2-220923012354198865"
  resource_group_name = azurerm_resource_group.test.name
  server_name         = azurerm_mysql_server.test.name
  charset             = "utf8"
  collation           = "utf8_unicode_ci"
...
resource "azurerm_spring_cloud_app_mysql_association" "test" {
  name                = "acctestscamb-220923012354198865"
