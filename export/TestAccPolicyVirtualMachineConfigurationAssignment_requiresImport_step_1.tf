...
locals {
  vm_name = "acctestvm47lnl"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-210830084325271800"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-210830084325271800"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-210830084325271800"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_policy_virtual_machine_configuration_assignment" "test" {
  name               = "acctest-gca-210830084325271800"
