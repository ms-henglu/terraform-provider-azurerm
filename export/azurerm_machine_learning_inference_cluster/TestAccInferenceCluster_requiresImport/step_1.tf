...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ml-221021031421032177"
...
resource "azurerm_application_insights" "test" {
  name                = "acctestai-221021031421032177"
...
resource "azurerm_machine_learning_workspace" "test" {
  name                    = "acctest-MLW2210210314210321"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet221021031421032177"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet221021031421032177"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks221021031421032177"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = join("", ["acctestaks", azurerm_resource_group.test.location])
  node_resource_group = "acctestRGAKS-221021031421032177"
