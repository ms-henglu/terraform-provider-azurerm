...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-231013043903022369"
  location = "West Europe"
}

resource "azurerm_mssql_server" "test_primary" {
  name                         = "acctestmssql231013043903022369-primary"
...
resource "azurerm_mssql_server" "test_secondary" {
  name                         = "acctestmssql231013043903022369-secondary"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = "West US 2"
  version                      = "12.0"
  administrator_login          = "mradministrator"
...
resource "azurerm_mssql_database" "test" {
  name        = "acctestdb231013043903022369"
...
resource "azurerm_mssql_failover_group" "test" {
  name      = "acctestsfg231013043903022369"
  server_id = azurerm_mssql_server.test_primary.id
