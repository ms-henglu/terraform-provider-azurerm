...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-210825041102402828"
  location = "West Europe"
}

resource "azurerm_sql_server" "test" {
  name                         = "acctest-sqlserver-210825041102402828"
...
resource "azurerm_mssql_database" "test" {
  name         = "acctest-db-210825041102402828"
...
resource "azurerm_resource_group" "second" {
  name     = "acctestRG-mssql2-210825041102402828"
  location = "West US 2"
}

resource "azurerm_sql_server" "second" {
  name                         = "acctest-sqlserver2-210825041102402828"
...
resource "azurerm_mssql_database" "secondary" {
  name                        = "acctest-dbs-210825041102402828"
