...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-211126031222768188"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet211126031222768188"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip211126031222768188"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall211126031222768188"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall_nat_rule_collection" "test" {
  name                = "acctestnrc-211126031222768188"
