...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-PAN-240105064401702833"
  location = "West Europe"
}

resource "azurerm_palo_alto_local_rulestack_certificate" "test" {
  name         = "testacc-palc-240105064401702833"
...
resource "azurerm_palo_alto_local_rulestack_certificate" "trust" {
  name         = "testacc-palcT-240105064401702833"
  rulestack_id = azurerm_palo_alto_local_rulestack.test.id
...
resource "azurerm_palo_alto_local_rulestack_certificate" "untrust" {
  name         = "testacc-palcU-240105064401702833"
  rulestack_id = azurerm_palo_alto_local_rulestack.test.id
...
resource "azurerm_palo_alto_local_rulestack" "test" {
  name                = "testAcc-palrs-240105064401702833"
  resource_group_name = azurerm_resource_group.test.name
  location            = "West Europe"
}
...
resource "azurerm_palo_alto_local_rulestack_rule" "test" {
  name         = "testacc-palr-240105064401702833"
  rulestack_id = azurerm_palo_alto_local_rulestack.test.id
  priority     = 100
  action       = "Allow"
