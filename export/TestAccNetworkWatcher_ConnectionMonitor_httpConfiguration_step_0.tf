...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-Watcher-211210024904046557"
  location = "West Europe"
}

resource "azurerm_network_watcher" "test" {
  name                = "acctest-Watcher-211210024904046557"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-Vnet-211210024904046557"
...
resource "azurerm_network_interface" "src" {
  name                = "acctest-SrcNIC-211210024904046557"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "src" {
  name                  = "acctest-SrcVM-211210024904046557"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_os_disk {
    name              = "osdisk-src211210024904046557"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "hostname211210024904046557"
...
resource "azurerm_network_interface" "dest" {
  name                = "acctest-DestNic-211210024904046557"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "dest" {
  name                  = "acctest-DestVM-211210024904046557"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_os_disk {
    name              = "osdisk-dest211210024904046557"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
  }
...
resource "azurerm_network_connection_monitor" "test" {
  name               = "acctest-CM-211210024904046557"
