...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-Watcher-231016034430999588"
  location = "West Europe"
}

resource "azurerm_network_watcher" "test" {
  name                = "acctest-Watcher-231016034430999588"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-Vnet-231016034430999588"
...
resource "azurerm_network_interface" "src" {
  name                = "acctest-SrcNIC-231016034430999588"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "src" {
  name                  = "acctest-SrcVM-231016034430999588"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_os_disk {
    name              = "osdisk-src231016034430999588"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "hostname231016034430999588"
...
resource "azurerm_network_interface" "dest" {
  name                = "acctest-DestNic-231016034430999588"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "dest" {
  name                  = "acctest-DestVM-231016034430999588"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_os_disk {
    name              = "osdisk-dest231016034430999588"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
  }
...
resource "azurerm_network_connection_monitor" "test" {
  name               = "acctest-CM-231016034430999588"
