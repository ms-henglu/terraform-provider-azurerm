...
locals {
  number            = "221111013238941936"
  location          = "West Europe"
  domain_name_label = "acctestvm-4b2nf"
  random_string     = "4b2nf"
  admin_username    = "testadmin221111013238941936"
  admin_password    = "Password1234!221111013238941936"
}
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig221111013238941936"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_shared_image" "test" {
  name                = "acctestimg221111013238941936"
  ...
  identifier {
    publisher = "AccTesPublisher221111013238941936"
    offer     = "AccTesOffer221111013238941936"
    sku       = "AccTesSku221111013238941936"
  }
}
