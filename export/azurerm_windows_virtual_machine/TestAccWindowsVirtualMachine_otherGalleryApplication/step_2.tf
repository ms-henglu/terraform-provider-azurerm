...
locals {
  vm_name = "acctestvmol93z"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230613071532244678"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230613071532244678"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-230613071532244678"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig230613071532244678"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_gallery_application" "test" {
  name              = "acctest-app-230613071532244678"
...
resource "azurerm_gallery_application" "test2" {
  name              = "acctest-app2-230613071532244678"
  gallery_id        = azurerm_shared_image_gallery.test.id
  location          = azurerm_shared_image_gallery.test.location
  supported_os_type = "Windows"
}
