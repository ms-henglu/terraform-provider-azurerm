...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-lab-240119025219823266"
  location = "West Europe"
}


resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig240119025219823266"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_shared_image" "test" {
  name                = "acctestimg240119025219823266"
  ...
  identifier {
    publisher = "AccTesPublisher240119025219823266"
    offer     = "AccTesOffer240119025219823266"
    sku       = "AccTesSku240119025219823266"
  }
}
...
resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-240119025219823266"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-sn-240119025219823266"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-240119025219823266"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-linux-240119025219823266"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  size                = "Standard_F2"
...
resource "azurerm_lab_service_plan" "test" {
  name                = "acctest-lslp-240119025219823266"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_lab_service_lab" "test" {
  name                = "acctest-lab-240119025219823266"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  title               = "Test Title"
