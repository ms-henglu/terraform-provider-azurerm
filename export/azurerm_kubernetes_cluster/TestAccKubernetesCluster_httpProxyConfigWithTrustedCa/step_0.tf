...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-231218071509722192"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet231218071509722192"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet231218071509722192"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "test-nic231218071509722192"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                            = "vm-test-proxy231218071509722192"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_B1s"
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks231218071509722192"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = "acctestaks231218071509722192"
  kubernetes_version  = "1.26.6"

  linux_profile {
    admin_username = "acctestuser231218071509722192"
    ssh_key {
