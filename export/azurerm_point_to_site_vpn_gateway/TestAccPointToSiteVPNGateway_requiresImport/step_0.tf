...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221028172539949899"
  location = "West Europe"
}

resource "azurerm_virtual_wan" "test" {
  name                = "acctestvwan-221028172539949899"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_virtual_hub" "test" {
  name                = "acctestvhub-221028172539949899"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_vpn_server_configuration" "test" {
  name                     = "acctestvpnsc-221028172539949899"
  resource_group_name      = azurerm_resource_group.test.name
  location                 = azurerm_resource_group.test.location
...
resource "azurerm_point_to_site_vpn_gateway" "test" {
  name                        = "acctestp2sVPNG-221028172539949899"
  location                    = azurerm_resource_group.test.location
  resource_group_name         = azurerm_resource_group.test.name
