...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mapp-221111013832895450"
  location = "West Europe"
}

resource "azurerm_managed_application_definition" "test" {
  name                = "acctestManagedAppDef221111013832895450"
...
resource "azurerm_managed_application" "test" {
  name                        = "acctestManagedApp221111013832895450"
  location                    = azurerm_resource_group.test.location
  resource_group_name         = azurerm_resource_group.test.name
  kind                        = "ServiceCatalog"
  managed_resource_group_name = "infraGroup221111013832895450"
  ...
  parameter_values = <<VALUES
	{
        ...
        "storageAccountNamePrefix": {"value": "storej1egv"},
