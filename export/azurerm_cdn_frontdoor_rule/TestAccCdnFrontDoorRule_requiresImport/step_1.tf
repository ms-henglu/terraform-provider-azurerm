...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-221021030919873955"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-221021030919873955"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "accTestOriginGroup-221021030919873955"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "accTestOrigin-221021030919873955"
...
resource "azurerm_cdn_frontdoor_rule_set" "test" {
  name                     = "accTestRuleSet221021030919873955"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
}
...
resource "azurerm_cdn_frontdoor_rule" "test" {
  ...
  name                      = "accTestRule221021030919873955"
