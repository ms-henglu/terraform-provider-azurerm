...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-221222034326971822"
  location = "West Europe"
}

resource "azurerm_dns_zone" "test" {
  name                = "acctest-dns-zone221222034326971822.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "acctest-profile-221222034326971822"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "accTest-origin-group-221222034326971822"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "acctest-origin-221222034326971822"
...
resource "azurerm_cdn_frontdoor_endpoint" "test" {
  name                     = "acctest-endpoint-221222034326971822"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
}

resource "azurerm_cdn_frontdoor_route" "test" {
  name                          = "acctest-route-221222034326971822"
...
resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "acctest-custom-domain-221222034326971822"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
  dns_zone_id              = azurerm_dns_zone.test.id
  host_name                = join(".", ["artdxztf", azurerm_dns_zone.test.name])
