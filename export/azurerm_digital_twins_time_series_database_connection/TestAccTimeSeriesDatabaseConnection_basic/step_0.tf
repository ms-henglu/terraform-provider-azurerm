...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-digitaltwin-230613071759727497"
  location = "West Europe"
}

resource "azurerm_digital_twins_instance" "test" {
  name                = "acctest-DT-230613071759727497"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-230613071759727497"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-230613071759727497"
...
resource "azurerm_kusto_database" "test" {
  name                = "acctestkd-230613071759727497"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_kusto_database_principal_assignment" "test" {
  name                = "acctestkdpa230613071759727497"
  resource_group_name = azurerm_resource_group.test.name
  cluster_name        = azurerm_kusto_cluster.test.name
...
resource "azurerm_digital_twins_time_series_database_connection" "test" {
  name                            = "connection-230613071759727497"
