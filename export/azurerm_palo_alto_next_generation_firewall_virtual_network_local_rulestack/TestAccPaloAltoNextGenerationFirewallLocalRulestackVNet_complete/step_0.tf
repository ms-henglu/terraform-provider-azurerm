...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-PANGFWVN-240105061331693123"
  location = "West Europe"
}

resource "azurerm_public_ip" "test" {
  name                = "acctestpublicip-240105061331693123"
...
resource "azurerm_public_ip" "egress" {
  name                = "acctestpublicip-240105061331693123-e"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  sku                 = "Standard"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet240105061331693123"
...
resource "azurerm_subnet" "test1" {
  name                 = "acctest-pangfw-240105061331693123-1"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "test2" {
  name                 = "acctest-pangfw-240105061331693123-2"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_palo_alto_local_rulestack" "test" {
  name                = "testAcc-palrs-240105061331693123"
  resource_group_name = azurerm_resource_group.test.name
  location            = "West Europe"
}

resource "azurerm_palo_alto_local_rulestack_rule" "test" {
  name         = "testacc-palr-240105061331693123"
...
resource "azurerm_palo_alto_next_generation_firewall_virtual_network_local_rulestack" "test" {
  name                = "acctest-ngfwvn-240105061331693123"
  resource_group_name = azurerm_resource_group.test.name
  rulestack_id        = azurerm_palo_alto_local_rulestack.test.id
