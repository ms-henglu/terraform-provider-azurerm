...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-postgresql-220415030931906919"
  location = "West Europe"
}


resource "azurerm_virtual_network" "test" {
  name                = "acctest-vn-220415030931906919"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-sn-220415030931906919"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_private_dns_zone" "test" {
  name                = "acc220415030931906919.postgres.database.azure.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_private_dns_zone_virtual_network_link" "test" {
  name                  = "acctestVnetZone220415030931906919.com"
...
resource "azurerm_postgresql_flexible_server" "test" {
  name                   = "acctest-fs-220415030931906919"
  resource_group_name    = azurerm_resource_group.test.name
  location               = azurerm_resource_group.test.location
