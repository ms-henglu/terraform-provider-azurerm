...
* `kind` - (Optional) The kind of the resource. Accepted values are `Linux` and `Windows`. Default to support both `Linux` and `Windows` if omitted it.
...
* `log_analytics_destination` - Zero or more `log_analytics_destination` blocks as defined below.

---

A `log_analytics_destination` block exports the following:

* `workspace_id` - The Customer ID of the Log Analytics workspace..
...
```shell
terraform import azurerm_monitor_data_collection_rule.example C:/Program Files/Git/subscriptions/00000000-0000-0000-0000-000000000000/resourceGroups/group1/providers/Microsoft.Insights/dataCollectionRules/rule1
```
