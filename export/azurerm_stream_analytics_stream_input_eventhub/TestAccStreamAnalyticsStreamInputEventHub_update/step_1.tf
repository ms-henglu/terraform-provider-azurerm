...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230316222422893805"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctestehn-230316222422893805"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteh-230316222422893805"
...
resource "azurerm_eventhub_consumer_group" "test" {
  name                = "acctesteventhubcg-230316222422893805"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-230316222422893805"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_eventhub_namespace" "updated" {
  name                = "acctestehn2-230316222422893805"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  capacity            = 1
...
resource "azurerm_eventhub" "updated" {
  name                = "acctesteh2-230316222422893805"
...
resource "azurerm_eventhub_consumer_group" "updated" {
  name                = "acctesteventhubcg2-230316222422893805"
  namespace_name      = azurerm_eventhub_namespace.updated.name
...
resource "azurerm_stream_analytics_stream_input_eventhub" "test" {
  name                         = "acctestinput-230316222422893805"
