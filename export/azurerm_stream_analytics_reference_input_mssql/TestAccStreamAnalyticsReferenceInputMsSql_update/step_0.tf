...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240119025936861343"
  location = "West Europe"
}

resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-240119025936861343"
...
resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-240119025936861343"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  version                      = "12.0"
...
resource "azurerm_mssql_database" "test" {
  name         = "acctest-db-240119025936861343"
...
resource "azurerm_stream_analytics_reference_input_mssql" "test" {
  name                      = "acctestinput-240119025936861343"
