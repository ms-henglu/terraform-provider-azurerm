...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220722052250211493"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-220722052250211493"
...
resource "azurerm_monitor_action_group" "test" {
  name                = "acctestActionGroup-220722052250211493"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_automation_account" "test" {
  name                = "acctestAA-220722052250211493"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_logic_app_workflow" "test" {
  name                = "acctestLA-220722052250211493"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
}
...
resource "azurerm_app_service_plan" "test" {
  name                = "acctestSP-220722052250211493"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_function_app" "test" {
  name                       = "acctestFA-220722052250211493"
  location                   = "${azurerm_resource_group.test.location}"
  resource_group_name        = "${azurerm_resource_group.test.name}"
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acceptanceTestEventHubNamespace-220722052250211493"
  location            = "West Europe"
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  capacity            = 1
