...
func resourceNetworkManager() *schema.Resource {
	return &schema.Resource{
		Create: resourceNetworkManagerCreateUpdate,
		Read:   resourceNetworkManagerRead,
		Update: resourceNetworkManagerCreateUpdate,
			...
			"network_manager_scope_accesses": {
				...
				Elem: &schema.Schema{
					...
					ValidateFunc: validation.StringInSlice([]string{
						string(network.ScopeAccessesSecurity),
						string(network.ScopeAccessesRouting),
						string(network.ScopeAccessesConnectivity),
					}, false),
				},
			},
...
func resourceNetworkManagerCreateUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	networkManagerScopeAccesses := make([]network.ScopeAccesses, 0)
	for _, item := range *(utils.ExpandStringSlice(d.Get("network_manager_scope_accesses").([]interface{}))) {
		networkManagerScopeAccesses = append(networkManagerScopeAccesses, (network.ScopeAccesses)(item))
	}
