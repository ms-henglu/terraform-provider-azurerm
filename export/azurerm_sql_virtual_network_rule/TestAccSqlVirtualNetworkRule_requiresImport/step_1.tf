...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240119025846955000"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvnet240119025846955000"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet240119025846955000"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_sql_server" "test" {
  name                         = "acctestsqlserver240119025846955000"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  version                      = "12.0"
  administrator_login          = "missadmin"
  administrator_login_password = "${md5(240119025846955000)}!"
}

resource "azurerm_sql_virtual_network_rule" "test" {
  name                                 = "acctestsqlvnetrule240119025846955000"
  resource_group_name                  = azurerm_resource_group.test.name
