...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-spring-240119022903094383"
  location = "West Europe"
}

resource "azurerm_spring_cloud_service" "test" {
  name                = "acctest-sc-240119022903094383"
...
resource "azurerm_spring_cloud_app" "test" {
  name                = "acctest-sca-240119022903094383"
...
resource "azurerm_application_insights" "test" {
  name                = "acctest-ai-240119022903094383"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_spring_cloud_application_insights_application_performance_monitoring" "test" {
  name                    = "acctest-apm-240119022903094383"
