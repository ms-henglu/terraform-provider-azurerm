...
resource "azurerm_resource_group" "test1" {
  name     = "acctestRG-api1-230324051554829971"
  location = "West Europe"
}

resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-api2-230324051554829971"
  location = "West US 2"
}

resource "azurerm_resource_group" "test3" {
  name     = "acctestRG-api3-230324051554829971"
  location = "East US 2"
}

resource "azurerm_api_management" "test" {
  name                      = "acctestAM-230324051554829971"
  publisher_name            = "pub1"
  ...
  hostname_configuration {
    proxy {
      host_name                    = "acctestAM-230324051554829971.azure-api.net"
