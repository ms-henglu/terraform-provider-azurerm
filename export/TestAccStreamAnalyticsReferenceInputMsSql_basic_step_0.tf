...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-211210025135485667"
  location = "West Europe"
}

resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-211210025135485667"
...
resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-211210025135485667"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  version                      = "12.0"
...
resource "azurerm_mssql_database" "test" {
  name      = "acctest-db-211210025135485667"
  server_id = azurerm_mssql_server.test.id
}


resource "azurerm_stream_analytics_reference_input_mssql" "test" {
  name                      = "acctestinput-211210025135485667"
