...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-storage-230616074835940152"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-VN-230616074835940152"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsub-230616074835940152"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_resource_group_template_deployment" "storage-containers" {
  name                = "acctest-strgctn-deployment-230616074835940152"
  resource_group_name = azurerm_storage_account.test.resource_group_name
  deployment_mode     = "Incremental"

  parameters_content = jsonencode({
    name = {
      value = "acctest-strgctn-hpc-230616074835940152"
    }
  })
...
resource "azurerm_hpc_cache" "test" {
  name                = "acctest-HPCC-230616074835940152"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
