...
resource "azurerm_network_interface" "client_single_subnet" {
  ...
  name                = "acctestnic-client-${count.index}-231020041508309601"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_availability_set" "test" {
  name                = "acctestavset-231020041508309601"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_windows_virtual_machine" "client" {
  ...
  name                = "acctest-${count.index}-m7usv"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  size                = "Standard_F2"
  admin_username      = local.admin_username
  ...
  network_interface_ids = [
    true ? azurerm_network_interface.client_single_subnet[count.index].id : azurerm_network_interface.client_multi_subnet[count.index].id,
  ]
...
resource "azurerm_mssql_virtual_machine_group" "test" {
  ...
  wsfc_domain_profile {
    ...
    cluster_subnet_type            = true ? "SingleSubnet" : "MultiSubnet"
  }
}


resource "azurerm_lb" "test" {
  name                = "acctestlb-231020041508309601"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
