...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221021031511335907"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-221021031511335907"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                            = "acctvmss-221021031511335907"
  location                        = azurerm_resource_group.test.location
  resource_group_name             = azurerm_resource_group.test.name
  upgrade_mode                    = "Manual"
  sku                             = "Standard_F2"
  ...
  computer_name_prefix            = "testvm-221021031511335907"
  ...
  network_interface {
    name    = "TestNetworkProfile-221021031511335907"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-221021031511335907"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-221021031511335907@example.com", "acctest2-221021031511335907@example.com"]
    }
  }
}
