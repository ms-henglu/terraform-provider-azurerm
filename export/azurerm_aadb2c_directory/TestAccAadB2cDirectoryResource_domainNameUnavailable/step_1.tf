...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230414020633506557"
...
resource "azurerm_aadb2c_directory" "test" {
  ...
  display_name            = "acctest230414020633506557"
  domain_name             = "acctest230414020633506557.onmicrosoft.com"
...
resource "azurerm_resource_group" "duplicate" {
  name     = "acctestRG-duplicate-230414020633506557"
  location = "West US 2"
}
...
resource "azurerm_aadb2c_directory" "duplicate" {
  ...
  display_name            = "acctest-duplicate-230414020633506557"
