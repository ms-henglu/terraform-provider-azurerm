...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-231020040657312163"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-231020040657312163"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "accTestOriginGroup-231020040657312163"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "accTestOrigin-231020040657312163"
...
resource "azurerm_cdn_frontdoor_endpoint" "test" {
  name                     = "accTestEndpoint-231020040657312163"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
}

resource "azurerm_cdn_frontdoor_rule_set" "test" {
  name                     = "accTestRuleSet231020040657312163"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
}


resource "azurerm_cdn_frontdoor_route" "test" {
  name                          = "accTestRoute-231020040657312163"
