...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-230203062905496952"
  location = "West Europe"
}

resource "azurerm_automation_account" "test" {
  name                = "acctestAA-230203062905496952"
...
resource "azurerm_automation_credential" "test" {
  name                    = "acctest-230203062905496952"
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230203062905496952"
...
resource "azurerm_network_interface" "test" {
  name                = "acctni-230203062905496952"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-230203062905496952"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  size                = "Standard_F2"
...
resource "azurerm_automation_runbook" "test" {
  name                    = "acc-runbook-230203062905496952"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
  automation_account_name = azurerm_automation_account.test.name
...
resource "azurerm_automation_watcher" "test" {
  ...
  name                  = "acctest-watcher-230203062905496952"
  location              = "West Europe"
