...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-CAEnv-240105063518217151"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestCAEnv-240105063518217151"
...
resource "azurerm_container_app_environment" "test" {
  name                       = "acctest-CAEnv240105063518217151"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
...
resource "azurerm_container_app_environment_dapr_component" "test" {
  name                         = "acctest-dapr-240105063518217151"
...
resource "azurerm_container_registry" "test" {
  name                = "testacccr240105063518217151"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Basic"
  admin_enabled       = true
...
resource "azurerm_container_app_environment_storage" "test" {
  name                         = "testacc-caes-240105063518217151"
  container_app_environment_id = azurerm_container_app_environment.test.id
...
resource "azurerm_container_app" "test" {
  name                         = "acctest-capp-240105063518217151"
  resource_group_name          = azurerm_resource_group.test.name
  container_app_environment_id = azurerm_container_app_environment.test.id
  ...
  template {
    container {
      name   = "acctest-cont-240105063518217151"
    ...
    init_container {
      name   = "init-cont-240105063518217151"
      image  = "jackofallops/azure-containerapps-python-acctest:v0.0.1"
      cpu    = 0.25
      memory = "0.5Gi"
      volume_mounts {
  ...
  dapr {
    app_id       = "acctest-cont-240105063518217151"
    app_port     = 5000
    app_protocol = "http"
  }
