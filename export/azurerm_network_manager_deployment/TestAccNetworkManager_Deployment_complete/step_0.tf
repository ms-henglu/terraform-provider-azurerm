...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-network-manager-231020041557283099"
...
resource "azurerm_network_manager" "test" {
  name                = "acctest-nm-231020041557283099"
...
resource "azurerm_network_manager_network_group" "test" {
  name               = "acctest-nmng-231020041557283099"
  network_manager_id = azurerm_network_manager.test.id
}

resource "azurerm_virtual_network" "test" {
  name                    = "acctest-vnet-231020041557283099"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_network_manager_connectivity_configuration" "test" {
  name                  = "acctest-nmcc-231020041557283099"
  network_manager_id    = azurerm_network_manager.test.id
...
resource "azurerm_network_manager_connectivity_configuration" "test2" {
  name                  = "acctest-nmcc2-231020041557283099"
  network_manager_id    = azurerm_network_manager.test.id
  connectivity_topology = "HubAndSpoke"
  applies_to_group {
    group_connectivity = "None"
