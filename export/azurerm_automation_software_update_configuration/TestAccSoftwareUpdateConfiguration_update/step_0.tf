...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-230519074229373362"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-230519074229373362"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-230519074229373362"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-230519074229373362"
  ...
  schedule {
    ...
    start_time          = "2023-05-19T17:42:29Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
