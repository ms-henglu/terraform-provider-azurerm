...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-240119024619129254"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-240119024619129254"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "accTestOriginGroup-240119024619129254"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "accTestOrigin-240119024619129254"
...
resource "azurerm_cdn_frontdoor_rule_set" "test" {
  name                     = "accTestRuleSet240119024619129254"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
}
...
resource "azurerm_cdn_frontdoor_rule" "test" {
  ...
  name                      = "accTestRule240119024619129254"
