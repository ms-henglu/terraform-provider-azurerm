...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-watcher-230818024518159791"
  location = "West Europe"
}

resource "azurerm_network_watcher" "test" {
  name                = "acctestnw-230818024518159791"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-230818024518159791"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                 = "acctestvmss-230818024518159791"
  resource_group_name  = azurerm_resource_group.test.name
  location             = azurerm_resource_group.test.location
  sku                  = "Standard_F2"
  instances            = 4
...
resource "azurerm_virtual_machine_scale_set_packet_capture" "test" {
  name                         = "acctestpc-230818024518159791"
