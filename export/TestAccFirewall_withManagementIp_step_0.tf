...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-211022002009990290"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet211022002009990290"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip211022002009990290"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "test_mgmt" {
  name                = "acctestmgmtpip211022002009990290"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  sku                 = "Standard"
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall211022002009990290"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
