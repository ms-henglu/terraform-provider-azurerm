...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ml-221019060804069705"
...
resource "azurerm_application_insights" "test" {
  name                = "acctestai-221019060804069705"
...
resource "azurerm_machine_learning_workspace" "test" {
  name                    = "acctest-MLW2210190608040697"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet221019060804069705"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet221019060804069705"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks221019060804069705"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = join("", ["acctestaks", azurerm_resource_group.test.location])
  node_resource_group = "acctestRGAKS-221019060804069705"
