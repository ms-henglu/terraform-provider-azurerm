...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-monitor-211008044703668988"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestWorkspace-211008044703668988"
...
resource "azurerm_monitor_action_group" "test" {
  name                = "acctestActionGroup-211008044703668988"
  resource_group_name = "${azurerm_resource_group.test.name}"
  short_name          = "acctestag"
}

resource "azurerm_monitor_scheduled_query_rules_log" "test" {
  name                = "acctestsqr-211008044703668988"
  resource_group_name = "${azurerm_resource_group.test.name}"
  location            = "${azurerm_resource_group.test.location}"
...
resource "azurerm_monitor_metric_alert" "test" {
  name                = "acctestmal-211008044703668988"
  resource_group_name = "${azurerm_resource_group.test.name}"
