...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-Watcher-210825045040108660"
  location = "West Europe"
}

resource "azurerm_network_watcher" "test" {
  name                = "acctest-Watcher-210825045040108660"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-Vnet-210825045040108660"
...
resource "azurerm_network_interface" "src" {
  name                = "acctest-SrcNIC-210825045040108660"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "src" {
  name                  = "acctest-SrcVM-210825045040108660"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_os_disk {
    name              = "osdisk-src210825045040108660"
    caching           = "ReadWrite"
  ...
  os_profile {
    computer_name  = "hostname210825045040108660"
...
resource "azurerm_network_interface" "dest" {
  name                = "acctest-DestNic-210825045040108660"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "dest" {
  name                  = "acctest-DestVM-210825045040108660"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_os_disk {
    name              = "osdisk-dest210825045040108660"
    caching           = "ReadWrite"
    create_option     = "FromImage"
    managed_disk_type = "Standard_LRS"
  }
...
resource "azurerm_network_connection_monitor" "test" {
  name               = "acctest-CM-210825045040108660"
