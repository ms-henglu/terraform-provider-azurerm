...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-postgresql-230106034903644416"
  location = "West Europe"
}


resource "azurerm_virtual_network" "test" {
  name                = "acctest-vn-230106034903644416"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-sn-230106034903644416"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_private_dns_zone" "test" {
  name                = "acc230106034903644416.postgres.database.azure.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_private_dns_zone_virtual_network_link" "test" {
  name                  = "acctestVnetZone230106034903644416.com"
...
resource "azurerm_postgresql_flexible_server" "test" {
  name                   = "acctest-fs-230106034903644416"
  resource_group_name    = azurerm_resource_group.test.name
  location               = azurerm_resource_group.test.location
