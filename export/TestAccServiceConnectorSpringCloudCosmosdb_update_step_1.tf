...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220905050509575948"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctestaccs0p9p"
...
resource "azurerm_spring_cloud_service" "test" {
  name                = "testspringcloudservice-s0p9p"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_spring_cloud_app" "test" {
  name                = "testspringcloud-s0p9p"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_cosmosdb_sql_container" "update" {
  name                = "test-containerupdates0p9p"
  resource_group_name = azurerm_cosmosdb_account.test.resource_group_name
  account_name        = azurerm_cosmosdb_account.test.name
...
resource "azurerm_spring_cloud_service" "update" {
  name                = "updatespringcloud-s0p9p"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
}

resource "azurerm_spring_cloud_app" "update" {
  name                = "testspringcloudupdate-s0p9p"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_spring_cloud_connection" "test" {
  name               = "acctestserviceconnector220905050509575948"
