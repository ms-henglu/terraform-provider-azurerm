...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-maint-230512004316967939"
  location = "West Europe"
}

resource "azurerm_maintenance_configuration" "test" {
  name                = "acctest-MC230512004316967939"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-230512004316967939"
...
resource "azurerm_network_interface" "test" {
  name                = "acctni-230512004316967939"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-230512004316967939"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  size                = "Standard_F2"
...
resource "azurerm_maintenance_configuration" "test2" {
  name                = "acctest-MC2230512004316967939"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  scope               = "SQLDB"
}
