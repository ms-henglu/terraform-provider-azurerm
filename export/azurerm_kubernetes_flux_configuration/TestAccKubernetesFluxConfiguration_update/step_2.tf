...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240112224211802741"
  location = "West Europe"
}

resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestAKC-240112224211802741"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = "acctestaks240112224211802741"
...
resource "azurerm_kubernetes_cluster_extension" "test" {
  name           = "acctest-kce-240112224211802741"
...
resource "azurerm_kubernetes_flux_configuration" "test" {
  name       = "acctest-fc-240112224211802741"
  cluster_id = azurerm_kubernetes_cluster.test.id
  namespace  = "flux"
  scope      = "cluster"
