...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-maint-240311032455067897"
  location = "West Europe"
}

resource "azurerm_maintenance_configuration" "test" {
  name                = "acctest-MC240311032455067897"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-240311032455067897"
...
resource "azurerm_network_interface" "test" {
  name                = "acctni-240311032455067897"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                = "acctestVM-240311032455067897"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  size                = "Standard_F2"
...
resource "azurerm_maintenance_configuration" "test2" {
  name                = "acctest-MC2240311032455067897"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  scope               = "SQLDB"
}
