...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-acr-221221204115031219"
  location = "West Europe"
}

resource "azurerm_container_registry" "test" {
  name                = "testacccr221221204115031219"
...
resource "azurerm_container_registry_token" "test" {
  name                    = "testtoken-221221204115031219"
  resource_group_name     = azurerm_resource_group.test.name
  container_registry_name = azurerm_container_registry.test.name
...
resource "azurerm_container_registry_token_password" "test" {
  ...
  password1 {
    expiry = "2022-12-21T21:41:15Z"
  }
}

resource "azurerm_resource_group" "consumer" {
  name     = "acctestRG-acr-consumer-221221204115031219"
  location = "West Europe"
  tags = {
