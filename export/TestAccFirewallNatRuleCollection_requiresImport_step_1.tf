...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-211001053751699344"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet211001053751699344"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip211001053751699344"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall211001053751699344"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall_nat_rule_collection" "test" {
  name                = "acctestnrc-211001053751699344"
