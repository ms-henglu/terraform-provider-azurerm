...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230613072624513788"
  location = "West Europe"
}

resource "azurerm_servicebus_namespace" "test" {
  name                = "acctestservicebusnamespace-230613072624513788"
...
resource "azurerm_servicebus_topic" "test" {
  name         = "acctestservicebustopic-230613072624513788"
  namespace_id = azurerm_servicebus_namespace.test.id
}

resource "azurerm_servicebus_subscription" "test" {
  name               = "acctestservicebussubscription-230613072624513788"
...
resource "azurerm_servicebus_subscription_rule" "test" {
  name            = "acctestservicebusrule-230613072624513788"
