...
locals {
  vm_name = "acctestvm2nxss"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-231218071435583532"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-231218071435583532"
...
resource "azurerm_public_ip" "first" {
  name                = "acctpip1-231218071435583532"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "first" {
  name                = "acctestnic1-231218071435583532"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "second" {
  name                = "acctpip2-231218071435583532"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Dynamic"
}
...
resource "azurerm_network_interface" "second" {
  name                = "acctestnic2-231218071435583532"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
