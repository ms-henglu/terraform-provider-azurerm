...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-ml-231013043741195207"
...
resource "azurerm_application_insights" "test" {
  name                = "acctestai-231013043741195207"
...
resource "azurerm_machine_learning_workspace" "test" {
  name                    = "acctest-MLW2310130437411952"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet231013043741195207"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet231013043741195207"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks231013043741195207"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = join("", ["acctestaks", azurerm_resource_group.test.location])
  node_resource_group = "acctestRGAKS-231013043741195207"
