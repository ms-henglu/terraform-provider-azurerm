...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-recovery-240105064457633595-1"
  location = "West Europe"
}

resource "azurerm_resource_group" "test2" {
  name     = "acctestRG-recovery-240105064457633595-2"
  location = "West US 2"
}

resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-vault-240105064457633595"
...
resource "azurerm_site_recovery_fabric" "test1" {
  ...
  name                = "acctest-fabric1-240105064457633595"
...
resource "azurerm_site_recovery_fabric" "test2" {
  resource_group_name = azurerm_resource_group.test2.name
  recovery_vault_name = azurerm_recovery_services_vault.test.name
  name                = "acctest-fabric2-240105064457633595"
  location            = azurerm_resource_group.test2.location
...
resource "azurerm_site_recovery_protection_container" "test1" {
  ...
  name                 = "acctest-protection-cont1-240105064457633595"
}
...
resource "azurerm_site_recovery_protection_container" "test2" {
  ...
  name                 = "acctest-protection-cont2-240105064457633595"
}

resource "azurerm_site_recovery_replication_policy" "test" {
  resource_group_name                                  = azurerm_resource_group.test2.name
  recovery_vault_name                                  = azurerm_recovery_services_vault.test.name
  name                                                 = "acctest-policy-240105064457633595"
...
resource "azurerm_site_recovery_protection_container_mapping" "test" {
  ...
  name                                      = "mapping-240105064457633595"
}

resource "azurerm_virtual_network" "test1" {
  name                = "net-240105064457633595"
...
resource "azurerm_subnet" "test2" {
  name                 = "snet-240105064457633595_2"
  resource_group_name  = azurerm_resource_group.test2.name
...
resource "azurerm_virtual_machine" "test" {
  ...
  os_profile {
    ...
    computer_name  = "vm-240105064457633595"
  }
...
resource "azurerm_public_ip" "test-source" {
  name                = "pubip240105064457633595-source"
...
resource "azurerm_public_ip" "test-recovery" {
  name                = "pubip240105064457633595-recovery"
  allocation_method   = "Static"
  location            = azurerm_resource_group.test2.location
  resource_group_name = azurerm_resource_group.test2.name
  sku                 = "Basic"
...
resource "azurerm_capacity_reservation_group" "test" {
  name                = "acctest-crg-240105064457633595"
  location            = azurerm_resource_group.test2.location
  resource_group_name = azurerm_resource_group.test2.name
}

resource "azurerm_capacity_reservation" "test" {
  name                          = "acctest-ccr-240105064457633595"
