...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220905050606597368"
  location = "West Europe"
}

resource "azurerm_servicebus_namespace" "test" {
  name                = "acctest-220905050606597368"
...
resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-220905050606597368"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_servicebus_namespace" "updated" {
  name                = "acctest2-220905050606597368"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_stream_analytics_output_servicebus_topic" "test" {
  name                      = "acctestinput-220905050606597368"
