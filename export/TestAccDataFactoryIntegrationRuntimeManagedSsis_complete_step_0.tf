...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-210924010904956636"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvnet210924010904956636"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet210924010904956636"
  resource_group_name  = "${azurerm_resource_group.test.name}"
...
resource "azurerm_public_ip" "test1" {
  name                = "acctpip1210924010904956636"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctpip1210924010904956636"
}

resource "azurerm_public_ip" "test2" {
  name                = "acctpip2210924010904956636"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctpip2210924010904956636"
}
...
resource "azurerm_sql_server" "test" {
  name                         = "acctestsql210924010904956636"
  resource_group_name          = "${azurerm_resource_group.test.name}"
  location                     = "${azurerm_resource_group.test.location}"
  version                      = "12.0"
...
resource "azurerm_data_factory" "test" {
  name                = "acctestdfirm210924010904956636"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
}

resource "azurerm_data_factory_linked_custom_service" "test" {
  name                 = "acctestls210924010904956636"
...
resource "azurerm_data_factory_linked_custom_service" "file_share_linked_service" {
  name                 = "acctestls1210924010904956636"
  data_factory_id      = azurerm_data_factory.test.id
