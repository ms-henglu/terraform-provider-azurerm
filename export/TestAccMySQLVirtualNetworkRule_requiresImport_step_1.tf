...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220506010032329327"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvnet220506010032329327"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet220506010032329327"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_mysql_server" "test" {
  name                         = "acctestmysqlsvr-220506010032329327"
  location                     = azurerm_resource_group.test.location
  resource_group_name          = azurerm_resource_group.test.name
...
resource "azurerm_mysql_virtual_network_rule" "test" {
  name                = "acctestmysqlvnetrule220506010032329327"
  resource_group_name = azurerm_resource_group.test.name
