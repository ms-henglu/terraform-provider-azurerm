...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-221117230800975852"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-221117230800975852"
...
resource "azurerm_public_ip" "test" {
  name                = "acctpip-221117230800975852"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-221117230800975852"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine_extension" "test" {
  name                 = "acctestExt-221117230800975852"
...
resource "azurerm_resource_group" "host" {
  name     = "acctesthostRG-df-221117230800975852"
  location = "West Europe"
}

resource "azurerm_data_factory" "host" {
  name                = "acctestdfirshh221117230800975852"
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "host" {
  name            = "acctestirshh221117230800975852"
  data_factory_id = azurerm_data_factory.host.id
}

resource "azurerm_resource_group" "target" {
  name     = "acctesttargetRG-221117230800975852"
  location = "West Europe"
}
...
resource "azurerm_data_factory" "target" {
  name                = "acctestdfirsht221117230800975852"
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "target" {
  name            = "acctestirsht221117230800975852"
