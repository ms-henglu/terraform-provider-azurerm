...
resource "azurerm_resource_group" "test" {
  name     = "acctest211105040303494093"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvn-211105040303494093"
...
resource "azurerm_policy_definition" "test" {
  name         = "acctestpol-211105040303494093"
  policy_type  = "Custom"
  mode         = "All"
  display_name = "acctestpol-211105040303494093"
...
resource "azurerm_resource_policy_assignment" "test" {
  name                 = "acctestpa-211105040303494093"
