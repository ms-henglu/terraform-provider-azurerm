...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-211022001855077692"
  location = "West Europe"
}

resource "azurerm_data_factory" "test" {
  name                = "acctestdf211022001855077692"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_data_factory_pipeline" "test" {
  name                = "acctestdfp211022001855077692"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_trigger_tumbling_window" "dependency" {
  name            = "acctestdft2211022001855077692"
...
resource "azurerm_data_factory_trigger_tumbling_window" "test" {
  name            = "acctestdft211022001855077692"
  data_factory_id = azurerm_data_factory.test.id
  start_time      = "2022-09-21T00:00:00Z"
