...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mssql-220326010914836363"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-VN-220326010914836363"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-SN-220326010914836363"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_public_ip" "vm" {
  name                = "acctest-PIP-220326010914836363"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_network_security_group" "test" {
  name                = "acctest-NSG-220326010914836363"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
}
...
resource "azurerm_network_interface" "test" {
  name                = "acctest-NIC-220326010914836363"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine" "test" {
  name                  = "acctest-VM-220326010914836363"
  location              = azurerm_resource_group.test.location
  resource_group_name   = azurerm_resource_group.test.name
  ...
  storage_os_disk {
    name              = "acctvm-220326010914836363OSDisk"
    caching           = "ReadOnly"
...
resource "azurerm_mssql_virtual_machine" "test" {
  ...
  auto_backup {
    ...
    encryption_password             = "P@55w0rD!!aihir"
