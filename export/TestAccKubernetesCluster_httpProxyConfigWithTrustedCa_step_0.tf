...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220630210627777565"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet220630210627777565"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet220630210627777565"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_network_interface" "test" {
  name                = "test-nic220630210627777565"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine" "test" {
  name                            = "vm-test-proxy220630210627777565"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_B1s"
...
resource "azurerm_kubernetes_cluster" "test" {
  name                = "acctestaks220630210627777565"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  dns_prefix          = "acctestaks220630210627777565"
  kubernetes_version  = "1.22.4"

  linux_profile {
    admin_username = "acctestuser220630210627777565"
    ssh_key {
