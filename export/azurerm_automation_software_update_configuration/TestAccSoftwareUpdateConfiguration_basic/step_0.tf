...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-221222034249694206"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-221222034249694206"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-221222034249694206"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-221222034249694206"
  ...
  schedule {
    ...
    start_time          = "2022-12-22T13:42:49Z"
    expiry_time         = "2022-12-24T05:42:49Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
