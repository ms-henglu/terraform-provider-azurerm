...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-df-210906022150081011"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestvnet210906022150081011"
...
resource "azurerm_subnet" "test" {
  name                 = "acctestsubnet210906022150081011"
  resource_group_name  = "${azurerm_resource_group.test.name}"
...
resource "azurerm_public_ip" "test1" {
  name                = "acctpip1210906022150081011"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctpip1210906022150081011"
}

resource "azurerm_public_ip" "test2" {
  name                = "acctpip2210906022150081011"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  allocation_method   = "Static"
  domain_name_label   = "acctpip2210906022150081011"
}
...
resource "azurerm_sql_server" "test" {
  name                         = "acctestsql210906022150081011"
  resource_group_name          = "${azurerm_resource_group.test.name}"
  location                     = "${azurerm_resource_group.test.location}"
  version                      = "12.0"
...
resource "azurerm_data_factory" "test" {
  name                = "acctestdfirm210906022150081011"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
}

resource "azurerm_data_factory_linked_custom_service" "test" {
  name                 = "acctestls210906022150081011"
...
resource "azurerm_data_factory_linked_custom_service" "file_share_linked_service" {
  name                 = "acctestls1210906022150081011"
  data_factory_id      = azurerm_data_factory.test.id
...
resource "azurerm_data_factory_linked_service_key_vault" "test" {
  name                = "acctestlinkkv210906022150081011"
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_data_factory_integration_runtime_self_hosted" "test" {
  name                = "acctestSIRsh210906022150081011"
  data_factory_name   = azurerm_data_factory.test.name
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_data_factory_integration_runtime_azure_ssis" "test" {
  name                = "acctestiras210906022150081011"
  description         = "acctest"
  data_factory_name   = azurerm_data_factory.test.name
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
