...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-210924004029444788"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-210924004029444788"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-210924004029444788"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard_F2"
  instances           = 1
...
resource "azurerm_virtual_machine_scale_set_extension" "first" {
  name                         = "acctestExt1-210924004029444788"
...
resource "azurerm_virtual_machine_scale_set_extension" "second" {
  name                         = "acctestExt2-210924004029444788"
  virtual_machine_scale_set_id = azurerm_linux_virtual_machine_scale_set.test.id
  publisher                    = "Microsoft.Azure.Extensions"
  type                         = "CustomScript"
