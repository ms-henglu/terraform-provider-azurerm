...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-backup-221124182205868095"
...
resource "azurerm_storage_share" "testshare1" {
  name                 = "acctest-ss-221124182205868095-1"
...
resource "azurerm_storage_share" "testshare2" {
  name                 = "acctest-ss-221124182205868095-2"
  storage_account_name = "${azurerm_storage_account.test1.name}"
  quota                = 1
  metadata             = {}
...
resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-VAULT-221124182205868095"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
  sku                 = "Standard"
...
resource "azurerm_backup_policy_file_share" "test" {
  name                = "acctest-PFS-221124182205868095"
  resource_group_name = "${azurerm_resource_group.test.name}"
