...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-OVMSS-220407230802467528"
  location = "West Europe"
}


resource "azurerm_public_ip" "test" {
  name                = "acctpip-220407230802467528"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctvn-220407230802467528"
...
resource "azurerm_subnet" "test" {
  name                 = "acctsub-220407230802467528"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_nat_gateway" "test" {
  name                    = "acctng-220407230802467528"
  location                = azurerm_resource_group.test.location
  resource_group_name     = azurerm_resource_group.test.name
  sku_name                = "Standard"
...
resource "azurerm_orchestrated_virtual_machine_scale_set" "test" {
  name                = "acctestOVMSS-220407230802467528"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    linux_configuration {
      computer_name_prefix = "testvm-220407230802467528"
...
resource "azurerm_subnet" "gwtest" {
  name                 = "gw-subnet-220407230802467528"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_public_ip" "gwtest" {
  name                = "acctest-pubip-220407230802467528"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_application_gateway" "test" {
  name                = "acctestgw-220407230802467528"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
