...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-210928075711785321"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctvn-210928075711785321"
...
resource "azurerm_virtual_machine_scale_set" "test" {
  name                = "acctvmss-210928075711785321"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  ...
  os_profile {
    computer_name_prefix = "testvm-210928075711785321"
  ...
  network_profile {
    name    = "TestNetworkProfile-210928075711785321"
    primary = true
...
resource "azurerm_monitor_autoscale_setting" "test" {
  name                = "acctestautoscale-210928075711785321"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  ...
  notification {
    email {
      ...
      custom_emails                         = ["acctest1-210928075711785321@example.com"]
    }
  }
}
