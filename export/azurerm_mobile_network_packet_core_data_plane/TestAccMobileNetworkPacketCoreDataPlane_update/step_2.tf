...
resource "azurerm_resource_group" "test" {
  name     = "acctest-mn-230721015600527025"
  location = "eastus"
}
...
resource "azurerm_mobile_network_site" "test" {
  name              = "acctest-mns-230721015600527025"
...
resource "azurerm_mobile_network_packet_core_control_plane" "test" {
  name                = "acctest-mnpccp-230721015600527025"
  resource_group_name = azurerm_resource_group.test.name
  location            = "eastus"
  sku                 = "G0"
...
resource "azurerm_mobile_network_packet_core_data_plane" "test" {
  name                                        = "acctest-mnpcdp-230721015600527025"
