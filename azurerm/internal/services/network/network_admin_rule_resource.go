...
func resourceNetworkAdminRuleCreateUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	id := parse.NewNetworkAdminRuleID(subscriptionId, resourceGroup, networkManagerName, configurationName, ruleCollectionName, name)
	...
	adminRule := network.AdminRule{
		AdminPropertiesFormat: &network.AdminPropertiesFormat{
			...
			Destinations:          expandAddressPrefixItemArray(d.Get("destinations").([]interface{})),
			...
			Sources:               expandAddressPrefixItemArray(d.Get("sources").([]interface{})),
			SourcePortRanges:      utils.ExpandStringSlice(d.Get("source_port_ranges").([]interface{})),
		},
	}
	if _, err := client.CreateOrUpdate(ctx, adminRule, id.ResourceGroup, id.NetworkManagerName, id.SecurityConfigurationName, id.RuleCollectionName, id.RuleName); err != nil {
...
func resourceNetworkAdminRuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	if adminRule, ok := resp.Value.AsAdminRule(); ok {
		if props := adminRule.AdminPropertiesFormat; props != nil {
			...
			if err := d.Set("sources", flattenAddressPrefixItemArray(props.Sources)); err != nil {
				return fmt.Errorf("setting `source`: %+v", err)
			}
			if err := d.Set("destinations", flattenAddressPrefixItemArray(props.Destinations)); err != nil {
...
func expandAddressPrefixItemArray(input []interface{}) *[]network.AddressPrefixItem {
...
func flattenAddressPrefixItemArray(input *[]network.AddressPrefixItem) []interface{} {
