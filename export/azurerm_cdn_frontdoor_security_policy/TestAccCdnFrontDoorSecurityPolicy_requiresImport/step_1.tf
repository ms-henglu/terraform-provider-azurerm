...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-221104005155223942"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF221104005155223942"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone221104005155223942.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-221104005155223942"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-221104005155223942"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol221104005155223942"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
