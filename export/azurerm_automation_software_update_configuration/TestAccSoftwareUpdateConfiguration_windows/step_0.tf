...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-auto-230602030153719447"
  location = "West US"
}

resource "azurerm_automation_account" "test" {
  name                = "acctest-230602030153719447"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestLAW-230602030153719447"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
...
resource "azurerm_automation_software_update_configuration" "test" {
  ...
  name                  = "acctest-suc-230602030153719447"
  ...
  schedule {
    ...
    expiry_time         = "2023-06-04T05:01:53Z"
    is_enabled          = true
    interval            = 1
    frequency           = "Hour"
    time_zone           = "Etc/UTC"
