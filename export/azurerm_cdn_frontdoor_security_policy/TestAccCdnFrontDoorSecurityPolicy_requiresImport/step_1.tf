...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-231016033515752397"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF231016033515752397"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone231016033515752397.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-231016033515752397"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-231016033515752397"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol231016033515752397"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
