...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-CAE-240315122626537900"
...
resource "azurerm_dns_txt_record" "test" {
  name                = "asuid.containerapp240315122626537900"
...
resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestCAEnv-240315122626537900"
...
resource "azurerm_container_app_environment" "test" {
  name                       = "acctest-CAEnv240315122626537900"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
...
resource "azurerm_container_app_environment_certificate" "test" {
  name                         = "acctest-cacert240315122626537900"
...
resource "azurerm_container_app" "test" {
  name                         = "acctest-capp-240315122626537900"
  resource_group_name          = azurerm_resource_group.test.name
  container_app_environment_id = azurerm_container_app_environment.test.id
  ...
  template {
    container {
      name   = "acctest-cont-240315122626537900"
...
resource "azurerm_dns_txt_record" "test2" {
  name                = "asuid.containerapp240315122626537900-2"
  resource_group_name = data.azurerm_dns_zone.test.resource_group_name
  zone_name           = data.azurerm_dns_zone.test.name
  ttl                 = 300
...
resource "azurerm_container_app_environment_certificate" "test2" {
  name                         = "acctest-cacert240315122626537900-2"
  container_app_environment_id = azurerm_container_app_environment.test.id
  certificate_blob_base64      = filebase64("testdata/testacc.pfx")
  certificate_password         = "TestAcc"
}
