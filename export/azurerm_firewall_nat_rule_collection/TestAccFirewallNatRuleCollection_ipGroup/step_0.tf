...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-230505050440999911"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet230505050440999911"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip230505050440999911"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall230505050440999911"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "AZFW_VNet"
  sku_tier            = "Standard"
...
resource "azurerm_firewall_nat_rule_collection" "test" {
  name                = "acctestnrc-230505050440999911"
