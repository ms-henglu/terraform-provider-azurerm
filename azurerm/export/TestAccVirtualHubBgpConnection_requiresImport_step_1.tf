...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-VHub-220627134828793456"
  location = "West Europe"
}

resource "azurerm_virtual_hub" "test" {
  name                = "acctest-VHub-220627134828793456"
...
resource "azurerm_public_ip" "test" {
  name                = "acctest-PIP-220627134828793456"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctest-VNet-220627134828793456"
...
resource "azurerm_virtual_hub_ip" "test" {
  name                         = "acctest-VHub-IP-220627134828793456"
...
resource "azurerm_virtual_hub_bgp_connection" "test" {
  name           = "acctest-VHub-BgpConnection-220627134828793456"
  virtual_hub_id = azurerm_virtual_hub.test.id
  peer_asn       = 65514
