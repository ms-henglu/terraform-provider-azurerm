...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-CAEnv-230707003600167768"
  location = "West Europe"
}

resource "azurerm_log_analytics_workspace" "test" {
  name                = "acctestCAEnv-230707003600167768"
...
resource "azurerm_container_app_environment" "test" {
  name                       = "acctest-CAEnv230707003600167768"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
...
resource "azurerm_container_app_environment_dapr_component" "test" {
  name                         = "acctest-dapr-230707003600167768"
...
resource "azurerm_container_registry" "test" {
  name                = "testacccr230707003600167768"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Basic"
  admin_enabled       = true
...
resource "azurerm_container_app_environment_storage" "test" {
  name                         = "testacc-caes-230707003600167768"
  container_app_environment_id = azurerm_container_app_environment.test.id
...
resource "azurerm_container_app" "test" {
  name                         = "acctest-capp-230707003600167768"
  resource_group_name          = azurerm_resource_group.test.name
  container_app_environment_id = azurerm_container_app_environment.test.id
  ...
  template {
    container {
      name   = "acctest-cont-230707003600167768"
  ...
  dapr {
    app_id       = "acctest-cont-230707003600167768"
    app_port     = 5000
    app_protocol = "http"
  }
