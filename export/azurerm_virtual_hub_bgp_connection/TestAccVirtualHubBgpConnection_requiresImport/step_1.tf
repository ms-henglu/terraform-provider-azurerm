...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-VHub-231020041557331062"
  location = "West Europe"
}

resource "azurerm_virtual_hub" "test" {
  name                = "acctest-VHub-231020041557331062"
...
resource "azurerm_public_ip" "test" {
  name                = "acctest-PIP-231020041557331062"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctest-VNet-231020041557331062"
...
resource "azurerm_virtual_hub_ip" "test" {
  name                         = "acctest-VHub-IP-231020041557331062"
...
resource "azurerm_virtual_hub_bgp_connection" "test" {
  name           = "acctest-VHub-BgpConnection-231020041557331062"
  virtual_hub_id = azurerm_virtual_hub.test.id
  peer_asn       = 65514
