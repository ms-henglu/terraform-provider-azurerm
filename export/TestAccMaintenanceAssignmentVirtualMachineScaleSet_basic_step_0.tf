...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220218070949431398"
...
resource "azurerm_lb_rule" "test" {
  ...
  backend_address_pool_ids       = web / function_app_resource.g[azurerm_lb_backend_address_pool.test.id]
...
resource "azurerm_maintenance_configuration" "test" {
  name                = "acctest-MC220218070949431398"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  scope               = "OSImage"
  visibility          = "Custom"
...
resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-220218070949431398"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                            = "acctestvmss-220218070949431398"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  sku                             = "Standard_F2"
  instances                       = 1
