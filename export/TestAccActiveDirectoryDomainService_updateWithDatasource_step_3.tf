...
resource "azurerm_resource_group" "test" {
  name     = "acclongtestRG-aadds-210924010955416478"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestVnet-aadds-210924010955416478"
...
resource "azurerm_subnet" "aadds" {
  name                 = "acctestSubnet-aadds-210924010955416478"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_subnet" "workload" {
  name                 = "acctestSubnet-workload-210924010955416478"
  resource_group_name  = azurerm_resource_group.test.name
  virtual_network_name = azurerm_virtual_network.test.name
...
resource "azurerm_network_security_group" "aadds" {
  name                = "acctestNSG-aadds-210924010955416478"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azuread_user" "test" {
  user_principal_name = "acctestAADDSAdminUser-210924010955416478@${data.azuread_domains.test.domains.0.domain_name}"
  display_name        = "acctestAADDSAdminUser-210924010955416478"
  password            = "p@$$Wd72des8"
}
...
resource "azurerm_resource_group" "test_secondary" {
  name     = "acclongtestRG-aadds-secondary-210924010955416478"
  location = "West US 2"
}

resource "azurerm_virtual_network" "test_secondary" {
  name                = "acctestVnet-aadds-secondary-210924010955416478"
...
resource "azurerm_subnet" "aadds_secondary" {
  name                 = "acctestSubnet-aadds-secondary-210924010955416478"
  resource_group_name  = azurerm_resource_group.test_secondary.name
...
resource "azurerm_subnet" "workload_secondary" {
  name                 = "acctestSubnet-workload-secondary-210924010955416478"
  resource_group_name  = azurerm_resource_group.test_secondary.name
  virtual_network_name = azurerm_virtual_network.test_secondary.name
...
resource "azurerm_network_security_group" "aadds_secondary" {
  name                = "acctestNSG-aadds-secondary-210924010955416478"
  location            = azurerm_resource_group.test_secondary.location
  resource_group_name = azurerm_resource_group.test_secondary.name
...
resource "azurerm_virtual_network_peering" "test_primary_secondary" {
  name                      = "acctestVnet-aadds-primary-secondary-210924010955416478"
...
resource "azurerm_virtual_network_peering" "test_secondary_primary" {
  name                      = "acctestVnet-aadds-secondary-primary-210924010955416478"
