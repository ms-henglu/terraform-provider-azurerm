...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221221204829065450"
  location = "West Europe"
}

resource "azurerm_servicebus_namespace" "test" {
  name                = "acctestservicebusnamespace-221221204829065450"
...
resource "azurerm_servicebus_topic" "test" {
  name         = "acctestservicebustopic-221221204829065450"
  namespace_id = azurerm_servicebus_namespace.test.id
}

resource "azurerm_servicebus_subscription" "test" {
  name               = "acctestservicebussubscription-221221204829065450"
...
resource "azurerm_servicebus_subscription_rule" "test" {
  name            = "acctestservicebusrule-221221204829065450"
