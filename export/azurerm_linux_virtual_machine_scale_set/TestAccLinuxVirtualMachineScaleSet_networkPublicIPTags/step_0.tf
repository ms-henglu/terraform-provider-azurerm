...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240112034042970714"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-240112034042970714"
...
resource "azurerm_public_ip" "test" {
  name                = "test-ip-240112034042970714"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  domain_name_label   = "acctest-ovooeqczc"

  sku = "Standard"
}

resource "azurerm_lb" "test" {
  name                = "acctestlb-240112034042970714"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-240112034042970714"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard_F2"
  instances           = 1
