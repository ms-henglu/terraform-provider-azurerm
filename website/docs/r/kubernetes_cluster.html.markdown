...
!> **Note:** Specifying `public_network_access_enabled` to `false` may not work as you expected. When specifying `network_profile.outbound_type` to `userDefinedRouting`, the `public_network_access_enabled` should not be set to `false`. Consider using `private_cluster_enabled` to restrict the access to the cluster.
