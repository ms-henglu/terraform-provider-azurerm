...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221104010002997183"
  location = "West Europe"
}

resource "azurerm_eventhub_namespace" "test" {
  name                = "acctestehn-221104010002997183"
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteh-221104010002997183"
...
resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-221104010002997183"
  resource_group_name                      = azurerm_resource_group.test.name
  location                                 = azurerm_resource_group.test.location
...
resource "azurerm_eventhub_namespace" "updated" {
  name                = "acctestehn2-221104010002997183"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
  capacity            = 1
...
resource "azurerm_eventhub" "updated" {
  name                = "acctesteh2-221104010002997183"
...
resource "azurerm_stream_analytics_output_eventhub" "test" {
  name                      = "acctestinput-221104010002997183"
