...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-vhub-220627130109785819"
  location = "West Europe"
}

resource "azurerm_virtual_hub" "test" {
  name                = "acctest-vhub-220627130109785819"
...
resource "azurerm_public_ip" "test" {
  name                = "acctest-pip-220627130109785819"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_virtual_network" "test" {
  name                = "acctest-vnet-220627130109785819"
...
resource "azurerm_virtual_hub_ip" "test" {
  name           = "acctest-vhubipconfig-220627130109785819"
