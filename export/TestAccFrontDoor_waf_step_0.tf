...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-frontdoor-220128082445312330"
...
resource "azurerm_frontdoor_firewall_policy" "test" {
  name                = "acctestwafp220128082445312330"
  resource_group_name = azurerm_resource_group.test.name
  mode                = "Prevention"
}

resource "azurerm_frontdoor" "test" {
  name                                         = "acctest-FD-220128082445312330"
  resource_group_name                          = azurerm_resource_group.test.name
  ...
  frontend_endpoint {
    ...
    host_name                               = "acctest-FD-220128082445312330.azurefd.net"
