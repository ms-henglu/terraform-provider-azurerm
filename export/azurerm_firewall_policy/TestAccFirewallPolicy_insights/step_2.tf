...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-networkfw-240315123101619073"
  location = "West Europe"
}


resource "azurerm_log_analytics_workspace" "default" {
  name                = "acctestLAW-240315123101619073"
...
resource "azurerm_log_analytics_workspace" "regional" {
  name                = "acctestLAW-region-240315123101619073"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "PerGB2018"
  retention_in_days   = 30
...
resource "azurerm_firewall_policy" "test" {
  name                = "acctest-networkfw-Policy-240315123101619073"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  insights {
    enabled                            = true
