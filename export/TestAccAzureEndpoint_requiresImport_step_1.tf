...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-traffic-220415031222685662"
  location = "West Europe"
}

resource "azurerm_traffic_manager_profile" "test" {
  name                   = "acctest-TMP-220415031222685662"
  resource_group_name    = azurerm_resource_group.test.name
  traffic_routing_method = "Weighted"

  dns_config {
    relative_name = "acctest-tmp-220415031222685662"
    ttl           = 30
  }
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpublicip-220415031222685662"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  allocation_method   = "Static"
  domain_name_label   = "acctestpublicip-220415031222685662"
}

resource "azurerm_traffic_manager_azure_endpoint" "test" {
  name               = "acctestend-azure220415031222685662"
