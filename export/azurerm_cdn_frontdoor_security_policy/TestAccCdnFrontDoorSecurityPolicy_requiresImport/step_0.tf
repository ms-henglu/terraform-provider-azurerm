...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-231020040657324316"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_firewall_policy" "test" {
  name                              = "accTestWAF231020040657324316"
...
resource "azurerm_dns_zone" "test" {
  name                = "acctestzone231020040657324316.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-231020040657324316"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Premium_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_custom_domain" "test" {
  name                     = "accTestCustomDomain-231020040657324316"
...
resource "azurerm_cdn_frontdoor_security_policy" "test" {
  name                     = "accTestSecPol231020040657324316"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
