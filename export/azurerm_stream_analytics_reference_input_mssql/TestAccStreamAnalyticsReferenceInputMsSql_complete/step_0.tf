...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-221222035425634869"
  location = "West Europe"
}

resource "azurerm_stream_analytics_job" "test" {
  name                                     = "acctestjob-221222035425634869"
...
resource "azurerm_mssql_server" "test" {
  name                         = "acctest-sqlserver-221222035425634869"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  version                      = "12.0"
...
resource "azurerm_mssql_database" "test" {
  name         = "acctest-db-221222035425634869"
...
resource "azurerm_stream_analytics_reference_input_mssql" "test" {
  name                      = "acctestinput-221222035425634869"
