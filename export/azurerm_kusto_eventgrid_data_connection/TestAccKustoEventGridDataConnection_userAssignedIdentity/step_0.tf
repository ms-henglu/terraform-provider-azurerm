...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-230728032500007724"
...
resource "azurerm_kusto_database" "test" {
  name                = "acctestkd-230728032500007724"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
...
resource "azurerm_eventhub_namespace" "test" {
  name                = "acctesteventhubnamespace-230728032500007724"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
  sku                 = "Standard"
}
...
resource "azurerm_eventhub" "test" {
  name                = "acctesteventhub-230728032500007724"
...
resource "azurerm_eventhub_consumer_group" "test" {
  name                = "acctesteventhubcg-230728032500007724"
  namespace_name      = azurerm_eventhub_namespace.test.name
...
resource "azurerm_eventgrid_event_subscription" "test" {
  name                  = "acctest-eg-230728032500007724"
...
resource "azurerm_kusto_eventgrid_data_connection" "test" {
  name                         = "acctestkrgdc-230728032500007724"
  resource_group_name          = azurerm_resource_group.test.name
  location                     = azurerm_resource_group.test.location
  cluster_name                 = azurerm_kusto_cluster.test.name
