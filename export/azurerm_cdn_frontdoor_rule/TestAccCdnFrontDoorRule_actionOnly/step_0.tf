...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-cdn-afdx-230613071446993225"
  location = "West Europe"
}

resource "azurerm_cdn_frontdoor_profile" "test" {
  name                = "accTestProfile-230613071446993225"
  resource_group_name = azurerm_resource_group.test.name
  sku_name            = "Standard_AzureFrontDoor"
}

resource "azurerm_cdn_frontdoor_origin_group" "test" {
  name                     = "accTestOriginGroup-230613071446993225"
...
resource "azurerm_cdn_frontdoor_origin" "test" {
  name                          = "accTestOrigin-230613071446993225"
...
resource "azurerm_cdn_frontdoor_rule_set" "test" {
  name                     = "accTestRuleSet230613071446993225"
  cdn_frontdoor_profile_id = azurerm_cdn_frontdoor_profile.test.id
}
...
resource "azurerm_cdn_frontdoor_rule" "test" {
  ...
  name                      = "accTestRule230613071446993225"
