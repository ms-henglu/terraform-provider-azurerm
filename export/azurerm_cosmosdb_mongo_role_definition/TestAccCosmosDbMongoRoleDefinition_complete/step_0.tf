...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-mongoroledef-230825024317965861"
  location = "West Europe"
}

resource "azurerm_cosmosdb_account" "test" {
  name                = "acctest-ca-230825024317965861"
...
resource "azurerm_cosmosdb_mongo_database" "test" {
  name                = "acctest-mongodb-230825024317965861"
...
resource "azurerm_cosmosdb_mongo_collection" "test" {
  name                = "acctest-mongocoll-230825024317965861"
...
resource "azurerm_cosmosdb_mongo_role_definition" "test" {
  cosmos_mongo_database_id = azurerm_cosmosdb_mongo_database.test.id
  role_name                = "acctestmongoroledef230825024317965861"
