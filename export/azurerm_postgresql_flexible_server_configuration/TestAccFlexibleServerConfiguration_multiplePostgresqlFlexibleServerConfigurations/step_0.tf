...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-postgresql-221021034447710941"
  location = "West Europe"
}


resource "azurerm_virtual_network" "test" {
  name                = "acctest-vn-221021034447710941"
...
resource "azurerm_subnet" "test" {
  name                 = "acctest-sn-221021034447710941"
  resource_group_name  = azurerm_resource_group.test.name
...
resource "azurerm_private_dns_zone" "test" {
  name                = "acc221021034447710941.postgres.database.azure.com"
  resource_group_name = azurerm_resource_group.test.name
}

resource "azurerm_private_dns_zone_virtual_network_link" "test" {
  name                  = "acctestVnetZone221021034447710941.com"
...
resource "azurerm_postgresql_flexible_server" "test" {
  name                   = "acctest-fs-221021034447710941"
  resource_group_name    = azurerm_resource_group.test.name
  location               = azurerm_resource_group.test.location
