...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-PAN-231218072332096081"
  location = "West Europe"
}

resource "azurerm_palo_alto_local_rulestack_certificate" "test" {
  name         = "testacc-palc-231218072332096081"
...
resource "azurerm_palo_alto_local_rulestack_certificate" "trust" {
  name         = "testacc-palcT-231218072332096081"
  rulestack_id = azurerm_palo_alto_local_rulestack.test.id
...
resource "azurerm_palo_alto_local_rulestack_certificate" "untrust" {
  name         = "testacc-palcU-231218072332096081"
  rulestack_id = azurerm_palo_alto_local_rulestack.test.id
...
resource "azurerm_palo_alto_local_rulestack" "test" {
  name                = "testAcc-palrs-231218072332096081"
  resource_group_name = azurerm_resource_group.test.name
  location            = "West Europe"
}
...
resource "azurerm_palo_alto_local_rulestack_rule" "test" {
  name         = "testacc-palr-231218072332096081"
  rulestack_id = azurerm_palo_alto_local_rulestack.test.id
  priority     = 100
  action       = "Allow"
