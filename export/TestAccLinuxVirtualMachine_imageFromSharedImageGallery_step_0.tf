...
locals {
  ...
  vm_name           = "acctestsourcevm-220909034011220602"
  admin_username    = "testadmin220909034011220602"
  admin_password    = "Password1234!220909034011220602"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220909034011220602"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-220909034011220602"
...
resource "azurerm_public_ip" "test" {
  name                = "acctpip-220909034011220602"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_network_interface" "public" {
  name                = "acctnicsource-220909034011220602"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-220909034011220602"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig220909034011220602"
  resource_group_name = "${azurerm_resource_group.test.name}"
  location            = "${azurerm_resource_group.test.location}"
}
...
resource "azurerm_linux_virtual_machine" "source" {
  name                            = "acctestsourceVM-220909034011220602"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_F2"
