...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-backup-230512004646882099"
...
resource "azurerm_storage_share" "testshare1" {
  name                 = "acctest-ss-230512004646882099-1"
...
resource "azurerm_storage_share" "testshare2" {
  name                 = "acctest-ss-230512004646882099-2"
  storage_account_name = "${azurerm_storage_account.test1.name}"
  quota                = 1
  metadata             = {}
...
resource "azurerm_recovery_services_vault" "test" {
  name                = "acctest-VAULT-230512004646882099"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
  sku                 = "Standard"
...
resource "azurerm_backup_policy_file_share" "test" {
  name                = "acctest-PFS-230512004646882099"
  resource_group_name = "${azurerm_resource_group.test.name}"
