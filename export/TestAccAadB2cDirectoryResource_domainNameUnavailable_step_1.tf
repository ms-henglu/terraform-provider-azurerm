...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220630210415542748"
...
resource "azurerm_aadb2c_directory" "test" {
  ...
  display_name            = "acctest220630210415542748"
  domain_name             = "acctest220630210415542748.onmicrosoft.com"
...
resource "azurerm_resource_group" "duplicate" {
  name     = "acctestRG-duplicate-220630210415542748"
  location = "West US 2"
}
...
resource "azurerm_aadb2c_directory" "duplicate" {
  ...
  display_name            = "acctest-duplicate-220630210415542748"
