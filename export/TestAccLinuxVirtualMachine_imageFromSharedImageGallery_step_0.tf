...
locals {
  ...
  vm_name           = "acctestsourcevm-220324163104777544"
  admin_username    = "testadmin220324163104777544"
  admin_password    = "Password1234!220324163104777544"
}

resource "azurerm_resource_group" "test" {
  name     = "acctestRG-220324163104777544"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-220324163104777544"
...
resource "azurerm_public_ip" "test" {
  name                = "acctpip-220324163104777544"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_network_interface" "public" {
  name                = "acctnicsource-220324163104777544"
  location            = "${azurerm_resource_group.test.location}"
  resource_group_name = "${azurerm_resource_group.test.name}"
...
resource "azurerm_network_interface" "test" {
  name                = "acctestnic-220324163104777544"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_shared_image_gallery" "test" {
  name                = "acctestsig220324163104777544"
  resource_group_name = "${azurerm_resource_group.test.name}"
  location            = "${azurerm_resource_group.test.location}"
}
...
resource "azurerm_linux_virtual_machine" "source" {
  name                            = "acctestsourceVM-220324163104777544"
  resource_group_name             = azurerm_resource_group.test.name
  location                        = azurerm_resource_group.test.location
  size                            = "Standard_F2"
