...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-functionapp-210825041454279337"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctest-VNET-210825041454279337"
...
resource "azurerm_app_service_plan" "test" {
  name                = "acctest-ASP-210825041454279337"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_function_app" "test" {
  name                       = "acctest-FA-210825041454279337"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
...
resource "azurerm_function_app_slot" "test-staging" {
  name                       = "acctest-FA-210825041454279337-staging"
  resource_group_name        = azurerm_resource_group.test.name
  location                   = azurerm_resource_group.test.location
  app_service_plan_id        = azurerm_app_service_plan.test.id
