...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-fw-211203161415687303"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestvirtnet211203161415687303"
...
resource "azurerm_public_ip" "test" {
  name                = "acctestpip211203161415687303"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall" "test" {
  name                = "acctestfirewall211203161415687303"
  location            = azurerm_resource_group.test.location
  resource_group_name = azurerm_resource_group.test.name
...
resource "azurerm_firewall_nat_rule_collection" "test" {
  name                = "acctestnrc-211203161415687303"
