...
resource "azurerm_resource_group" "test" {
  name     = "acctestRG-240112034043053819"
  location = "West Europe"
}

resource "azurerm_virtual_network" "test" {
  name                = "acctestnw-240112034043053819"
...
resource "azurerm_linux_virtual_machine_scale_set" "test" {
  name                = "acctestvmss-240112034043053819"
  resource_group_name = azurerm_resource_group.test.name
  location            = azurerm_resource_group.test.location
  sku                 = "Standard_F2"
  instances           = 1
...
resource "azurerm_key_vault" "test" {
  ...
  name                   = "acctestkv${count.index}b7lf0"
  location               = azurerm_resource_group.test.location
  resource_group_name    = azurerm_resource_group.test.name
...
resource "azurerm_virtual_machine_scale_set_extension" "test" {
  name                         = "acctestExt-240112034043053819"
